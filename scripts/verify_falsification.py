...
def main():
    ...
    print(
        f"  Loaded: {len(chunks_df)} chunks, {len(nodes_df)} nodes, {len(edges_df)} edges"
    )
    ...
    for doc_id, name in top_docs:
        doc_chunks = (
            chunks_df[chunks_df["doc_id"] == doc_id]["chunk_text"].head(10).tolist()
        )
        if doc_chunks:
    ...
    for doc_id in test_docs:
        ...
        inflation = (
            "⚠️ INFLATED" if abs(chunk_count - in_degree) < 50 else "✅ Semantic"
        )
        print(
            f"  {doc_id[:16]}: chunks={chunk_count}, in_degree={in_degree}, out={out_degree} {inflation}"
        )
    ...
    if "relation" in edges_df.columns:
        ...
        print(
            f"  Graph density without PART_OF: {semantic_edges / len(nodes_df):.3f} edges/node"
        )
    ...
    entity_degrees = edges_df[edges_df["target"].str.startswith("ent::")][
        "target"
    ].value_counts()
    ...
    if "kind" in nodes_df.columns:
        ...
        if len(unique_kinds) <= 2:
            print(
                "  ⚠️ TRIVIAL TAXONOMY: Only 2 kinds means cross-domain is by definition"
            )
    else:
    ...
    if "relation" in edges_df.columns:
        part_of_pct = (
            edges_df["relation"].value_counts().get("PART_OF", 0) / len(edges_df) * 100
        )
        if part_of_pct > 80:
