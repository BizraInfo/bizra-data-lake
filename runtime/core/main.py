...
from core.synapse import SynapseConnection
from core.mission_router import router as mission_router
...
REQUESTS_TOTAL = Counter(
    "bizra_kernel_requests_total", "Total requests", ["path", "method", "status"]
)
REQUEST_LATENCY = Histogram(
    "bizra_kernel_request_latency_seconds", "Request latency", ["path", "method"]
)
FATE_VERDICTS = Counter(
    "bizra_kernel_fate_verdict_total", "FATE verdict totals", ["verdict"]
)
GRAPH_QUERIES = Counter(
    "bizra_kernel_graph_queries_total", "Graph query totals", ["status"]
)
GRAPH_QUERY_LATENCY = Histogram(
    "bizra_kernel_graph_query_latency_seconds", "Graph query latency"
)

SAPE_REQUESTS = Counter(
    "bizra_kernel_sape_requests_total", "Total SAPE requests", ["endpoint", "status"]
)
SAPE_LLM_CALLS = Counter(
    "bizra_kernel_sape_llm_calls_total",
    "Total SAPE LLM calls",
    ["provider", "model", "status"],
)
SAPE_LLM_LATENCY = Histogram(
    "bizra_kernel_sape_llm_latency_seconds",
    "SAPE LLM call latency",
    ["provider", "model"],
)
...
def _require_token(expected: str, token: str) -> None:
    if not expected:
        raise HTTPException(
            status_code=503, detail="kernel misconfigured: BIZRA_API_TOKEN not set"
        )
    if token != expected:
...
def _receipt_dir() -> Path:
    ...
    return (
        Path(__file__).resolve().parents[1] / "docs" / "evidence" / "receipts"
    ).resolve()
...
def _write_receipt(payload: Dict[str, Any]) -> Optional[Path]:
    ...
    folder = (
        base
        / f"kernel_request_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%SZ')}_{payload['request_id']}"
    )
...
cors_origins = [
    o.strip()
    for o in os.getenv(
        "BIZRA_CORS_ORIGINS", "http://localhost,http://localhost:3000"
    ).split(",")
    if o.strip()
]
app.add_middleware(
...
# ━━━ Mission Router v2 (constitutional pipeline bridge) ━━━
app.include_router(mission_router, prefix="/api")

fate_level = _env_name("BIZRA_FATE_STRICT", "FATE_LEVEL", "1")
fate = FateEngine(
    strict_mode=fate_level.strip() != "0"
    and fate_level.strip().lower() not in {"lenient", "off"}
)
...
def _get_model_family() -> ModelFamily:
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"model_family_unavailable: {_MODEL_FAMILY_ERROR}"
        ) from e
...
def _candidate_models(mf: ModelFamily, slot: str) -> List[Dict[str, Any]]:
    ...
    ollama_base = (
        os.getenv("OLLAMA_URL") or os.getenv("OLLAMA_HOST") or "http://127.0.0.1:11434"
    ).strip()
    lmstudio_base = (
        ...
        out.append(
            {
                ...
                "model_id": a.model_id,
                "endpoint_base_url": (
                    ollama_base if a.provider == "ollama" else lmstudio_base
                ),
            }
        )
    return out
...
def _collect_graph_evidence(
    ...
    topics = [
        t for t in (req.evidence_topics or []) if isinstance(t, str) and t.strip()
    ]
    if not topics:
        topics = [req.domain]
...
async def healthz():
    ...
    redis_timeout_s = max(
        0.05, min(_env_float("BIZRA_HEALTHZ_REDIS_TIMEOUT_S", 0.35), budget_s)
    )
    neo4j_timeout_s = max(
        0.05, min(_env_float("BIZRA_HEALTHZ_NEO4J_TIMEOUT_S", 0.55), budget_s)
    )
    llm_timeout_s = max(
        0.05, min(_env_float("BIZRA_HEALTHZ_LLM_TIMEOUT_S", 0.25), budget_s)
    )
    ...
    async def _run() -> tuple[Dict[str, Any], int]:
        ...
        checks["kernel_config"] = {
            "ok": token_set,
            "detail": (
                "BIZRA_API_TOKEN is set" if token_set else "BIZRA_API_TOKEN missing"
            ),
        }
        if not token_set:
            healthy = False
        ...
        def _llm_provider_status(
            provider: str, *, enabled: bool, details: Dict[str, Any]
        ) -> Dict[str, Any]:
            if not enabled:
        ...
        async def _llm_check() -> Dict[str, Any]:
            ...
            async def _tcp(url: str) -> Dict[str, Any]:
                ...
                try:
                    timeout = min(
                        llm_timeout_s, max(0.05, budget_s - (time.monotonic() - t0))
                    )
                    await asyncio.wait_for(
                        asyncio.to_thread(_tcp_connect_sync, url, timeout),
                        timeout=timeout,
                    )
                    return {
                        "ok": True,
            ...
            results = (
                await asyncio.gather(*tasks_llm, return_exceptions=False)
                if tasks_llm
                else []
            )
        ...
        async def _redis_check() -> Dict[str, Any]:
            ...
            try:
                ...
                if conn.connect():
                    ...
                    is_fake = (
                        "fakeredis" in str(type(conn.redis))
                        or redis_version == "unknown"
                    )
                    ...
                    return {
                        ...
                        "mode": (
                            "autonomous/fakeredis" if is_fake else "connected/synapse"
                        ),
                        "latency_ms": round((time.monotonic() - start) * 1000.0, 2),
                    }
                else:
        ...
        async def _neo4j_check() -> Dict[str, Any]:
            ...
            if not auth:
                return {
                    ...
                    "error": "invalid NEO4J_AUTH (expected user/pass)",
                }
            try:
                timeout = min(
                    neo4j_timeout_s, max(0.05, budget_s - (time.monotonic() - t0))
                )
                await asyncio.wait_for(
                    asyncio.to_thread(
                        _neo4j_verify_sync, uri=wisdom_url, auth=auth, timeout_s=timeout
                    ),
                    timeout=timeout,
                )
                return {
        ...
        if not isinstance(llm_details, dict):
            llm_details = {}
        checks["ollama"] = _llm_provider_status(
            "ollama", enabled=bool(ollama_cfg), details=llm_details
        )
        checks["lmstudio"] = _llm_provider_status(
            "lmstudio", enabled=bool(lmstudio_cfg), details=llm_details
        )
    ...
    try:
        ...
        if (
            cache_ttl_s > 0
            and _HEALTHZ_CACHE
            and (now2 - _HEALTHZ_CACHE[0]) <= cache_ttl_s
        ):
            cached_body, cached_status = _HEALTHZ_CACHE[1], _HEALTHZ_CACHE[2]
            body = dict(cached_body)
            body["cached"] = True
...
async def agent_query_knowledge(request: AgentRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"knowledge_graph_offline: {e}"
        ) from e
...
@app.post(
    "/v1/sape/plan",
    response_model=SapePlanResponse,
    dependencies=[Depends(verify_token)],
)
async def sape_plan(request: SapePlanRequest):
    ...
    if request.require_graph_evidence:
        ...
        if request.stakes == "H" and not evidence:
            SAPE_REQUESTS.labels(
                endpoint="/v1/sape/plan", status="BLOCKED_BY_EVIDENCE"
            ).inc()
            payload = {
                "schema": "bizra_sape_plan_receipt_v1",
                "generated_at": utc_now_iso(),
    ...
    if os.getenv("BIZRA_KERNEL_RECEIPTS_INCLUDE_PROMPTS", "0").strip() in {
        "1",
        "true",
        "TRUE",
        "yes",
...
@app.post(
    "/v1/sape/execute",
    response_model=SapeExecuteResponse,
    dependencies=[Depends(verify_token)],
)
async def sape_execute(request: SapeExecuteRequest):
    ...
    if seal.verdict == "REJECTED":
        SAPE_REQUESTS.labels(
            endpoint="/v1/sape/execute", status="BLOCKED_BY_FATE"
        ).inc()
        payload = {
    ...
    if request.require_graph_evidence:
        ...
        if request.stakes == "H" and not evidence:
            SAPE_REQUESTS.labels(
                endpoint="/v1/sape/execute", status="BLOCKED_BY_EVIDENCE"
            ).inc()
            payload = {
                "schema": "bizra_sape_execute_receipt_v1",
                "generated_at": utc_now_iso(),
    ...
    try:
        ...
        return SapeExecuteResponse(
            ...
            prompt_sha256=compiled.prompt_sha256,
            system_prompt=(
                compiled.system_prompt if request.include_prompts_in_response else None
            ),
            user_prompt=(
                compiled.user_prompt if request.include_prompts_in_response else None
            ),
            evidence=evidence,
...
@app.get(
    "/v1/quality/radar",
    response_model=QualityRadarResponse,
    dependencies=[Depends(verify_token)],
)
    ...
    try:
        ...
        return QualityRadarResponse(
            ...
            invariants_passed=sum(
                1 for i in data.get("invariants", []) if i.get("passed")
            ),
...
@app.post(
    "/v1/ecosystem/seal",
    response_model=EcosystemSealResponse,
    dependencies=[Depends(verify_token)],
)
async def ecosystem_seal(req: EcosystemSealRequest):
    ...
    try:
        ...
        manifest = build_manifest(
            repo_root=repo_root, cfg=cfg, max_projects=req.max_projects
        )
...
def _extract_meta_prompt_evidence(
...
async def meta_prompt_query(req: MetaPromptRequest):
    ...
    pre_context = _safe_json_snippet(
        {"context": req.context, "preferences": req.preferences}
    )
...
class CognitiveRequest(BaseModel):
    ...
    slot: Optional[str] = Field(
        None, description="Model slot (cold_core, fast, thinking, etc.)"
    )
    hook: str = Field(
        "default", description="Named hook (default, fast, planner, thinker)"
    )
...
@app.get(
    "/v1/cognitive/slots", tags=["Cognitive T1.2"], dependencies=[Depends(verify_token)]
)
...
@app.get(
    "/v1/cognitive/stats", tags=["Cognitive T1.2"], dependencies=[Depends(verify_token)]
)
...
class RustCognitiveRequest(BaseModel):
    ...
    context_vector: list[float] = Field(
        default_factory=list, description="Context embeddings"
    )
    mode: str = Field(
        "HybridSynergy",
        description="ThinkingMode: FastPat, DeepSat, HybridSynergy, Reflexion, GraphOfThought",
    )
...
@app.post(
    "/process",
    response_model=RustCognitiveResponse,
    tags=["Rust Bridge"],
    dependencies=[Depends(verify_token)],
)
...
@app.post(
    "/v1/cognitive/recursion/cycle",
    tags=["Peak Masterpiece"],
    dependencies=[Depends(verify_token)],
)
async def trigger_recursive_cycle(req: RecursiveCycleRequest):
    ...
    if not _COGNITIVE_EXPANDER:
        raise HTTPException(
            status_code=503, detail="Cognitive Expander not initialized"
        )
...
async def invoke_constellation(req: ConstellationRequest):
    ...
    if not _CONSTELLATION_ORCHESTRATOR:
        raise HTTPException(
            status_code=503, detail="Constellation Orchestrator not initialized"
        )

    # Synchronous execution
    try:
        output = _CONSTELLATION_ORCHESTRATOR.execute(
            req.query, context={"stakes": req.stakes}
        )
        # Serialize dataclass
        return asdict(output)
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Constellation execution failed: {str(e)}"
        )
...
@app.get(
    "/voice/moshi/status", tags=["Moshi Voice"], dependencies=[Depends(verify_token)]
)
...
class UnifiedCognitionRequest(BaseModel):
    ...
    task: str = Field(
        ...
        min_length=1,
        max_length=10000,
        description="Task description for dual-agentic processing",
    )
    user_id: str = Field("anonymous", description="Optional user identifier")
    ihsan_floor: float = Field(
        0.95, ge=0.0, le=1.0, description="Minimum Ihsan score required"
    )
    ...
    taint_secrecy: str = Field(
        "Internal", description="Secrecy level: Public, Internal, Confidential, Secret"
    )
    taint_integrity: str = Field(
        "Untrusted",
        description="Integrity level: Untrusted, Validated, Attested, Sovereign",
    )
...
class UnifiedCognitionResponse(BaseModel):
    ...
    signature: Optional[str] = Field(
        None, description="Ed25519 signature of response (hex-encoded)"
    )
    signer_public_key: Optional[str] = Field(
        None, description="Signer public key (hex-encoded)"
    )
    sat_validation_ms: Optional[int] = Field(
        None, description="SAT validation latency in milliseconds"
    )
    pat_execution_ms: Optional[int] = Field(
        None, description="PAT execution latency in milliseconds"
    )
...
@app.post(
    "/v1/cognition",
    response_model=UnifiedCognitionResponse,
    tags=["Unified Cognition"],
    dependencies=[Depends(verify_token)],
)
...
async def agentic_flow_worker(req: AgenticFlowWorkerRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid worker type: {req.worker_type}"
        )
...
async def startup_event():
    ...
    try:
        ...
        _COGNITIVE_EXPANDER = RecursiveExpander(
            config={"max_cycles": 5, "complexity_multiplier_base": 1.2}
        )
        ...
        if constellation_path.exists():
            _CONSTELLATION_ORCHESTRATOR = ConstellationOrchestrator(
