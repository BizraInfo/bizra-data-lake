...
def check_generic_exceptions() -> CheckResult:
    ...
    severity = (
        "CRITICAL" if critical_count > MAX_GENERIC_EXCEPTIONS_CRITICAL else "WARNING"
    )
    ...
    for py_file in get_source_files():
        ...
        try:
            ...
            for i, line in enumerate(content.splitlines(), 1):
                ...
                if (
                    "if __name__" in content[: content.find(line)]
                    and "__main__" in content[: content.find(line)]
                ):
...
def check_consensus_claims() -> CheckResult:
    ...
    return CheckResult(
        ...
        severity="WARNING",
        message=(
            "Consensus model appears honestly labeled"
            if has_honest_label
            else "Consensus model labeling unclear"
        ),
        details=[],
    )
...
def check_pqc_status() -> CheckResult:
    ...
    for py_file in get_source_files():
        try:
            ...
            if (
                "pqcrypto" in content
                or "dilithium" in content.lower()
                or "kyber" in content.lower()
            ):
...
def check_docker_compose_exists() -> CheckResult:
    ...
    return CheckResult(
        name=name, passed=False, severity="INFO", message="docker-compose.yml not found"
    )
...
def create_genesis_manifest() -> Path:
    ...
    manifest = {
        ...
        "deployment": {"mode": "SOLO_OPERATOR", "nodes": 1, "distributed": False},
        "attestation": {
            "operator": "UNSIGNED",
...
def run_all_checks() -> GenesisReadinessReport:
    ...
    report = GenesisReadinessReport(timestamp=datetime.now(timezone.utc).isoformat())
...
def main():
    ...
    parser.add_argument(
        "--create-manifest", action="store_true", help="Create genesis-manifest.yaml"
    )
