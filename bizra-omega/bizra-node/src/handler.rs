...
pub fn handle(cmd: Command, state: &mut NodeInternals<'_>) -> Response {
    match cmd {
        ...
        Command::MissionReceive { content, timestamp } => {
            handle_mission_receive(state, &content, timestamp)
        }
        Command::Teach {
            kind,
            content,
...
fn handle_resources(state: &mut NodeInternals<'_>) -> Response {
    ...
    Response::ok(vec![
        ...
        (
            "total_model_storage_gb",
            format!("{:.2}", m.total_model_storage_gb),
        ),
        ("models", model_str),
...
fn handle_mission_receive(
    ...
    *state.last_receipt_id = Some(result.receipt.receipt_id);
    ...
    let mut fields = vec![
        ...
        (
            "states_traversed",
            format!("{}", mission.state_history.len()),
        ),
    ];
    ...
    if !mission.degradation_reasons.is_empty() {
        let reasons: Vec<String> = mission
            .degradation_reasons
            .iter()
            .map(|r| format!("{:?}", r))
            .collect();
