...
SKIP = {'.git','.venv','node_modules','__pycache__','.mypy_cache','.pytest_cache','.ruff_cache','.hypothesis','.cache','dist','.benchmarks','.fastembed_cache'}

RULES = {
    'sacred':{'pat':['al_risalah','al_bazrah','الرسالة','البذرة','the massage'],'pth':['01_ARABIC_FOUNDING']},
    'research_paper':{'ext':['.tex','.bib'],'pat':['arxiv','latex','hierarchical-coordination','recursive language'],'pth':['research-papers','research_archive']},
    'architecture_spec':{'pat':['architecture','blueprint','unified','ddagi'],'pth':['docs/architecture','docs/adr']},
    'phase_spec':{'pat':['phase_'],'pth':['docs/specs']},
    'constitution':{'pat':['constitution','ihsan','soul.md','identity.md','covenant','genesis_covenant'],'pth':['constitution']},
    'evidence':{'pat':['evidence','proof','receipt','ledger','attestation'],'pth':['sovereign_state','evidence-pack','04_GOLD']},
    'token_econ':{'pat':['token','seed','bloom','zakat','tokenomics']},
    'investor':{'pat':['investor','pitch','strategy_deck','revenue','competitive','moneyshot','technical_brief']},
...
def classify(fp, fn):
    ...
    for cat, r in RULES.items():
        hit = False
        for p in r.get('pth',[]):
            if p.lower() in fpl: hit=True; break
        if not hit:
            for p in r.get('pat',[]):
                if p.lower() in fnl or p.lower() in fpl: hit=True; break
        if not hit:
            for e in r.get('ext',[]):
                if ext.lower()==e: hit=True; break
        if hit: cats.append(cat)
    ...
    for dp, dns, fns in os.walk(root):
        ...
        for fn in fns:
            ...
            try:
                ...
                _, ext = os.path.splitext(fn)
                assets.append({
                    'filename': fn, 'ext': ext.lower(),
                    'abs': fp, 'root': root,
                    'bytes': st.st_size, 'size': fmt(st.st_size),
                    'modified': datetime.fromtimestamp(st.st_mtime).strftime('%Y-%m-%d'),
                    'categories': cats, 'primary': cats[0],
                })
            except: pass
    return assets
...
for a in all_a:
    ...
    for c in a.get('categories',[]): cats[c]+=1
    exts[a.get('ext','')] += 1
    if 'pollution' in a.get('categories',[]): pollution+=1

print(f"\n{'='*60}")
print(f"TOTAL: {len(all_a):,} unique files | {fmt(total_sz)}")
print(f"Pollution: {pollution} | Sacred: {cats.get('sacred',0)} | IP docs: {cats.get('architecture_spec',0)+cats.get('phase_spec',0)+cats.get('research_paper',0)+cats.get('constitution',0)}")
print(f"{'='*60}")
...
print("\nEXTENSIONS (top 15):")
for k,v in sorted(exts.items(), key=lambda x:-x[1])[:15]: print(f"  {k or '(none)'}: {v:,}")

# Sacred
sacred = [a for a in all_a if 'sacred' in a.get('categories',[]) or 'constitution' in a.get('categories',[])]
print(f"\nSACRED/CONSTITUTIONAL ({len(sacred)}):")
for s in sorted(sacred, key=lambda x:x['filename']): print(f"  {s['filename']} ({s['size']}) [{s['modified']}]")

# IP assets
ip_cats = {'research_paper','architecture_spec','phase_spec','constitution','golden_gem','token_econ','investor'}
...
# Filename duplicates
nm = defaultdict(list)
for a in all_a: nm[a['filename']].append(a)
dupes = {k:v for k,v in nm.items() if len(v)>1 and any(e in k.lower() for e in ['.md','.pdf','.yaml','.json','.py'])}
if dupes:
...
manifest = {
    'version':'1.1','mission':'NODE0-MISSION-001','timestamp':datetime.now().isoformat(),
    'totals':{'files':len(all_a),'size':fmt(total_sz),'bytes':total_sz,'pollution':pollution,
              'sacred':len(sacred),'ip':len(ip),'duplicates':len(dupes)},
...
with open(os.path.join(out,'node0_ip_assets.json'),'w',encoding='utf-8') as f:
    json.dump([{'name':a['filename'],'cats':a['categories'],'size':a['size'],'path':a['abs'],'modified':a['modified']} for a in ip],
              f,indent=2,ensure_ascii=False)

with open(os.path.join(out,'node0_full_inventory.jsonl'),'w',encoding='utf-8') as f:
    for a in all_a: f.write(json.dumps(a,ensure_ascii=False)+'\n')

with open(os.path.join(out,'node0_sacred_assets.json'),'w',encoding='utf-8') as f:
    json.dump([{'name':a['filename'],'cats':a['categories'],'size':a['size'],'path':a['abs'],'modified':a['modified']} for a in sacred],
              f,indent=2,ensure_ascii=False)
