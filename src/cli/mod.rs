...
use meta_alpha_dual_agentic::{
    create_http_server, ihsan, pat_enhanced, MetaAlphaDualAgentic,
};
use meta_alpha_dual_agentic::types::{
    DualAgenticRequest, EnhancedDualAgenticRequest,
};
...
pub async fn run() -> anyhow::Result<()> {
    ...
    match cli.command.unwrap_or(Commands::Serve { port: 8080 }) {
        ...
        Commands::Status => {
            ...
            table.add_row(vec![
             ...
             table.add_row(vec![
                ...
                Cell::new(format!("{} (Threshold: {:.2})", constitution.id(), constitution.threshold()))
            ]);
            table.add_row(vec![
        ...
        Commands::Task { prompt, spawn } => {
            ...
            match enhanced_pat.execute_enhanced(request).await {
                Ok(response) => {
                    ...
                    println!("\n{}\n", response.pat_contributions.first().cloned().unwrap_or_default());
                    
                    let mut stats = Table::new();
                    stats.load_preset(UTF8_FULL);
                    stats.set_header(vec!["Metric", "Value"]);
                    stats.add_row(vec!["Synergy Score", &format!("{:.3}", response.synergy_score)]);
                    stats.add_row(vec!["Ihsan Integrity", &format!("{:.3}", response.ihsan_score)]);
                    stats.add_row(vec!["Latency", &format!("{:.2}s", response.latency.as_secs_f64())]);
        ...
        Commands::Models => {
             ...
             let mut table = Table::new();
             table.load_preset(UTF8_FULL).set_header(vec!["Slot", "Model", "Provider"]);
