    ...
    if parsed and isinstance(parsed, dict):
        ...
        for line in txt.splitlines():
            ...
            if m:
                ...
                continue
            m = re.match(
                r"^([A-Za-z0-9_-]+)\s*=\s*\\{.*version\\s*=\\s*\"([^\"]+)\".*\\}", s
            )
            if m:
                deps[m.group(1)] = m.group(2)
...
def parse_package_jsons(repo_root: Path) -> Dict[str, Any]:
    ...
    for p in iter_files(repo_root):
        ...
        try:
            ...
            packages.append(
                {"path": sanitize_rel_path(p, repo_root), "error": "parse_failed"}
            )
...
def build_summary_text(inv: Dict[str, Any]) -> str:
    ...
    for r in (files.get("by_extension") or [])[:10]:
        lines.append(
            f"  - {r.get('ext')}: files={r.get('files')} bytes={r.get('bytes')}"
        )
    lines.append("")
    ...
    if cargo.get("present"):
        ...
        lines.append(
            f"- crate: {pkg.get('name')} {pkg.get('version')} (edition {pkg.get('edition')})"
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="High-SNR codebase inventory (evidence-first)."
    )
    ap.add_argument(
        "--repo-root", default="", help="Repo root (defaults to script parent)"
    )
    ap.add_argument("--out", default="", help="Write JSON to this path")
    ap.add_argument(
        "--summary-out", default="", help="Write a compact context text file"
    )
    args = ap.parse_args()
    ...
    by_ext_rows = [
        {
            "ext": k,
            "files": int(ext_files[k]),
            "bytes": int(ext_bytes[k]),
            "truth_label": "MEASURED",
        }
    ...
    loc_rows = [
        {
            "ext": k,
            "loc": int(loc_by_ext[k]),
            "files": int(loc_files_by_ext[k]),
            "truth_label": "MEASURED",
        }
    ...
    inv: Dict[str, Any] = {
        ...
        "files": {
            ...
            "largest_files": [
                {"bytes": int(sz), "path": p, "truth_label": "MEASURED"}
                for sz, p in largest
            ],
            "truth_label": "MEASURED",
        },
        "loc": {
            ...
            "truth_label": "MEASURED",
            "notes": [
                f"Counts only files <= {DEFAULT_MAX_BYTES_TEXT_SCAN} bytes; binary/large files skipped."
            ],
        },
        "dependencies": {
