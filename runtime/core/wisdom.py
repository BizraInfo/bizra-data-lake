...
def _graph_config() -> GraphConfig:
    uri = os.getenv(
        "WISDOM_URL",
        os.getenv("NEO4J_URI", os.getenv("BIZRA_NEO4J_URI", "bolt://localhost:7687")),
    ).strip()
    ...
    if auth:
        user, password = auth
    database = (
        os.getenv("NEO4J_DATABASE", os.getenv("BIZRA_NEO4J_DATABASE", "")).strip()
        or None
    )
...
class HouseOfWisdom:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._session() as session:
            row2 = session.run(
                "MATCH (a:Artifact) RETURN count(a) AS total_files, sum(a.impact_value) AS total_value_bzr_g"
            ).single()
        return {
            "status": "ONLINE",
