...
class InstructionSynthesizer:
        ...
        for root, dirs, files in os.walk(self.workspace):
            ...
            for filename in files:
                ...
                if (
                    ...
                    in CODE_EXTENSIONS | DOC_EXTENSIONS | CONFIG_EXTENSIONS
                ):
                    yield filepath
    ...
    def verify_pair_ihsan(self, pair: SynthesisPair) -> float:
        ...
        try:
            ...
            resp = requests.post(
                f"{self.kernel_url}/v1/fate/evaluate", json=payload, timeout=10
            )
    ...
    def synthesize_docgen(
        self, code: str, lang: str, source_file: str
        ...
        code_without_doc = re.sub(
            r"'''.*?'''", "", code_without_doc, count=1, flags=re.DOTALL
        )
    ...
    def synthesize_from_file(self, path: Path) -> Generator[SynthesisPair, None, None]:
        ...
        if lang == "python":
            ...
            for func_code, _, _ in functions:
                ...
                if (
                    SynthesisStrategy.REVIEW in self.strategies
                ...
                for pair in self.synthesize_from_file(filepath):
                    ...
                    if pair.ihsan_score >= IHSAN_THRESHOLD:
                        ...
                        if self.stats.accepted_pairs % 100 == 0:
                            print(
                                f"  📝 Generated {self.stats.accepted_pairs} pairs..."
                            )
                    else:
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", type=str, default=None, help="Output JSONL file path"
    )
    parser.add_argument(
        "--strategy",
        "-s",
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify pairs with Ihsān gate"
    )
    parser.add_argument(
        "--kernel-url",
        type=str,
    ...
    if args.output:
        ...
        output_path = (
            ...
            / "bizra_data_vault"
            / "training"
            / f"instructions_{timestamp}.jsonl"
        )
