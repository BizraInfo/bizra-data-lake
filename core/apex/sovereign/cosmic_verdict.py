...
class CosmicVerdictResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "decision": self.decision.value,
            "guardian_votes": {
                role.value: vote.to_dict()
                for role, vote in self.guardian_votes.items()
            },
            "majlis_decision": self.majlis_decision.value,
            "persona_consensus": self.persona_consensus.to_dict() if self.persona_consensus else None,
...
class CosmicVerdictEngine:
    ...
    async def execute_verdict(
        ...
        if quorum_result.qualified_vetoes and quorum_result.approve_count >= QUALIFIED_OVERRIDE_QUORUM:
            logger.info(
    ...
    async def _phase_scout(self, request: VerdictRequest) -> ScoutResult:
        ...
        logger.debug(
            f"SCOUT phase complete: {len(votes)} votes, {duration_ms:.1f}ms"
        )
    ...
    async def _guardian_independent_reasoning(
        ...
        # Al-Amin evaluates security
            ...
            if request.snr_score < SNR_THRESHOLD:
                decision = VoteDecision.REJECT
                reasoning = f"SNR score {request.snr_score:.3f} below threshold {SNR_THRESHOLD}"
                raw_confidence = 0.94
    ...
    def _generate_evidence_chain(
        ...
        waggle_data = {
            "phase": "waggle",
            "evidence_scores": {k.value: v for k, v in waggle_result.evidence_scores.items()},
            "duration_ms": waggle_result.duration_ms,
        }
        chain.append(f"waggle:{self._hash(json.dumps(waggle_data, sort_keys=True).encode())[:16]}")
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        print(f"  BFT Parameters: n={stats['bft_parameters']['n']}, "
