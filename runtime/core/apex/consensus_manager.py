...
class SecurityGuardian(SATValidator):
    ...
    async def validate(self, request: DualAgenticRequest) -> SATVote:
        ...
        return SATVote(
            ...
            rejection_codes=(
                [f"THREAT:{t}" for t in threats_found] if threats_found else []
            ),
            latency_ms=latency_ms,
            reasoning=(
                f"Found {len(threats_found)} threat patterns"
                if threats_found
...
class EthicsValidator(SATValidator):
    ...
    async def validate(self, request: DualAgenticRequest) -> SATVote:
        ...
        return SATVote(
            ...
            confidence=confidence,
            rejection_codes=(
                [f"ETHICS:{v}" for v in violations_found] if violations_found else []
            ),
            latency_ms=latency_ms,
            reasoning=(
                f"Found {len(violations_found)} ethical concerns"
                if violations_found
                else "No ethical violations"
            ),
        )
...
SAPE_BATCHES: Dict[str, List[str]] = {
    "batch_1": [
        SAPEProbeType.THREAT_SCAN.value,
        SAPEProbeType.COMPLIANCE.value,
        SAPEProbeType.BIAS.value,
    ],
    "batch_2": [
        SAPEProbeType.USER_BENEFIT.value,
        SAPEProbeType.CORRECTNESS.value,
        SAPEProbeType.SAFETY.value,
    ],
    "batch_3": [
        SAPEProbeType.GROUNDEDNESS.value,
        SAPEProbeType.RELEVANCE.value,
        SAPEProbeType.FLUENCY.value,
    ],
}
...
class ConsensusManager:
    ...
    async def obtain_sat_consensus(
        self, request: DualAgenticRequest
    ) -> ConsensusResult:
        """
        ...
        vote_data = "|".join(
            f"{v.validator_id}:{v.approved}:{v.confidence}" for v in votes
        )
    ...
    async def run_sape_probes(self, context: ProbeContext) -> SAPEResult:
        ...
        for batch_name, probe_names in SAPE_BATCHES.items():
            ...
            probe_tasks = [
                SAPE_PROBES[probe_name](context) for probe_name in probe_names
            ]
        ...
        self._elevation_patterns[content_hash] = (
            self._elevation_patterns.get(content_hash, 0) + 1
        )
