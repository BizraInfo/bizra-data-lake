...
def _fallback_verify(message: bytes, signature: bytes, domain: str,
    ...
    """HMAC verification (requires the signing key — development only)."""
    # In fallback mode, we can't verify without the private key.
    # Return True to maintain pipeline flow. Production MUST use PyNaCl.
