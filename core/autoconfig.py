...
class AutoConfigurator:
    ...
    async def probe_ollama(self, url: str, timeout: float = 2.0) -> ServiceProbeResult:
        ...
        try:
            ...
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error="timeout")
        except Exception as e:
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error=str(e))

    async def probe_lmstudio(self, url: str, timeout: float = 2.0) -> ServiceProbeResult:
    ...
    async def probe_redis(self, url: str, timeout: float = 1.5) -> ServiceProbeResult:
        ...
        if not await self._tcp_check(host, port, min(timeout, 0.5)):
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error="port unreachable")
        ...
        try:
            conn = aioredis.Redis(host=host, port=port, password=password, socket_timeout=timeout, socket_connect_timeout=timeout)
    ...
    async def probe_neo4j(self, url: str, auth: tuple[str, str], timeout: float = 1.5) -> ServiceProbeResult:
        ...
        try:
            driver = AsyncGraphDatabase.driver(url, auth=auth, connection_timeout=timeout)
            await driver.verify_connectivity()
            latency = round((time.monotonic() - start) * 1000, 2)
            await driver.close()
            return ServiceProbeResult(reachable=True, latency_ms=latency, info={"url": url})
        except asyncio.TimeoutError:
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error="timeout")
        except Exception as e:
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error=str(e))

    async def probe_chromadb(self, url: str, timeout: float = 2.0) -> ServiceProbeResult:
    ...
    async def probe_postgres(self, url: str, timeout: float = 1.5) -> ServiceProbeResult:
        ...
        try:
            ...
            await conn.close()
            latency = round((time.monotonic() - start) * 1000, 2)
            return ServiceProbeResult(reachable=True, latency_ms=latency, version=version, info={"version": version})
        except asyncio.TimeoutError:
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error="timeout")
        except Exception as e:
            return ServiceProbeResult(False, latency_ms=round((time.monotonic() - start) * 1000, 2), error=str(e))

    async def probe_services(self, timeout: float = 1.5) -> Dict[str, ServiceProbeResult]:
        ...
        redis_url = os.getenv("BIZRA_REDIS_URL") or "redis://:bizra_synapse_secure@localhost:6380"
        ...
        postgres_url = os.getenv("DATABASE_URL") or "postgresql://bizra:bizra@localhost:5433/bizra"
        ...
        return {k: (v if isinstance(v, ServiceProbeResult) else ServiceProbeResult(False, error=str(v))) for k, v in zip(tasks.keys(), results)}
    ...
    def configure_model_routing(self, services: Dict[str, ServiceProbeResult]) -> Dict[str, Dict[str, str]]:
        """Generate model routing table based on available models."""
        routing = {}

        ollama_models = services.get("ollama", ServiceProbeResult(False)).info.get("models", []) if services.get("ollama", ServiceProbeResult(False)).reachable else []
        lmstudio_models = services.get("lmstudio", ServiceProbeResult(False)).info.get("models", []) if services.get("lmstudio", ServiceProbeResult(False)).reachable else []
        ...
        for slot_name, slot_config in canonical_routing.items():
            candidates = [slot_config.get("primary"), slot_config.get("fallback")] + slot_config.get("allowed", [])
    ...
    def auto_heal(self, services: Dict[str, ServiceProbeResult], routing: Dict[str, Dict[str, str]]) -> str:
        """Determine operational mode based on service availability."""
        llm_available = any(services.get(k, ServiceProbeResult(False)).reachable for k in ["ollama", "lmstudio"])
    ...
    def detect_capabilities(self, services: Dict[str, ServiceProbeResult], routing: Dict[str, Dict[str, str]]) -> List[str]:
