...
impl AgentEntry {
    ...
    pub fn record_completion(&mut self, duration_us: u64, confidence: Confidence, timestamp: u64) {
        ...
        self.avg_confidence = self.avg_confidence
            + (confidence.as_f32() - self.avg_confidence) / total as f32;
    }
...
impl AgentRoster {
    ...
    pub fn new(user_hash: u32, timestamp: u64) -> Self {
        ...
        let agents = core::array::from_fn(|i| {
            AgentEntry::new(roles[i], user_hash, timestamp)
        });
    ...
    pub fn degraded_count(&self) -> usize {
        self.agents.iter().filter(|a| a.state == AgentState::Degraded).count()
    }
    ...
    pub fn team_health(&self) -> f32 {
        let weighted_sum: f32 = self.agents.iter()
            .map(|a| a.success_rate() * a.role.consensus_weight())
            .sum();
        let weight_sum: f32 = self.agents.iter()
            .map(|a| a.role.consensus_weight())
            .sum();
...
mod tests {
    ...
    fn complete_task_marks_idle() {
        ...
        roster.complete_task(AgentRole::Scholar, 500, Confidence::HIGH, 1500);
    ...
    fn ihsan_degradation() {
        let mut roster = AgentRoster::new(0xBEEF, 1000);
        roster.update_ihsan_all(IhsanScore::new(9000));
        ...
        // Recovery
        roster.update_ihsan_all(IhsanScore::new(9900));
    ...
    fn roster_snapshot() {
        ...
        roster.complete_task(AgentRole::Navigator, 100, Confidence::HIGH, 1100);
    ...
    fn agent_success_rate() {
        ...
        entry.record_completion(100, Confidence::HIGH, 1100);
        entry.record_completion(200, Confidence::MEDIUM, 1200);
