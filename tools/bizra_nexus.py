...
class NexusConfig:
    ...
    data_lake_path: Path = field(
        default_factory=lambda: Path(
            os.environ.get(
                "BIZRA_DATA_LAKE_ROOT",
                (
                    "/mnt/c/BIZRA-DATA-LAKE"
                    if os.path.exists("/mnt/c")
                    else "C:/BIZRA-DATA-LAKE"
                ),
            )
        )
    )
...
class EngineAdapter(ABC):
    ...
    def query(self, query: str, use_cache: bool = True, **kwargs) -> QueryResult:
        ...
        # Check cache
        cache_key = (
            f"{self.name}:{hashlib.blake2b(query.encode(), digest_size=16).hexdigest()}"
        )
        if use_cache:
    ...
    def _do_initialize(self):
        ...
        if index_dir.exists():
            ...
            if summary_files:
                try:
                    with open(
                        summary_files[0], "r", encoding="utf-8", errors="ignore"
                    ) as f:
                        self._index = json.load(f)
                    log.info(
                        f"Loaded file index: {self._index.get('total_files', 0)} files"
                    )
                except Exception as e:
    ...
    def _do_query(self, query: str, **kwargs) -> QueryResult:
        ...
        for ext, count in self._index.get("extensions", {}).items():
            if any(word in ext.lower() for word in query_words):
                results.append({"type": "extension", "extension": ext, "count": count})
        ...
        for gem in self._index.get("golden_gems", []):
            ...
            if any(word in gem_str for word in query_words):
                results.append({"type": "golden_gem", "gem": gem})
    ...
    def _do_query(self, query: str, **kwargs) -> QueryResult:
        ...
        for key, value in self._state.items():
            if query_lower in key.lower():
                ...
                results.append(
                    {"key": key, "value": value[:200], "match_type": "value"}
                )
        ...
        if graph_dir.exists():
            ...
            for graph_file in graph_dir.glob("*.json"):
                try:
                    ...
                    pass
            log.info(
                f"Loaded graph: {len(self._nodes)} nodes, {len(self._edges)} edges"
            )
...
class ChatHistoryAdapter(EngineAdapter):
    ...
    def _do_initialize(self):
        ...
        if chat_dir.exists():
            for chat_file in list(chat_dir.glob("*.json"))[
                :100
                    ...
                    with open(chat_file, "r", encoding="utf-8", errors="ignore") as f:
                        ...
                        if isinstance(data, dict):
                            self._conversations.append(
                                {"file": chat_file.name, "data": data}
                            )
                        elif isinstance(data, list):
                            self._conversations.append(
                                {"file": chat_file.name, "messages": data}
                            )
                except Exception:
                    pass
    ...
    def _do_query(self, query: str, **kwargs) -> QueryResult:
        ...
        for conv in self._conversations:
            ...
            if query_lower in file_name.lower():
                results.append({"file": file_name, "match_type": "filename"})
    ...
    def _do_health_check(self) -> HealthCheck:
        return HealthCheck(
            engine=self.name,
            status=(
                HealthStatus.HEALTHY if self._conversations else HealthStatus.DEGRADED
            ),
            latency_ms=0,
...
class EmbeddingsAdapter(EngineAdapter):
    ...
    def _do_initialize(self):
        ...
        if embed_dir.exists():
            ...
            embedding_files = list(embed_dir.glob("*.npy")) + list(
                embed_dir.glob("*.pkl")
            )
            self._embeddings_loaded = len(embedding_files) > 0
            log.info(
                f"Found {len(embedding_files)} embedding files, {len(self._metadata)} metadata entries"
            )
    ...
    def _do_health_check(self) -> HealthCheck:
        status = (
            HealthStatus.HEALTHY if self._embeddings_loaded else HealthStatus.DEGRADED
        )
        return HealthCheck(
            engine=self.name,
            status=status,
...
class QueryRouter:
    ...
    def route(
        self, query: str, engines: Dict[str, EngineAdapter]
...
class BizraNexus:
    ...
    def query(
        ...
        **kwargs,
        ...
        self._event_bus.publish(
            Event(type=EventType.QUERY_START, source="nexus", payload={"query": query})
        )
        ...
        if parallel and len(target_engines) > 1:
            # Parallel execution
            futures = {
                self._executor.submit(self._engines[name].query, query, **kwargs): name
...
Examples:
  ...
  python bizra_nexus.py stats
    ...
    parser.add_argument(
        "command",
        choices=["query", "health", "stats", "init"],
    ...
    parser.add_argument(
        "--engine", "-e", type=str, action="append", help="Specific engine(s) to use"
    )
    parser.add_argument(
        "--no-aggregate", action="store_true", help="Don't aggregate results"
    )
    ...
    with BizraNexus() as nexus:
        if args.command == "init":
            ...
            result = nexus.query(
                args.query_text, engines=args.engine, aggregate=not args.no_aggregate
            )
            ...
            if args.json:
                if isinstance(result, QueryResult):
                    ...
                    print(
                        json.dumps(
                            {k: v.to_dict() for k, v in result.items()}, indent=2
                        )
                    )
            else:
                if isinstance(result, QueryResult):
            ...
            for name, check in health.items():
                ...
                print(
                    f"  {status_icon} {name}: {check.status.value} ({check.latency_ms:.1f}ms)"
                )
                if check.message:
