...
class PatternExtractor:
    ...
    def extract_success_patterns(
        ...
        patterns = [
            ...
            if p.pattern_type == PatternType.SUCCESS
        ]
    ...
    def _process_record(self, record: ExecutionRecord) -> None:
        ...
        # Agent pattern
        agent_hash = self.compute_pattern_hash(
        ...
        # Category pattern
        category_hash = self.compute_pattern_hash(
    ...
    def _update_or_create_pattern(
        ...
        if pattern_id not in self._patterns:
            ...
            initial_type = PatternType.SUCCESS if record.success else PatternType.FAILURE
    ...
    def _extract_sequence_patterns(
        ...
        for window_size in range(2, min(self.max_sequence_length + 1, len(history) + 1)):
            for i in range(len(history) - window_size + 1):
                ...
                if pattern_hash in self._patterns:
                    ...
                    success = all(r.success for r in window)
                    initial_type = PatternType.SUCCESS if success else PatternType.FAILURE
    ...
    def compute_pattern_hash(self, signature: Sequence[str]) -> str:
        ...
        if len(signature) > 0 and not any(":" in s and s.count(":") > 1 for s in signature):
...
def main():
    ...
    parser.add_argument("--elevation", action="store_true", help="Show elevation candidates")
