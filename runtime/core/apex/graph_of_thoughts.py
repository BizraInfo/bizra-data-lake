...
class GoTNode:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.snr_score <= 1.0:
            raise ValueError(
                f"SNR score must be between 0.0 and 1.0, got {self.snr_score}"
            )
        if self.depth < 0:
...
class GoTEdge:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.weight <= 1.0:
            raise ValueError(
                f"Edge weight must be between 0.0 and 1.0, got {self.weight}"
            )
...
class GraphOfThoughtsEngine:
    ...
    def __init__(
        ...
        if not 0.0 <= snr_threshold <= 1.0:
            raise ValueError(
                f"SNR threshold must be between 0.0 and 1.0, got {snr_threshold}"
            )
        if max_depth < 1:
            raise ValueError(f"Max depth must be at least 1, got {max_depth}")
        if diversity_bonus < 0:
            raise ValueError(
                f"Diversity bonus must be non-negative, got {diversity_bonus}"
            )
    ...
    def build_reasoning_graph(
        ...
        logger.info(
            f"Building reasoning graph: graph_id={graph_id}, domains={len(task_domains)}"
        )
    ...
    def _add_cross_domain_edges(self, task_domains: List[TaskDomain]) -> None:
        ...
        cross_domain_count = sum(
            1 for e in self._edges.values() if e.edge_type == GoTEdgeType.CROSS_DOMAIN
        )
    ...
    def _add_consensus_edges(
        ...
        consensus_count = sum(
            1 for e in self._edges.values() if e.edge_type == GoTEdgeType.CONSENSUS
        )
    ...
    def _add_synthesis_edges(
        ...
        synthesis_count = sum(
            1 for e in self._edges.values() if e.edge_type == GoTEdgeType.SYNTHESIS
        )
    ...
    def _compute_synthesis_snr(
        ...
        type_scores: Dict[str, List[float]] = {t.value: [] for t in GoTNodeType}
        ...
        domains = set(node.domain for node in soup_nodes if node.domain is not None)
    ...
    def _create_veto_gates(
        ...
        security_gate = GoTNode(
            ...
            veto_reason=(
                "Security constraint triggered"
                if self._check_security_veto(task)
                else None
            ),
        )
        ...
        ethics_gate = GoTNode(
            ...
            veto_reason=(
                "Ethics constraint triggered" if self._check_ethics_veto(task) else None
            ),
        )
    ...
    def _create_edge(
        ...
        if (
            target_id in self._nodes
            and source_id not in self._nodes[target_id].parent_ids
        ):
            self._nodes[target_id].parent_ids.append(source_id)
        if (
            source_id in self._nodes
            and target_id not in self._nodes[source_id].children_ids
        ):
