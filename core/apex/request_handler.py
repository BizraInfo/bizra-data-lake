...
from core.pci.gates import GateChain, GateResult, verify_envelope
...
class RequestHandler:
    ...
    MODE_ESCALATION_KEYWORDS = {
        OrchestrationMode.ELEVATED: [
            "sensitive", "private", "confidential", "secure",
            "financial", "medical", "legal", "compliance",
        ],
        OrchestrationMode.SOVEREIGN: [
            "offline", "sovereign", "air-gapped", "isolated",
            "critical", "emergency", "failsafe",
        ],
        OrchestrationMode.TRANSCENDENT: [
            "formal", "proof", "verified", "certified",
            "audit", "regulatory", "immutable",
        ],
    }
    ...
    async def handle(
        ...
        ihsan_score = request.scores.get("ihsan", self.ihsan_threshold) if request.scores else self.ihsan_threshold
        snr_score = request.scores.get("snr", self.snr_threshold) if request.scores else self.snr_threshold
        ...
        if self.validate_gates:
            passed, rejection, gate_results = verify_envelope(
                ...
                ihsan_threshold=self.ihsan_threshold,
                snr_threshold=self.snr_threshold,
                require_expensive=(final_mode in [OrchestrationMode.SOVEREIGN, OrchestrationMode.TRANSCENDENT]),
            )
    ...
    def _build_envelope(
        ...
        envelope = (builder
            .with_sender(AgentType.PAT, request.agent_id, self.keypair.public_key_hex)
    ...
    def determine_mode(self, request: OrchestrationRequest) -> OrchestrationMode:
        ...
        for keyword in self.MODE_ESCALATION_KEYWORDS[OrchestrationMode.TRANSCENDENT]:
            if keyword in combined_text:
                if self._mode_value(OrchestrationMode.TRANSCENDENT) > self._mode_value(current_mode):
        ...
        for keyword in self.MODE_ESCALATION_KEYWORDS[OrchestrationMode.SOVEREIGN]:
            if keyword in combined_text:
                if self._mode_value(OrchestrationMode.SOVEREIGN) > self._mode_value(current_mode):
        ...
        for keyword in self.MODE_ESCALATION_KEYWORDS[OrchestrationMode.ELEVATED]:
            if keyword in combined_text:
                if self._mode_value(OrchestrationMode.ELEVATED) > self._mode_value(current_mode):
