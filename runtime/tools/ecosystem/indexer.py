...
def _project_identity(
    path: Path, git: Optional[GitInfo], *, skip_names: Sequence[str]
) -> str:
    if git is not None:
        payload = {
            "path": str(path.resolve()),
...
def build_manifest(
    *,
    ...
    roots_out: List[Root] = [
        Root(key=k, path=str(v))
        for k, v in sorted(resolved_roots.items(), key=lambda kv: kv[0])
    ]

    projects: List[Project] = []

    for root_key, root_path in sorted(resolved_roots.items(), key=lambda kv: kv[0]):
        for child in _safe_list_children(
            root_path, skip_names=cfg.scan.skip_directories
        ):
            ...
            identity = _project_identity(
                child, git, skip_names=cfg.scan.skip_directories
            )
            pid = sha256_canonical_json(
                {"root": root_key, "path": str(child.resolve())}
            )[:16]
    ...
    config_sha = sha256_file(
        repo_root / "tools" / "ecosystem" / "ecosystem_config.yaml"
    )
