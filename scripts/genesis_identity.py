    ...
    def _run_wmic(query: str) -> str:
        ...
        try:
            result = subprocess.run(
                ["wmic"] + query.split(), capture_output=True, text=True, timeout=10
            )
...
def verify_genesis_hardware() -> GenesisVerificationResult:
    ...
    if stored_t2.get("ram_signature") != current_t2["ram_signature"]:
        result.tier_2_warnings.append(
            "RAM signature changed — hardware upgrade detected"
        )
...
def delegate_authority(
    target_node_id: str, permissions: list, attestation: str
) -> Dict:
    """
    ...
    node0_hash = (
        genesis.get("hardware_covenant", {})
        .get("tier_1_root", {})
        .get("combined_root_hash")
    )
...
def initialize_genesis(author: str = "MoMo", force: bool = False) -> Dict:
    ...
    print(
        f"   Root Hash: {hardware_covenant['tier_1_root']['combined_root_hash'][:16]}..."
    )
