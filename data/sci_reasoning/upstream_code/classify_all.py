...
def classify_batch(
    ...
    taxonomy_ref = "\n".join(
        [f"- {p['id']}: {p['name']} - {p['description'][:100]}..." for p in taxonomy]
    )
    ...
    system_prompt = (
        "You are an expert at classifying research thinking patterns. Be concise."
    )
...
def main():
    ...
    total_batches = (
        len(papers) - start_idx + CLASSIFICATION_BATCH_SIZE - 1
    ...
    for i in range(start_idx, len(papers), CLASSIFICATION_BATCH_SIZE):
        ...
        if batch_num % 25 == 0 or i + CLASSIFICATION_BATCH_SIZE >= len(papers):
            ...
            print(
                f"  Progress: {len(classified_papers)}/{len(papers)} ({pct:.1f}%) | Cost: ${cost_tracker.get_total_cost():.4f}"
            )
