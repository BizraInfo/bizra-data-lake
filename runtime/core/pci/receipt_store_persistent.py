...
class ReceiptChain:
    ...
    def get_range(self, start_offset: int, end_offset: int) -> List[StoredReceipt]:
        ...
        return [r for r in self._receipts if start_offset <= r.offset <= end_offset]
...
class PostgreSQLReceiptStore:
            ...
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    ...
                    (
                        ...
                        kg_decision,
                        json.dumps(
                            [{"type": "envelope", "digest": stored.envelope_digest}]
                        ),
                        json.dumps(
                            {
                        ...
                        json.dumps(
                            {
                                ...
                                "tier": "PCI",
                                "gates_passed": stored.gate_results.get(
                                    "gates_passed", []
                                ),
                            }
                        ),
        ...
        async with await self._get_connection() as conn:
            async with conn.cursor() as cur:
                ...
                return StoredReceipt(
                    ...
                    decision=(
                        "COMMIT" if payload.get("decision") != "REJECT" else "REJECT"
                    ),
    ...
    async def verify_chain_integrity(self, start_offset: int = 0) -> Tuple[bool, int]:
        """
    ...
    async def stream_all(self, start_offset: int = 0) -> AsyncIterator[StoredReceipt]:
        """Stream all receipts for audit."""
        async with await self._get_connection() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
        ...
        async for receipt in self.pg.stream_all(start_offset=jsonl_latest + 1):
            try:
                ...
                async with aiofiles.open(
                    self.jsonl.filepath, "a", encoding="utf-8"
                ) as f:
                    await f.write(json_line + "\n")
