...
class Guardian:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.embedding:
            embedding_input = f"{CONSTELLATION_DOMAIN}:guardian:{self.role.value}:{self.name_ar}"
    ...
    def _get_expertise_domains(self) -> List[str]:
        ...
        domain_map = {
            GuardianRole.AL_MUHASIB: ["finance", "accounting", "resource-management", "tokenomics"],
            GuardianRole.AL_MUJTAHID: ["legal", "compliance", "constitutional", "governance"],
            GuardianRole.AL_MURABBI: ["education", "knowledge-transfer", "documentation", "onboarding"],
            GuardianRole.AR_RUH: ["ethics", "ihsan", "spiritual", "excellence"],
            GuardianRole.AL_AMIN: ["security", "cryptography", "key-management", "integrity"],
            GuardianRole.AL_MUSTASHAR: ["strategy", "advisory", "risk-assessment", "decision-support"],
            GuardianRole.AL_RAQIB: ["monitoring", "observability", "anomaly-detection", "audit"],
            GuardianRole.MAJLIS_AL_KAWNI: ["consensus", "swarm-intelligence", "collective-decision", "arbitration"],
        }
    ...
    def _get_capabilities(self) -> List[str]:
        ...
        capability_map = {
            GuardianRole.AL_MUHASIB: ["resource-audit", "token-validation", "economic-analysis", "budget-enforcement"],
            GuardianRole.AL_MUJTAHID: ["legal-reasoning", "compliance-check", "constitutional-interpretation", "policy-enforcement"],
            GuardianRole.AL_MURABBI: ["knowledge-synthesis", "documentation-generation", "onboarding-flow", "learning-path-design"],
            GuardianRole.AR_RUH: ["ihsan-scoring", "ethical-review", "excellence-enforcement", "spiritual-alignment"],
            GuardianRole.AL_AMIN: ["key-management", "signature-verification", "encryption", "integrity-check"],
            GuardianRole.AL_MUSTASHAR: ["risk-modeling", "strategic-analysis", "scenario-planning", "recommendation-engine"],
            GuardianRole.AL_RAQIB: ["real-time-monitoring", "anomaly-detection", "alert-generation", "audit-trail-maintenance"],
            GuardianRole.MAJLIS_AL_KAWNI: ["consensus-building", "vote-aggregation", "conflict-resolution", "final-arbitration"],
        }
...
class GuardianConstellation:
    ...
    def _initialize_guardians(self) -> None:
        ...
        self._guardians[GuardianRole.MAJLIS_AL_KAWNI] = Guardian(
            ...
            snr_threshold=0.99,
            veto_domains={VetoDomain.ETHICS, VetoDomain.SECURITY, VetoDomain.COMPLIANCE, VetoDomain.SAFETY},
    ...
    def get_the_seven(self) -> List[Guardian]:
        ...
        return [
            g for r, g in self._guardians.items()
            if r != GuardianRole.MAJLIS_AL_KAWNI
        ]
    ...
    def get_guardians_with_veto(self, veto_domain: Optional[VetoDomain] = None) -> List[Guardian]:
        """
        ...
        return [
            g for g in self._guardians.values()
            if veto_domain in g.veto_domains
        ]
    ...
    def get_absolute_veto_guardians(self) -> List[Guardian]:
        ...
        return [
            g for g in self._guardians.values()
            if g.veto_power == VetoPower.ABSOLUTE
        ]
    ...
    def request_veto_check(
        ...
        if guardian.role == GuardianRole.AR_RUH:
            if ihsan_score < IHSAN_THRESHOLD:
                ...
                reasoning = f"Ihsan score {ihsan_score} below threshold {IHSAN_THRESHOLD}"
        ...
        if snr_score < guardian.snr_threshold:
            if guardian.veto_power == VetoPower.ABSOLUTE:
                ...
                reasoning = f"SNR score {snr_score} below threshold, escalating to Majlis"
        ...
        if guardian.role == GuardianRole.AL_AMIN:
            ...
            if any(kw in payload_str for kw in security_keywords):
                ...
                if "expose" in payload_str or "leak" in payload_str:
                    result = VetoResult.VETOED
                    reasoning = "Security-sensitive operation with potential exposure risk"
    ...
    def convene_majlis(
        ...
        absolute_vetoes = [
            role for role, vote in votes.items()
            if vote == VetoResult.VETOED and
            self._guardians[role].veto_power == VetoPower.ABSOLUTE
        ]

        if absolute_vetoes:
            decision = MajlisDecision.DEADLOCK
            consensus_reasoning = f"ABSOLUTE veto by: {[r.value for r in absolute_vetoes]}"
            ...
            consensus_reasoning = f"Split decision ({approved_count}/7 approved, {vetoed_count}/7 vetoed)"
        else:
            decision = MajlisDecision.DEADLOCK
    ...
    def _emit_veto_receipt(self, request: VetoCheckRequest, response: VetoCheckResponse) -> ConstellationReceipt:
        ...
        receipt = ConstellationReceipt(
            ...
            integrity_hash=self._compute_receipt_hash(request.to_dict(), response.to_dict()),
        )
        self._receipts.append(receipt)
        return receipt

    def _emit_majlis_receipt(self, query: MajlisQuery, response: MajlisResponse) -> ConstellationReceipt:
        """Emit evidence receipt for Majlis decision."""
        receipt = ConstellationReceipt(
            receipt_id=str(uuid4()),
            operation="majlis_convene",
            guardian_roles_involved=list(response.votes.keys()) + [GuardianRole.MAJLIS_AL_KAWNI],
            decision=response.decision.value,
            ihsan_score=response.collective_ihsan_score,
            snr_score=response.collective_snr_score,
            timestamp=response.timestamp,
            integrity_hash=self._compute_receipt_hash(query.to_dict(), response.to_dict()),
        )
        self._receipts.append(receipt)
        return receipt
...
def get_guardian_by_arabic_name(constellation: GuardianConstellation, arabic_name: str) -> Optional[Guardian]:
    """
