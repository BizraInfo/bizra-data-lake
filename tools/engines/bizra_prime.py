...
class BizraPrime:
    ...
    def __init__(self):
        ...
        print("═" * 70)
        print(
            f"   ✅ BIZRA PRIME ONLINE | Agents: {len(self.agents)} | Knowledge: {len(self.sovereign_df):,} nodes"
        )
        print(
            f"   🔌 LLM Gateway: {llm_status} | Model: {self.llm_gateway.active_model or 'None'}"
        )
    ...
    def _load_quran_knowledge(self):
        if QURAN_DATA_PATH.exists() and QURAN_VECTORS_PATH.exists():
            ...
            print(
                f"   📖 Quran Loaded: {len(df)} verses, {vectors.shape[1]}-dim vectors."
            )
            return df, vectors
    ...
    def _record_poi(self, action: str, benchmarks: Dict[str, float]):
        ...
        self.state["total_impact"] = self.state.get("total_impact", 0) + sum(
            benchmarks.values()
        )
        self._save_state()
        ...
        if self.sovereign_bridge and SOVEREIGN_BRIDGE_AVAILABLE:
            try:
                ...
                if loop.is_running():
                    asyncio.create_task(
                        self.sovereign_bridge.record_poi(
                            action, benchmarks, poi.attestation_hash
                        )
                    )
                else:
    ...
    def _research(self, query: str) -> str:
        ...
        if self.sovereign_memory.is_ready():
            ...
            if m6_results:
                best = m6_results[0]
                results.append(
                    f"[M6-Sovereign] {best.name} in {best.domain} (SNR: {best.snr_score:.2f})"
                )
        ...
        if len(self.quran_vectors) > 0:
            ...
            norm_vectors = (
                self.quran_vectors
                / np.linalg.norm(self.quran_vectors, axis=1)[:, np.newaxis]
            )
            ...
            results.append(
                f"[M3-Quran] {verse['surah_name']} {verse['surah']}:{verse['ayah']}: {verse['english_text'][:80]}..."
            )
    ...
    def _evaluate(self, output: str) -> Dict[str, float]:
        ...
        # Use ARTE to get SNR
        snr = self.arte.resolve_tension(
            output, "symbolic_grounding", "neural_intuition"
        )
    ...
    def _self_diagnose(self) -> Dict[str, Any]:
        ...
        diagnosis = {
            ...
            "memory_tiers": {
                ...
                "M4_vector": (
                    f"{len(self.quran_vectors)} embeddings"
                    if len(self.quran_vectors) > 0
                    else "EMPTY"
                ),
                "M5_hypergraph": f"{len(self.sovereign_df):,} nodes",
                "M6_sovereign": (
                    f"{sum(s['total_nodes'] for s in m6_stats.values()):,} cross-domain"
                    if m6_stats
                    else "OFFLINE"
                ),
            },
    ...
    def reason(self, goal: str) -> Dict[str, Any]:
        ...
        thought_graph["final_output"] = {
            ...
            "quality_scores": (
                evaluation.get("output") if isinstance(evaluation, dict) else evaluation
            ),
        }
    ...
    def ask(self, question: str, use_context: bool = True) -> str:
        ...
        if use_context:
            ...
            # M6: Sovereign Memory
            if self.sovereign_memory.is_ready():
                m6_results = self.sovereign_memory.search_across_domains(
                    question, top_k=3
                )
                ...
                for idx in top_indices:
                    if scores[idx] > 0.3:  # Only include if relevant
                        ...
                        context_chunks.append(
                            f"[Quran {verse['surah']}:{verse['ayah']}] {verse['english_text']}"
                        )
...
Follow the Ihsan constraint: always act with excellence and avoid harm."""
            ...
            response = self.llm_gateway.generate_with_context(
                query=question, context=context_text, system=system_prompt
            )
