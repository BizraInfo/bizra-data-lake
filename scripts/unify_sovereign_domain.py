...
def calculate_snr_score(file_path):
    ...
    if ext in [".md", ".txt", ".org"]:
        return 1.0
    if ext in [".py", ".rs", ".js", ".ts", ".go", ".cpp", ".h", ".java", ".cs"]:
        return 0.95
    if ext in [".json", ".yaml", ".yml", ".xml", ".toml"]:
        return 0.8
    if ext in [".pdf", ".docx", ".pptx"]:
        return 0.6  # High value but hard to parse quickly
...
def unify_domain():
    print("🌟 INITIATING SOVEREIGN UNIFICATION PROTOCOL")
                ...
                records.append(
                    {
                        ...
                        "domain_source": (
                            path_root.parts[4]
                            if len(path_root.parts) > 4
                            else "Unknown"
                        ),  # Extract which junction
                    }
                )
