...
class AblationStudy:
    ...
    ablation_types: list[AblationType] = field(
        default_factory=lambda: [AblationType.REMOVE]
    )
...
class AblationEngine:
    ...
    def create_study(
        ...
        logger.info(
            f"Created ablation study '{name}' with {len(study.components)} components"
        )
        return study
    ...
    def generate_abgen_plan(
        ...
        study = AblationStudy(
            name=f"abgen-{hex_digest(system_description.encode())[:8]}",
            ...
            ablation_types=[
                AblationType.REMOVE,
                AblationType.DISABLE,
                AblationType.REPLACE,
            ],
        )
    ...
    def generate_report(self, study_id: str) -> str:
        ...
        for name, contribution, verdict in study.get_ranking():
            indicator = (
                "🟢"
                if verdict in ["ESSENTIAL", "BENEFICIAL"]
                else "🟡" if verdict in ["MARGINAL", "NEUTRAL"] else "🔴"
            )
...
if __name__ == "__main__":
    ...
    for comp in study.components:
        ...
        print(
            f"  {comp.name}: {result.contribution:+.4f} ({result.contribution_pct:+.1f}%)"
        )
