...
class GenesisStateMachine:
    ...
    def get_phase_requirements(self, phase: Optional[PhaseState] = None) -> List[PhaseRequirement]:
        """
    ...
    async def transition(
        ...
        if not can_proceed:
            transition = PhaseTransition(
                ...
                timestamp=timestamp,
                result=TransitionResult.REJECTED_BACKWARD_TRANSITION
        ...
        if target_phase == PhaseState.ACTIVATION:
            if ihsan_score < IHSAN_THRESHOLD:
                transition = PhaseTransition(
                    ...
                    requirements_failed=[f"ihsan_score={ihsan_score} < {IHSAN_THRESHOLD}"],
                )
                self._emit_receipt(transition)
                return transition
        ...
        for validator in custom_validators:
            try:
                ...
                transition = PhaseTransition(
                    ...
                    timestamp=timestamp,
                    requirements_met=requirements_met,
                    requirements_failed=requirements_failed + [f"validator_exception: {e}"],
