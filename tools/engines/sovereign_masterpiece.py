...
class ThoughtNode:
    ...
    def calculate_composite_score(
        self, config: MasterpieceConfig = DEFAULT_CONFIG
    ) -> float:
        ...
        return (
            ...
            * self.confidence
            * (1 if self.novelty_score > 0.5 else 0.5)
        )
    ...
    def can_merge_with(
        ...
        other: "ThoughtNode",
        similarity: float,
        config: MasterpieceConfig = DEFAULT_CONFIG,
    ) -> bool:
        """Check if this thought can merge with another."""
        return (
            similarity >= config.merge_similarity_threshold
            and self.status not in {ThoughtStatus.PRUNED, ThoughtStatus.MERGED}
            and other.status not in {ThoughtStatus.PRUNED, ThoughtStatus.MERGED}
        )
...
class ThoughtGraph:
    ...
    def validate_thought(
        self, thought_id: str, new_snr: float, grounding: float = 0.0
    ) -> bool:
    ...
    def synthesize_thoughts(
        self, thought_id_1: str, thought_id_2: str, similarity: float
...
class SNREngine:
    ...
    def calculate_snr(
        ...
        symbolic_grounding = min(
            len(symbolic_facts) / (len(result_embeddings) + 1), 1.0
        )

        # ─── Coverage Balance ──────────────────────────────────────────────────
        # Diversity of sources represented
        coverage_balance = min(
            len(source_diversity) / max(len(result_embeddings), 1), 1.0
        )
    ...
    def optimize_toward_ihsan(
        self, current_snr: SNRComponents, max_iterations: int = 5
        ...
        if not suggestions:
            suggestions.append(
                f"✓ Ihsān achieved: Overall SNR = {current_snr.overall:.4f}"
            )
...
class InterdisciplinaryMatrix:
    ...
    def _build_domain_bridges(self) -> Dict[str, Set[str]]:
        ...
        bridges["physics"].update(
            ["complexity_science", "chaos_theory", "information_theory"]
        )
        bridges["cognitive_psychology"].update(
            ["nlp", "neuroscience", "meta_cognition"]
        )
        bridges["systems_theory"].update(
            ["complexity_science", "cybernetics", "emergence"]
        )
        bridges["synthesis"].update(
            [d.code for d in CognitiveDomain]
        )  # Meta-domain connects all
    ...
    def synthesize(
        self, query: str, perspectives: List[DomainPerspective]
        ...
        if total_weight > 0:
            ...
            diversity = len(set(p.domain.code for p in perspectives)) / len(
                perspectives
            )
            synthesis_snr = sum(
                p.relevance_score * w for p, w in zip(perspectives, normalized_weights)
            ) * (0.7 + 0.3 * diversity)
        else:
            synthesis_snr = 0.0
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "interdisciplinary_synthesis": (
                ...
                if self.interdisciplinary_synthesis
                else None
            ),
...
class SovereignMasterpiece:
    ...
    def __init__(self, config: MasterpieceConfig = DEFAULT_CONFIG):
        ...
        log.info(
            f"  • WARP integration: {'Enabled' if config.warp_enabled else 'Disabled'}"
        )
        log.info("═" * 80)
    ...
    async def reason(self, query: MasterpieceQuery) -> MasterpieceResult:
        ...
        if not conclusions:
            ...
            for thought in self.thought_graph._nodes.values():
                if (
                    thought.snr_score > self.config.snr_threshold
                    and thought.status == ThoughtStatus.VALIDATED
                    and not thought.child_ids
                ):
        ...
        result = MasterpieceResult(
            ...
            thoughts=[
                t
                for t in self.thought_graph._nodes.values()
        ...
        log.info(
            f"  ✓ Complete in {execution_time:.1f}ms | SNR: {snr_components.overall:.4f}"
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ihsan_rate": (self.ihsan_achieved_count / max(self.total_queries, 1)),
...
""")
    ...
    for i, query_text in enumerate(queries, 1):
        ...
        print(
            f"  🌐 Domains Activated: {result.interdisciplinary_synthesis.domains_activated if result.interdisciplinary_synthesis else 0}"
        )
