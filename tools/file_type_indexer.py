...
FILE_CATEGORIES = {
    ...
    "notebooks": {"extensions": [".ipynb"], "icon": "📓", "priority": "critical"},
    "diagrams": {
        "extensions": [
            ".drawio",
            ".vsdx",
    ...
    "logs": {"extensions": [".log", ".logs"], "icon": "📋", "priority": "low"},
}
...
class FileTypeIndexer:
    ...
    def __init__(self, output_dir: Path = None):
        self.output_dir = output_dir or Path(
            r"C:\BIZRA-DATA-LAKE\03_INDEXED\file_index"
        )
        ...
        for cat, config in FILE_CATEGORIES.items():
            self.categories[cat] = CategoryStats(category=cat, icon=config["icon"])
    ...
    def _update_stats(self, file_info: FileInfo):
        ...
        cat.extensions[file_info.extension] = (
            cat.extensions.get(file_info.extension, 0) + 1
        )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for cat_name, cat in sorted(
            self.categories.items(), key=lambda x: x[1].count, reverse=True
        ):
            if cat.count > 0:
                summary["categories"][cat_name] = {
                    ...
                    "top_extensions": dict(
                        sorted(
                            cat.extensions.items(), key=lambda x: x[1], reverse=True
                        )[:10]
                    ),
                }
    ...
    def print_summary(self):
        ...
        for cat_name in ["code", "data", "archives", "documents"]:
            if cat_name in summary["categories"]:
                ...
                if exts:
                    ext_str = ", ".join(
                        [f"{e}({c})" for e, c in list(exts.items())[:5]]
                    )
...
def main():
    ...
    parser.add_argument(
        "--downloads", action="store_true", help="Scan Downloads folder"
    )
    parser.add_argument("--all", action="store_true", help="Scan all common locations")
    parser.add_argument(
        "--max-files", type=int, default=None, help="Max files to index"
    )
    ...
    if args.downloads:
        ...
        nested_downloads = Path(
            r"C:\Users\BIZRA-OS\Downloads\OneDrive\Desktop\OneDrive\Downloads"
        )
