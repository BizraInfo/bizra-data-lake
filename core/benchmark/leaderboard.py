...
class SubmissionResult:
    ...
    def kami_score(self) -> float:
        ...
        latency_efficiency = max(0, 1 - (self.latency_total_ms / 60000)) * 0.15  # 60s budget
...
class AntiGamingValidator:
    ...
    def validate_submission(
        ...
        for query_id, response in responses:
            ...
            if is_null:
                issues.append(f"Query {query_id}: Null model response detected ({null_conf:.0%})")
            ...
            if not is_consistent:
                issues.append(f"Query {query_id}: Inconsistent response ({cons_conf:.0%})")
...
class LeaderboardManager:
    ...
    def record_result(
        ...
        submission.log(f"Result recorded: {raw_score:.4f} (normalized: {normalized:.4f})")
    ...
    def get_leaderboard(
        ...
        if sort_by == "kami_score":
            ...
            leaderboard = sorted(leaderboard, key=lambda r: r.normalized_score / max(0.001, r.cost_usd), reverse=True)
...
if __name__ == "__main__":
    ...
    for benchmark in benchmarks_to_test:
        ...
        for entry in leaderboard:
            ...
            print(f"  {sota_indicator} #{entry['rank']} {entry['agent_id']}: "
