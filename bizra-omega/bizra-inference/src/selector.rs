...
impl TaskComplexity {
    pub fn estimate(prompt: &str, max_tokens: usize) -> Self {
        ...
        if max_tokens > 2000 || (has_reasoning && words > 200) { Self::Expert }
        else if has_code || has_reasoning || words > 100 { Self::Complex }
        else if words > 30 || max_tokens > 500 { Self::Medium }
        else { Self::Simple }
    }
}
...
impl ModelSelector {
    pub fn new() -> Self { Self { forced_tier: None } }

    pub fn force_tier(&mut self, tier: ModelTier) { self.forced_tier = Some(tier); }

    pub fn select_tier(&self, complexity: &TaskComplexity) -> ModelTier {
        if let Some(t) = self.forced_tier { return t; }
        match complexity {
