...
def transcribe(audio_path: Optional[str] = None, timeout: int = 10) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            logger.error("No STT engine available. Install: pip install openai-whisper OR pip install SpeechRecognition")
            return None
        except Exception as e:
...
class MoshiVoice:
    ...
    def __init__(self, device: str = "auto", model_id: str = "kyutai/moshiko-pytorch-bf16"):
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if self.device == "cuda":
                ...
                if free_mem < 15:
                    logger.warning("Low VRAM - model may not fit. Consider fp16 or smaller model.")
            
            # Load model with bfloat16 for memory efficiency
            # Note: trust_remote_code=True required for Moshi
            logger.info("Loading model (bf16)... This may take 1-2 minutes on first run.")
            ...
            with torch.no_grad():
                ...
                if audio_input:
                    ...
                    try:
                        ...
                        return {"error": "torchaudio not installed. Install with: pip install torchaudio"}
    ...
    def get_status(self) -> dict:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                status["warning"] = "CUDA not available. CPU loading is very slow (2-5 minutes). Install PyTorch with CUDA support for GPU acceleration."
        except Exception as e:
...
class MicrophoneStream:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self._running = True
            logger.info(f"Microphone started: {self.sample_rate}Hz, {self.channels}ch, chunk={self.chunk_size}")
            return True
