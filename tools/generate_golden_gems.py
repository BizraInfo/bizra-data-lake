...
def generate_gems():
    ...
    degree_merged = pd.merge(
        node_degree, node_in_degree, on="node_id", how="outer"
    ).fillna(0)
    degree_merged["total_degree"] = (
        degree_merged["out_degree"] + degree_merged["in_degree"]
    )
    ...
    if "relation" in edges_df.columns:
        ...
        for edge_type, count in edge_types.items():
            ...
            sample_pairs = [
                (r["source"][:12], r["target"][:12]) for _, r in sample_edges.iterrows()
            ]
    ...
    if "kind" in nodes_df.columns:
        ...
        cross_domain = edge_with_types[
            edge_with_types["source_kind"] != edge_with_types["target_kind"]
        ]

        if len(cross_domain) > 0:
            # Count cross-domain bridges per source node
            bridge_counts = (
                cross_domain.groupby("source").size().reset_index(name="bridge_count")
            )
    ...
    for g in sorted_gems:
        print(
            f"  - [{g['type']}] {g['title'][:40]}... (score={g['score_connectivity']:.1f})"
        )
