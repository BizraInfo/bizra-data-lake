...
assert len(BIP39_WORDLIST) == 256, f"Wordlist must have 256 entries, got {len(BIP39_WORDLIST)}"
...
def decrypt_private_key(encrypted: Dict[str, str], passphrase: str) -> str:
    ...
    if not hmac.compare_digest(actual_checksum, expected_checksum):
        raise ValueError(f"Decryption checksum mismatch: expected {expected_checksum}, got {actual_checksum}")
...
def _hkdf_expand(key: bytes, length: int) -> bytes:
    ...
    while len(b"".join(blocks)) < length:
        prev = hmac.new(key, prev + block_num.to_bytes(1, "big"), hashlib.sha256).digest()
        blocks.append(prev)
        block_num += 1
...
def save_sovereign_state(state: SovereignState, state_dir: Path) -> None:
    ...
    logger.info("Sovereign state persisted to %s (hash=%s)", state_dir, manifest["state_hash"])
