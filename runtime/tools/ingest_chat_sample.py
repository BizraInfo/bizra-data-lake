...
def is_batch_conversations_export(obj: Any) -> bool:
    ...
    return (
        ...
        and "uuid" in head
        and ("chat_messages" in head or "created_at" in head)
    )


def iter_conversations_from_batch(
        ...
        if isinstance(chat_messages, list):
            for m in chat_messages:
                if not isinstance(m, dict):
                    continue
                role = safe_text(
                    m.get("sender") or m.get("role") or m.get("author") or "unknown"
                ).strip()
...
CMD_RE = re.compile(
    r"\b(docker|git|cargo|npm|ollama|powershell|cmd|robocopy|manage-bde|vssadmin)\b",
    re.I,
)
...
def main(argv: Sequence[str]) -> int:
    ...
    p = argparse.ArgumentParser(
        description="Ingest chat export zips into a high-SNR indexed format."
    )
    p.add_argument(
        "--input-dir",
        default=str(repo_root / "chat data sample"),
        help="Directory containing .zip exports",
    )
    p.add_argument(
        "--out-dir",
        default="",
        help="Output directory (defaults to Data Lake 03_INDEXED when available)",
    )
    p.add_argument(
        "--max-text-chars",
        type=int,
        default=DEFAULT_MAX_TEXT_CHARS,
        help="Max chars stored per message in index",
    )
    p.add_argument(
        "--store-full-text",
        action="store_true",
        help="Also store full messages to messages_full.jsonl.gz",
    )
    ...
    out_dir = (
        ...
        if args.out_dir
        else (default_output_base(repo_root) / ts)
    )
    ...
    for zp in zip_paths:
        sources.append(
            SourceFile(
                path=str(zp), size_bytes=zp.stat().st_size, sha256=sha256_file(zp)
            )
        )
        ...
        with ZipFile(zp) as z:
                ...
                if isinstance(convs, list) and is_batch_conversations_export(convs):
                    for conv, msgs in iter_conversations_from_batch(convs):
                        ...
                        for m in msgs:
                            ...
                            truncated, was_truncated = truncate_text(
            ...
            for member in members:
                ...
                if member.lower().endswith(
                    "conversations.json"
                ) or member.lower().endswith("projects.json"):
                    continue
                if member.lower().endswith("memories.json") or member.lower().endswith(
                    "users.json"
                ):
                    continue
    ...
    write_json(
        ...
        {"generated_at": utc_now_iso(), "sources": sources_dicts},
    )
    ...
    full_msg_count = 0
    if args.store_full_text:
        full_msg_count = write_jsonl_gz(
            out_dir / "messages_full.jsonl.gz", messages_full_rows
        )
    ...
    write_json(
        out_dir / "topics.json", {"generated_at": utc_now_iso(), "topics": topics_rows}
    )
    ...
    for conv in conversations_rows:
        ...
        for term in conv_topics.get(conv_id, []):
            ...
            edges.append(
                {"from": conv_id, "to": term_id, "kind": "has_term", "weight": term[1]}
            )
    ...
    write_json(
        out_dir / "graph.json",
        {"generated_at": utc_now_iso(), "nodes": nodes, "edges": edges},
    )
