...
class TaskAnalyzer:
    ...
    def analyze(self, query: str, context: Optional[dict] = None) -> TaskAnalysis:
        ...
        reasoning_mode = self._determine_reasoning_mode(stakes, len(domains), keywords)
    ...
    def _determine_reasoning_mode(
        self, stakes: Stakes, domain_count: int, keywords: list[str]
    ) -> ReasoningMode:
...
class EvidenceEvaluator:
    ...
    def evaluate(
        self, output: AgentOutput, analysis: TaskAnalysis
    ) -> VerificationResult:
        """
...
class PolymathSynthesizer:
    ...
    def _create_summary(self, outputs: list[AgentOutput]) -> list[str]:
        ...
        for output in outputs[:5]:  # Max 5 agents for summary
            ...
            if output.claims:
                summaries.append(
                    f"[{output.agent_name}] {output.claims[0].get('text', '')[:100]}"
                )
            else:
    ...
    def _generate_next_steps(
        self, outputs: list[AgentOutput], verifications: list[VerificationResult]
    ) -> list[str]:
    ...
    def _build_checklist(
        self, analysis: TaskAnalysis, verifications: list[VerificationResult]
    ) -> dict:
...
class ConstellationOrchestrator:
    ...
    def _execute_verification(
        self, outputs: list[AgentOutput], verifiers: list[Agent], analysis: TaskAnalysis
    ...
    def _mock_agent_execution(
        self, agent: Agent, query: str, analysis: TaskAnalysis
    ) -> AgentOutput:
