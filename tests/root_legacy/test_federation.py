...
def test_gossip_membership():
    ...
    engines = [
        GossipEngine(
            node_id=f"node_{i}", address=f"127.0.0.1:{7650 + i}", public_key=f"pk_{i}"
        )
        for i in range(3)
    ]
...
def test_pattern_elevation():
    ...
    for i in range(3):
        ...
        print(
            f"  Use {i+1}: candidates={len(store.pending_candidates)}, patterns={len(store.local_patterns)}"
        )

    # Should be elevated now
    assert (
        len(store.local_patterns) == 1
    ), f"Should have 1 elevated pattern, got {len(store.local_patterns)}"
    ...
    assert (
        pattern.metrics.uses >= 3
    ), f"Should have at least 3 uses, got {pattern.metrics.uses}"
...
def test_pattern_propagation():
    ...
    print(
        f"  Uses: {pattern.metrics.uses}, Success: {pattern.metrics.success_rate:.0%}"
    )
...
def test_network_effect():
    ...
    for node_count in [1, 3, 5, 10, 20]:
        engine = GossipEngine(
            node_id="primary", address="127.0.0.1:9000", public_key="pk_primary"
        )

        # Add peers
        for i in range(node_count - 1):
            engine.add_seed_node(address=f"127.0.0.1:{9001 + i}", node_id=f"peer_{i}")
    ...
    for i in range(1, len(multipliers)):
        assert (
            multipliers[i][1] >= multipliers[i - 1][1]
