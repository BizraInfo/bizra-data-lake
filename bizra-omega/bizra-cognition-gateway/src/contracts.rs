...
pub struct PrincipalIdentityStatusContract {
    pub schema: String,
    #[serde(rename = "runtimeDomain")]
    #[ts(rename = "runtimeDomain")]
...
mod tests {
    ...
    fn ts_bindings_emit_wire_compatible_camel_case_keys() {
        // Binding files are generated by ts-rs exporter tests and committed.
        // Constructing the DTO here also locks the Rust-side field shape.
    ...
    fn principal_status_contracts_instantiate_and_emit_bindings() {
        // The ts-rs exporter tests regenerate the committed bindings; this
        // constructor independently locks the Rust-side field shape.
        ...
        let _ = PrincipalIdentityStatusContract {
            schema: "bizra.node0.principal_identity_status.v0.2".into(),
            runtime_domain: "bizra-cognition-gateway-v1".into(),
    ...
    fn principal_status_binding_uses_camel_case_and_optional_identity() {
        ...
        let identity = principal_binding("VerifiedPrincipalIdentityContract");
        assert!(
            identity.contains("timestampNs: string"),
            "nanosecond timestamps must cross the JSON/TypeScript boundary losslessly: {identity}"
        );
