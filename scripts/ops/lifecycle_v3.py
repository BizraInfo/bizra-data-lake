...
import sys, os, time, hashlib, asyncio
sys.path.insert(0, ".")
...
ck("Genesis created", lambda: f"class={gen.sovereignty_class.name} SEED={gen.genesis_wallet_state.seed_balance}")
...
from core.integration.constants import (
    ...
    KERNEL_INVARIANTS, DOMAIN_IDENTITY_GENESIS, IHSAN_WEIGHTS,
...
cap = []
async def run_bus():
    async def handler(e): cap.append(e)
    bus.subscribe("genesis.*", handler)
    await bus.publish(Event(topic="genesis.boot", payload={"node":"NODE0"}, source="lifecycle"))
    await bus.publish(Event(topic="genesis.identity", payload={"id":iid[:16]}, source="lifecycle", priority=EventPriority.HIGH))
...
for i, act in enumerate(["heartbeat","mission_complete","reflex_compile","token_mint","audit_pass","chain_seal"],2):
    prev = receipts[-1]
    rd = {"action":act,"prev_hash":prev,"ihsan":0.95+0.01*i,"ts":time.time(),"receipt_num":i}
    cd = canonical_bytes(rd); hd = blake3.blake3(cd).hexdigest()
    receipts.append(hd)
...
# Tamper test
tampered = canonical_bytes({"action":"heartbeat","prev_hash":"TAMPERED","ihsan":0.98,"ts":0})
...
try:
    ...
    ck("ReflexCompiler methods", lambda: [m for m in dir(rc) if not m.startswith("_")][:8])
except Exception as e:
...
try:
    from core.token.bloom import TokenMinter, WalletState, BloomBalance, compute_gini
    from core.token.bloom import TOKEN_ZAKAT_RATE, SEED_MINT_FLOOR, BLOOM_MINT_FLOOR
...
try:
    ...
    c = router.route("Redesign the ReflexCache for concurrent UAB action streams with lock-free System-1 reads")
    ck("Complex query route()", lambda: c)
    e1 = router.estimate_complexity("hello")
    ck("Estimate simple complexity", lambda: e1)
    e2 = router.estimate_complexity("Design a thread-safe double-buffered hot/cold HashMap with ArcSwap for atomic pointer swap")
...
try:
    from core.constitutional.simulation import (
        SovereignNetworkSimulation, SimulationConfig, run_simulation)
