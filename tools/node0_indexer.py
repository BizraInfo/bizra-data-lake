...
PATHS = {
    ...
    "folder_index": Path(
        r"C:\BIZRA-DATA-LAKE\03_INDEXED\knowledge\node0_folders.jsonl"
    ),
    "software_index": Path(
        r"C:\BIZRA-DATA-LAKE\03_INDEXED\knowledge\node0_software.jsonl"
    ),
...
class Node0AssetIndexer:
            ...
            for item in root.iterdir():
                if item.is_dir() and not item.name.startswith("$"):
                    folder_info = {
                        ...
                        "path": str(item),
                        "is_bizra": any(
                            p.lower() in item.name.lower() for p in BIZRA_PATTERNS
                        ),
                    }
        ...
        if downloads_path.exists():
            try:
                ...
                bizra_in_dl = [
                    d.name
                    for d in dl_dirs
                    if any(p.lower() in d.name.lower() for p in BIZRA_PATTERNS)
                ]
    ...
    def full_scan(self) -> Dict[str, Any]:
        ...
        log.info("═" * 60)
        log.info(
            f"  Platform: {self.inventory['hardware'].get('platform', {}).get('model', 'Unknown')}"
        )
        log.info(
            f"  CPU: {self.inventory['hardware'].get('cpu', {}).get('name', 'Unknown')}"
        )
        log.info(
            f"  RAM: {self.inventory['hardware'].get('memory', {}).get('total_gb', 0)} GB"
        )
        log.info(
            f"  Software: {self.inventory['software'].get('installed_count', 0)} programs"
        )
        log.info(
            f"  WSL Distros: {len(self.inventory['software'].get('wsl_distros', []))}"
        )
        log.info(
            f"  BIZRA Folders: {self.inventory['data'].get('bizra_folder_count', 0)}"
        )
        ...
        log.info(
            f"  BIZRA in Downloads: {len(downloads_info.get('bizra_related_folders', []))}"
        )
        log.info("═" * 60)
