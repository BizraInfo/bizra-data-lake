...
from core.meta_prompt.models import (
    MetaPromptRequest,
    MetaPromptResponse,
    MetaPromptResult,
)
...
class MetaPromptEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self.wisdom = wisdom
        self.agents = {
            "OntologyArchitect": OntologyArchitect(
                name="OntologyArchitect",
                role="Structure",
                capabilities=caps.get("OntologyArchitect", []),
            ),
            "LanguageMaster": LanguageMaster(
                name="LanguageMaster",
                role="NLP",
                capabilities=caps.get("LanguageMaster", []),
            ),
            "LearningCore": LearningCore(
                name="LearningCore",
                role="Learning",
                capabilities=caps.get("LearningCore", []),
            ),
            "PromptEngineer": PromptEngineer(
                name="PromptEngineer",
                role="Prompts",
                capabilities=caps.get("PromptEngineer", []),
            ),
            "KnowledgeHarvester": KnowledgeHarvester(
                name="KnowledgeHarvester",
                role="Info",
                capabilities=caps.get("KnowledgeHarvester", []),
            ),
            "ReasoningEngine": ReasoningEngine(
                name="ReasoningEngine",
                role="Logic",
                capabilities=caps.get("ReasoningEngine", []),
            ),
            "MetaLearner": MetaLearner(
                name="MetaLearner",
                role="Optimization",
                capabilities=caps.get("MetaLearner", []),
            ),
            "OutputSynthesizer": OutputSynthesizer(
                name="OutputSynthesizer",
                role="Format",
                capabilities=caps.get("OutputSynthesizer", []),
            ),
            "EthicsGuardian": EthicsGuardian(
                name="EthicsGuardian",
                role="Ethics",
                capabilities=caps.get("EthicsGuardian", []),
            ),
        }

    async def run_knowledge_expansion(
        ...
        for agent_name in steps:
            try:
                ...
                results.append(
                    self._result(
                        agent_name, {"error": str(e)}, success=False, error=str(e)
                    )
                )
        ...
        explanation = (
            ...
            else "Workflow completed with critical failure"
        )
