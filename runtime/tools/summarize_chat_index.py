...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Aggregate a chat index (topics.json) into high-SNR summaries."
    )
    p.add_argument(
        "--run-dir", required=True, help="Path to chat_history/<run_id> directory"
    )
    p.add_argument(
        "--top-n",
        type=int,
        default=100,
        help="How many top items to keep per aggregate",
    )
    args = p.parse_args()
    ...
    def counter_to_rows(
        total: Counter, conv: Counter, top_n: int
    ) -> List[Dict[str, Any]]:
