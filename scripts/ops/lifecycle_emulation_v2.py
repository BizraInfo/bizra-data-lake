...
ck(
    "Ed25519 signature",
    lambda: f"{len(sig_bytes)} bytes, hex={sig_bytes.hex()[:24]}...",
)
# Verify signature
try:
...
e1 = Event(
    topic="genesis.boot", payload={"node": "NODE0", "id": iid[:16]}, source="lifecycle"
)
...
ck(
    ...
    lambda: f"events={len(cap)} priority={cap[1].priority.name}",
)
...
tampered = canonical_bytes(
    {"action": "heartbeat", "prev": "TAMPERED", "ihsan": 0.98, "ts": time.time()}
)
...
try:
    ...
    for i in range(3):
        cache.add(
            Reflex(trigger="rust_crypto", action="scholar", confidence=0.96 + i * 0.01)
        )
...
try:
    r = subprocess.run(
        ...
        text=True,
        timeout=60,
        cwd="/mnt/c/BIZRA-DATA-LAKE",
    )
    lines = r.stdout.strip().split("\n")
    count_line = [
        l
        for l in lines
        if "test" in l.lower() and ("selected" in l.lower() or "item" in l.lower())
    ]
    if count_line:
