...
def probe_container_health(state: GuardianState, correct: bool) -> list[ProbeResult]:
    ...
    if rc != 0:
        return [
            ProbeResult("containers", Severity.CRIT, f"Cannot list containers: {out}")
        ]
        ...
        if "unhealthy" in status.lower():
            ...
            if correct and name in RESTARTABLE_CONTAINERS and state.can_restart(name):
                ...
                state.record_correction(
                    "container_health", f"Restarted unhealthy: {name}"
                )
                results.append(
                    ProbeResult(
                        f"container:{name}",
    ...
    try:
        ...
        if used_pct >= MEMORY_CRIT_PCT:
            if correct:
                ...
                state.record_correction(
                    name, f"Pruned Docker build cache at {used_pct}% memory"
                )
                return ProbeResult(
                    name,
                    Severity.FIXED,
...
def probe_disk(state: GuardianState, correct: bool) -> list[ProbeResult]:
    ...
    for mount in ["/", "/mnt/c"]:
        ...
        try:
            ...
            results.append(
                ProbeResult(name, Severity.WARN, f"Cannot stat {mount}: {e}")
            )
    return results
...
def probe_ext4_errors(state: GuardianState, correct: bool) -> ProbeResult:
    ...
    ext4_lines = [
        l for l in out.splitlines() if "EXT4-fs error" in l or "ext4_" in l.lower()
    ]
    if ext4_lines:
        return ProbeResult(
            name,
...
def probe_critical_services(state: GuardianState, correct: bool) -> list[ProbeResult]:
    ...
    try:
        with socket.create_connection(("127.0.0.1", 6379), timeout=3) as s:
            ...
            if "+PONG" in resp or "NOAUTH" in resp:
                results.append(
                    ProbeResult("service:redis", Severity.OK, "Redis alive on 6379")
                )
            else:
                results.append(
                    ProbeResult(
                        "service:redis", Severity.WARN, f"Redis unexpected: {resp}"
                    )
                )
    except (OSError, ConnectionRefusedError):
        results.append(
            ProbeResult("service:redis", Severity.CRIT, "Redis unreachable on 6379")
        )
    ...
    try:
        with socket.create_connection(("127.0.0.1", 5433), timeout=3):
            results.append(
                ProbeResult(
                    "service:postgres", Severity.OK, "PostgreSQL accepting on 5433"
                )
            )
    except (OSError, ConnectionRefusedError):
        results.append(
            ProbeResult(
                "service:postgres", Severity.CRIT, "PostgreSQL unreachable on 5433"
            )
        )
    ...
    try:
        with socket.create_connection(("127.0.0.1", 8001), timeout=3):
            results.append(
                ProbeResult(
                    "service:chromadb", Severity.OK, "ChromaDB accepting on 8001"
                )
            )
    except (OSError, ConnectionRefusedError):
        results.append(
            ProbeResult(
                "service:chromadb", Severity.WARN, "ChromaDB unreachable on 8001"
            )
        )
...
def probe_journal_health(state: GuardianState, correct: bool) -> ProbeResult:
    ...
    if size_mb > JOURNAL_MAX_MB:
        if correct:
            ...
            state.record_correction(
                name, f"Vacuumed journals from {size_mb:.0f}MB to 200MB"
            )
            return ProbeResult(
                name,
                Severity.FIXED,
...
def probe_banned_services(state: GuardianState, correct: bool) -> list[ProbeResult]:
    ...
    for svc in BANNED_SYSTEMD_SERVICES:
        ...
        if out.strip() == "active":
            if correct:
                ...
                state.record_correction(
                    f"banned_svc:{svc}", f"Stopped and disabled {svc}"
                )
                results.append(
                    ProbeResult(
                        f"banned_svc:{svc}",
    ...
    if not results:
        results.append(
            ProbeResult("banned_svcs", Severity.OK, "No conflicting systemd services")
        )
    return results
...
def self_evaluate(results: list[ProbeResult], state: GuardianState) -> ProbeResult:
    ...
    if ihsan >= 0.95:
        return ProbeResult(
            "self_eval", Severity.OK, f"Ihsan {ihsan:.3f} — system excellent", details
        )
    elif ihsan >= 0.80:
        return ProbeResult(
            "self_eval", Severity.WARN, f"Ihsan {ihsan:.3f} — needs attention", details
        )
    else:
        return ProbeResult(
            "self_eval", Severity.CRIT, f"Ihsan {ihsan:.3f} — system degraded", details
        )
...
def daemon_loop(correct: bool) -> None:
    ...
    log.info(
        "Guardian daemon started (v%s, interval=%ds, correct=%s)",
        GUARDIAN_VERSION,
        DAEMON_INTERVAL,
        correct,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--check", action="store_true", help="Single health check (exit 0=ok, 1=issues)"
    )
    parser.add_argument(
        "--correct",
        action="store_true",
        help="Single check + auto-correct known issues",
    )
    parser.add_argument(
        "--daemon", action="store_true", help="Continuous monitoring loop"
    )
    parser.add_argument(
        "--report", action="store_true", help="JSON health report to stdout"
    )
