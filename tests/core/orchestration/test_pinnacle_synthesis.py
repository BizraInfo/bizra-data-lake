...
async def verify_pinnacle_synthesis():
    ...
    orchestrator = LearningLoopOrchestrator(
        hmm_engine=hhmm, context_cache=cache, enabled=True
    )
    _ = orchestrator
    ...
    logger.info(
        f"Strategic Prediction: {prediction.strategic_goal.name} (Conf: {prediction.strategic_confidence:.2f})"
    )
    ...
    logger.info(
        f"Amplification Context: Focus={ctx.focus}, Depth={ctx.got_depth}, Hypotheses={ctx.got_hypotheses}"
    )
