...
class UnifiedOrchestrator:
        ...
        if PCI_AVAILABLE:
            try:
                ...
                policy_hash = hashlib.blake2b(
                    f"ihsan_threshold:{self.ihsan_threshold}".encode(), digest_size=32
                ).hexdigest()
                state_hash = hashlib.blake2b(
                    f"snr_threshold:{self.snr_threshold}".encode(), digest_size=32
                ).hexdigest()
    ...
    async def process(self, request: OrchestrationRequest) -> OrchestrationResult:
        """
        ...
        Processing Flow:
            ...
            7. Final Ihsan check
        """
        return await self._process_internal(request, mode=ProcessingMode.ONLINE)
    ...
    async def process_offline(
        ...
        return await self._process_internal(request, mode=ProcessingMode.OFFLINE)
    ...
    async def _process_internal(
        self, request: OrchestrationRequest, mode: ProcessingMode
    ) -> OrchestrationResult:
        """
        ...
        try:
            ...
            if not gate_result.passed:
                return self._create_failed_result(
                    ...
                    stage_results,
                    receipt_chain,
                    ProcessingStage.PCI_GATE_CHAIN,
                    mode,
                    start_time,
                )
            ...
            if not pat_result.passed:
                return self._create_failed_result(
                    ...
                    stage_results,
                    receipt_chain,
                    ProcessingStage.PAT_ENFORCEMENT,
                    mode,
                    start_time,
                )
            ...
            if not snr_result.passed:
                return self._create_failed_result(
                    ...
                    stage_results,
                    receipt_chain,
                    ProcessingStage.SNR_ENFORCEMENT,
                    mode,
                    start_time,
                )
            ...
            if not sov_result.passed:
                return self._create_failed_result(
                    ...
                    stage_results,
                    receipt_chain,
                    ProcessingStage.SOVEREIGNTY_CHECK,
                    mode,
                    start_time,
                )
    ...
    async def _stage_pci_envelope(
        ...
        try:
            ...
            envelope = (
                EnvelopeBuilder()
                .with_sender(
                    agent_type, request.agent_id, self.pci_keypair.public_key_hex
                )
                .with_action(
                    "orchestrate",
                    {
                ...
                .with_scores(
                    ihsan=(
                        request.ihsan_scores.get("composite", 0.95)
