...
class EcosystemBridge:
    ...
    async def query(self, unified_query: UnifiedQuery) -> UnifiedResponse:
        ...
        if self.ultimate_engine and unified_query.require_constitution_check:
            const_ok, const_reason = (
                self.ultimate_engine.constitution.verify_compliance(
        ...
        if self.orchestrator and unified_query.use_orchestrator:
            try:
                ...
                reasoning_trace.append(
                    f"✓ Orchestrator enhanced with {len(sources)} sources"
                )
    ...
    def get_health(self) -> EcosystemHealth:
        ...
        if ULTIMATE_ENGINE_AVAILABLE:
            health.kernel_invariants_ok = (
                RIBA_ZERO and ZANN_ZERO and IHSAN_FLOOR == 0.90
            )
...
def get_ecosystem(
    human_name: str = "Ahmed Al-Mansoori", daughter_name: str = "Layla"
) -> EcosystemBridge:
...
async def demonstrate_ecosystem():
    ...
    ecosystem = await initialize_ecosystem(
        human_name="Ahmed Al-Mansoori", daughter_name="Layla"
    )
    ...
    for i, query_text in enumerate(queries, 1):
        ...
        query = UnifiedQuery(
            text=query_text, require_constitution_check=True, require_daughter_test=True
        )
...
def main():
    ...
    parser.add_argument(
        "--status", "-s", action="store_true", help="Show ecosystem status"
    )
    parser.add_argument(
        "--health", "-H", action="store_true", help="Show health report"
    )
