...
# Configure logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s | %(levelname)s | DISCIPLINE | %(message)s"
)
...
def build_discipline_taxonomy() -> Dict[int, Discipline]:
    ...
    l4_disciplines = [
        ...
        (
            27,
            "Poetics",
            "High-Density Meaning Compression",
...
class GraphTheoryGenerator(GeneratorBase):
    ...
    def get_disciplines(self) -> List[Discipline]:
        return [d for d in self.taxonomy.values() if self.generator in d.generators]
...
class InformationTheoryGenerator(GeneratorBase):
    ...
    def calculate_strength(self, corpus_stats: Dict) -> float:
        ...
        # Combined strength
        strength = (
            (snr_score * 0.5) + (embedding_score * 0.3) + (embedding_coverage * 0.2)
        )
        return min(strength, 1.0)
...
class DisciplineSynthesisEngine:
    ...
    def __init__(self):
        ...
        logger.info(
            f"DisciplineSynthesisEngine initialized with {len(self.taxonomy)} disciplines"
        )
            ...
            try:
                with open(embedding_checkpoint, "r") as f:
                    ...
                    stats["embedding_coverage"] = (
                        processed / total_docs if total_docs > 0 else 0.0
                    )
            except Exception as e:
    ...
    def scan_corpus_for_disciplines(self) -> Dict[int, float]:
        ...
        if GOLD_PATH.exists():
            for gold_file in GOLD_PATH.glob("*.jsonl"):
                try:
                    corpus_text.append(
                        gold_file.read_text(encoding="utf-8", errors="ignore")
                    )
                except Exception:
                    pass
        ...
        if INDEXED_PATH.exists():
            for idx_file in list(INDEXED_PATH.glob("*.jsonl"))[:50]:  # Limit
                try:
                    corpus_text.append(
                        idx_file.read_text(encoding="utf-8", errors="ignore")
                    )
                except Exception:
                    pass
        ...
        combined_text = "\n".join(corpus_text)
        logger.info(
            f"Scanned {len(corpus_text)} files, {len(combined_text):,} characters"
        )
    ...
    def calculate_generator_strengths(
    ...
    def cascade_amplification(
        self, generator_strengths: Dict[Generator, float]
    ) -> Dict[int, float]:
        """
    ...
    def generate_recommendations(
        self, gaps: List[Discipline], generator_strengths: Dict[Generator, float]
    ) -> List[str]:
        ...
        for gap in gaps:
            ...
            for gen_type, strength in sorted(
                generator_strengths.items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def print_report(self, report: DisciplineReport):
        ...
        print("\n📊 GENERATOR STRENGTHS:")
        print("-" * 40)
        for gen, strength in sorted(
            report.generator_strength.items(), key=lambda x: x[1], reverse=True
        ):
            bar = "█" * int(strength * 20)
        ...
        for gap in report.critical_gaps[:5]:
            print(
                f"  [{gap.id:2}] {gap.name:<25} {gap.coverage:.1%} ({gap.layer.value})"
            )
        ...
        for strong in report.strong_disciplines[:5]:
            print(
                f"  [{strong.id:2}] {strong.name:<25} {strong.coverage:.1%} ({strong.layer.value})"
            )
