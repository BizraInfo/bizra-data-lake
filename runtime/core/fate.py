    ...
    def threshold_for(self, env: str, artifact_class: Optional[str] = None) -> float:
        ...
        if artifact_class:
            ...
            if normalized_artifact:
                artifact_threshold = self.thresholds_by_artifact_class.get(
                    normalized_artifact
                )
...
class IhsanVector(BaseModel):
    ...
    def composite_score(self, weights: Dict[str, float]) -> float:
        ...
        return float(
            sum(
                float(weights.get(k, 0.0)) * float(data.get(k, 0.0))
                for k in CANONICAL_DIMENSIONS
            )
        )
...
class FateEngine:
    def __init__(
        self, *, strict_mode: bool = True, validator: str = "Node0_Sovereign_Kernel"
    ):
    ...
    def _env(self) -> str:
        return (
            os.getenv("BIZRA_ENV") or os.getenv("ENV") or self.policy.default_env
        ) or self.policy.default_env
        ...
        *,
        ...
        if self.strict_mode and not self.policy.loaded:
            return self._seal(
                env=normalized_env,
                threshold=1.0,
                vector=IhsanVector.model_validate(
                    {k: 0.0 for k in CANONICAL_DIMENSIONS}
                ),
                composite=0.0,
                verdict="REJECTED",
        ...
        verdict: Literal["APPROVED", "REJECTED"] = (
            "APPROVED" if composite >= threshold else "REJECTED"
        )
        return self._seal(
            env=normalized_env,
            threshold=threshold,
...
CORRECTION_GUIDANCE: Dict[RejectionCode, Dict[str, Any]] = {
    ...
    RejectionCode.RJ_IH_002: {
        ...
        "examples": [
            "Add 'My objective is to...' at the start",
    ...
    RejectionCode.RJ_SV_001: {
        ...
        "examples": [
            "Use MasterReasoner (local) instead of GPT-4",
            "Set BIZRA_BACKEND=ollama",
        ],
        "retryable": True,
    },
    ...
    RejectionCode.RJ_EV_001: {
        ...
        "examples": [
            "Include 'evidence_hash' in payload",
            "Generate via genesis_receipt.py",
        ],
        "retryable": True,
    },
...
class FateEngineWithCorrection(FateEngine):
    ...
    def _build_feedback(
        self, code: RejectionCode, seal: FateSeal, request_hash: str, retry_count: int
    ) -> CorrectionFeedback:
        """Build structured correction feedback."""
        guidance = CORRECTION_GUIDANCE.get(
            code, CORRECTION_GUIDANCE[RejectionCode.RJ_IH_001]
        )

        explanation = guidance["explanation"]
        if "{score" in explanation:
            explanation = explanation.format(
                score=seal.composite_score, threshold=seal.threshold
            )
    ...
    def audit_request_with_feedback(
        ...
        *,
        ...
        # Get base seal
        seal = self.audit_request(
            intent=intent, context=context, artifact_class=artifact_class
        )
    ...
    def submit_correction(
        self, original_hash: str, corrected_intent: str, context: str = ""
    ) -> Tuple[FateSeal, Optional[CorrectionFeedback]]:
        """
        ...
        return self.audit_request_with_feedback(
            intent=corrected_intent, context=context
        )
