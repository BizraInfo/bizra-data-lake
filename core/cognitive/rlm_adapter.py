...
# Import BIZRA core
from core.fate import FateEngine, FateSeal, IhsanVector, get_fate_engine
from core.sape import compile_sape_plan, SapePlanRequest
...
class BizraRLMAdapter:
    ...
    def _init_rlm(self):
        ...
        try:
            ...
            # Get model from slot
            slot_config = BIZRA_MODEL_SLOTS.get(self.config.slot, BIZRA_MODEL_SLOTS["cold_core"])
            ...
            raise RuntimeError(
                f"RLM not available. Ensure rlm/ is in path. Error: {e}"
            )
    ...
    def _calculate_ihsan(self, response: str, execution_time: float) -> Tuple[float, bool, List[str]]:
    ...
    def completion(
        ...
        if hasattr(result, "usage_summary") and result.usage_summary:
            usage_dict = result.usage_summary.to_dict() if hasattr(result.usage_summary, "to_dict") else {}
...
if __name__ == "__main__":
    ...
    parser.add_argument("prompt", nargs="?", default="Calculate 2^10 using Python code.")
    parser.add_argument("--slot", default="cold_core", choices=["cold_core", "warm_surface", "primary_reasoning"])
    ...
    print(f"   Ihsān Score: {result.ihsan_score:.4f} {'✅' if result.ihsan_passed else '❌'}")
