...
class CostAnalyzer:
    ...
    def _update_metrics(self, record: UsageRecord) -> None:
        ...
        self._metrics.tokens_by_model[record.model_name] += record.input_tokens + record.output_tokens
        ...
        self._metrics.tokens_by_agent[record.agent_name] += record.input_tokens + record.output_tokens
        ...
        self._metrics.tokens_by_category[record.task_category] += record.input_tokens + record.output_tokens
    ...
    def compute_savings(self) -> Dict[str, Any]:
        ...
        max_cost_model = max(
            ...
            key=lambda c: c.avg_cost_per_1k_tokens
        )
        ...
        for record in self._usage_history:
            ...
            baseline_cost += (total_tokens / 1000) * max_cost_model.avg_cost_per_1k_tokens
        ...
        target_status = "below_target" if savings_rate < self.TARGET_SAVINGS_MIN else (
            "above_target" if savings_rate > self.TARGET_SAVINGS_MAX else "on_target"
        )
...
def main():
    ...
    parser.add_argument("--optimize", type=str, help="Optimize model selection for category")
