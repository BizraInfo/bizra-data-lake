...
def test_complete_sovereignty_workflow():
    ...
    for i, node in enumerate(proof_chain):
        op = node.data.get('data', {}).get('operation', node.data.get('type', 'unknown'))
    ...
    bad_receipt = constitution.verify(
        operation="non_compliant_operation",
        scores=bad_scores
    )
    ...
    print(f"✓ HYPER LOOPBACK verified (100% offline)")
    print(f"✓ Ihsān threshold maintained (≥ 0.95)")
    print(f"✓ Evidence chain integrity confirmed")
    print(f"✓ Constitutional compliance enforced")
