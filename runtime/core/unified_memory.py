...
if not DATA_LAKE_AVAILABLE:
    logger.warning(
        "Data Lake bridge not available - unified memory will use local tiers only"
    )
    ...
    async def initialize(self) -> None:
        ...
        if DATA_LAKE_AVAILABLE and get_data_lake_bridge is not None:
            try:
                ...
                if status.online:
                    logger.info(
                        "Data Lake connected: %s (%d nodes)", status.url, status.nodes
                    )
                else:
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        await memory.store(
            "BIZRA uses PAT-SAT dual-agentic architecture", tier=MemoryTier.L4_SEMANTIC
        )
        await memory.store(
            "SAPE has 9 probes for validation", tier=MemoryTier.L4_SEMANTIC
        )
