...
class VectorEngine:
    ...
    def process_all(self, include_images: bool = True):
        ...
        for _, row in tqdm(
            df_docs.iterrows(), total=len(df_docs), desc="Contextual Chunking"
        ):
        ...
        if all_chunks:
            ...
            if save_every and save_every > 0:
                print(
                    f"⚡ Generating text embeddings on {self.device} (chunked save every {save_every})..."
                )
                part = 0
                ...
                for i in range(0, len(df_chunks), save_every):
                    ...
                    CHECKPOINT_PATH.write_text(
                        json.dumps({"last_index": i, "part": part}), encoding="utf-8"
                    )
                ...
                print(
                    f"✅ Text Chunks saved: {CHUNKS_TABLE_PATH} ({len(df_chunks)} chunks)"
                )
    ...
    def _process_images(self, image_docs: List[Dict]):
            ...
            image_chunks.append(
                {
                    ...
                    "chunk_metadata_json": json.dumps(
                        {"source_type": "image", "original_path": file_path}
                    ),
                }
            )
        ...
        if image_chunks:
            ...
            print(
                f"✅ Image embeddings saved: {image_chunks_path} ({len(df_images)} images)"
            )
