...
class SNRMaximizer:
    ...
    async def optimize(self, content: str) -> Dict[str, Any]:
        """
        Async optimize method for SovereignRuntime API compatibility.

        Wraps the synchronous maximize() method with an async interface
        and returns a dict suitable for the runtime pipeline.

        Args:
            content: Text content to optimize for maximum SNR

        Returns:
            Dict containing:
            - snr_score: float - SNR score after optimization
            - ihsan_score: float - Ihsān compliance score
            - optimized: str - Optimized content (if improved)
            - passed: bool - Whether content passes Ihsān threshold
            - recommendations: List[str] - Improvement suggestions
        """
        # Run synchronous maximize in executor to avoid blocking
