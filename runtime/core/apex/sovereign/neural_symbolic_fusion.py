...
class FusionContext:
    ...
    def context_id(self) -> str:
        ...
        content = (
            f"{self.query}:{self.domains}:{datetime.now(timezone.utc).isoformat()}"
        )
...
class DefaultSAPEEngine:
    ...
    async def run_probes_parallel(
        ...
        """Run all 9 SAPE probes in parallel."""
        # Create probe tasks
        tasks = [self._run_probe(probe, content, context) for probe in self.PROBES]
...
class NeuralSymbolicFusionEngine:
    ...
    def __init__(
        ...
        self._constitutional_verifier = (
            constitutional_verifier or DefaultConstitutionalVerifier()
        )
    ...
    async def _fuse_iterative(self, context: FusionContext) -> FusionResult:
        ...
        while iteration < context.max_iterations:
            ...
            if (
                best_result is None
                or result.fused_confidence > best_result.fused_confidence
            ):
                best_result = result
    ...
    async def _symbolic_preprocessing(
        ...
        if context.knowledge_graph_nodes:
            try:
                await self._kg_connector.ground_reasoning(
                    "", context.knowledge_graph_nodes[:1]
                )
            except Exception:
                pass
                ...
                claims.append(
                    {
                        ...
                        "domain": (
                            neural_result.domains_covered[0]
                            if neural_result.domains_covered
                            else "general"
                        ),
                    }
                )
    ...
    def _combine_results(
        ...
        success = (
            ...
            and neural_result.snr_score >= DEFAULT_SNR_THRESHOLD * 0.95
        )
...
async def main() -> None:
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        print(
            f"   - Constraints Checked: {len(result.symbolic_result.constraints_checked)}"
        )
        ...
        if result.symbolic_result.violations:
            ...
            for v in result.symbolic_result.violations[:3]:
                print(
                    f"   - {v.constraint_id}: {v.actual_value:.3f} < {v.expected_value:.3f}"
                )
