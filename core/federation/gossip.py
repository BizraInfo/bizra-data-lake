...
class GossipEngine:
    ...
    def _collect_piggyback(self, max_items: int = 5) -> List[Dict]:
        ...
        updates = []
        for peer in sorted(self.peers.values(), key=lambda p: p.last_seen, reverse=True)[:max_items]:
    ...
    def _merge_node_state(self, update: Dict):
        ...
        if node_id in self.peers:
            ...
            if new_incarnation > existing.incarnation:
                ...
                existing.ihsan_average = update.get("ihsan_average", existing.ihsan_average)
        else:
    ...
    def _handle_ping(self, msg: GossipMessage) -> bytes:
        ...
        ack = self._create_message(MessageType.PING_ACK, {
            "in_response_to": msg.sequence,
            "node_count": self.get_network_size()
        })
        return ack.to_bytes()
    ...
    def _handle_pattern_share(self, msg: GossipMessage) -> bytes:
        ...
        ack = self._create_message(MessageType.PATTERN_ACK, {
    ...
    def create_announce_message(self) -> bytes:
        ...
        msg = self._create_message(MessageType.ANNOUNCE, {
    ...
    def create_leave_message(self) -> bytes:
        ...
        msg = self._create_message(MessageType.LEAVE, {
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "suspect_peers": len([p for p in self.peers.values() if p.state == NodeState.SUSPECT]),
            "dead_peers": len([p for p in self.peers.values() if p.state == NodeState.DEAD]),
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        engine = GossipEngine(
            ...
            on_pattern_received=lambda p: print(f"  📦 Pattern received: {p.get('pattern_id', 'unknown')}")
        )
        nodes.append(engine)
    ...
    for i, node in enumerate(nodes):
        ...
        print(f"  Node {i}: {stats['alive_peers']} peers, multiplier={stats['network_multiplier']}")
