...
class DiffusionReasoningAmplifier:
    ...
    def amplify(
        ...
        confidence = max(0.0, min(1.0, float(prediction.prediction_confidence)))
        ...
        complexity_hint = min(
            ...
            confidence * self.config.max_complexity_hint,
        )
        got_hypotheses = max(
            1,
            min(
        ...
        got_depth = max(
            ...
            min(
                ...
                int(round(1 + confidence * (self.config.max_depth - 1))),
            ),
        )
        if confidence >= 0.98:
