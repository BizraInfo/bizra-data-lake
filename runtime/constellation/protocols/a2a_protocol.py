...
class AgentMailbox:
    ...
    handlers: dict[MessageType, list[MessageHandler]] = field(
...
class A2ARouter:
    ...
    def __init__(self):
        ...
        self._threads: dict[str, list[str]] = defaultdict(
            list
    ...
    async def _deliver(self, message: A2AMessage) -> DeliveryStatus:
        ...
        logger.debug(
            f"Delivered {message.type.value} from {message.sender} to {message.recipient}"
        )
...
class MessageBuilder:
    ...
    def propose(
        self, proposal: str, details: Optional[dict] = None
    ) -> "MessageBuilder":
