...
def _stdlib_beta_sample(
    alpha: float, beta: float, rng: Optional[random.Random] = None
) -> float:
...
class CapabilityMatrix:
    ...
    def get_candidates(
        self, category: TaskCategory, min_affinity: float = 0.5
    ) -> List[str]:
        """
...
class ThompsonSamplingRouter:
    ...
    def __init__(
        ...
        if NUMPY_AVAILABLE:
            ...
            logger.warning(
                "numpy not available - using stdlib random (reduced precision)"
            )
        ...
        for key_str, prior_data in data.get("priors", {}).items():
            ...
            if len(parts) == 2:
                ...
                try:
                    ...
                    router._priors[(agent_name, category)] = BetaPrior.from_dict(
                        prior_data
                    )
                except ValueError:
        ...
        try:
            ...
            for key_str, prior_data in data.get("priors", {}).items():
                ...
                if len(parts) == 2:
                    ...
                    try:
                        category = TaskCategory(cat_str)
                        self._priors[(agent_name, category)] = BetaPrior.from_dict(
                            prior_data
                        )
                    except ValueError:
                        pass
...
def main():
    ...
    if args.task:
        ...
        for category in [
            ...
            for r in rankings[:3]:
                print(
                    f"    {r['agent_name']}: mean={r['mean']:.3f}, samples={r['total_samples']}"
                )
        ...
        for agent_name in router.capability_matrix.profiles.keys():
            ...
            for cat, cat_stats in stats["categories"].items():
                if cat_stats["total_samples"] > 0:
                    print(
                        f"    {cat}: mean={cat_stats['mean']:.3f}, samples={cat_stats['total_samples']}"
                    )
