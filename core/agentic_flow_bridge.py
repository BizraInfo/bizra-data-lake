...
AGENTIC_FLOW_DEFAULT_TOPOLOGY = os.getenv("AGENTIC_FLOW_DEFAULT_TOPOLOGY", "hierarchical-mesh")

# Receipt storage
RECEIPT_DIR = Path(__file__).parent.parent / "docs" / "evidence" / "receipts" / "agentic_flow"
...
class AgenticFlowBridge:
    ...
    async def invoke_swarm(
        ...
        try:
            ...
            receipt = self._emit_receipt("swarm_invoke", "success", {
                "task": task,
                "topology": topology.value,
                "agent_count": agent_count,
            ...
            latency = (datetime.now(timezone.utc) - start).total_seconds() * 1000
            self._emit_receipt("swarm_invoke", "failure", {
                "task": task,
                "topology": topology.value,
                "error": str(e),
                "latency_ms": latency,
    ...
    async def call_mcp_tool(self, tool_name: str, arguments: dict) -> dict:
        ...
        try:
            ...
            self._emit_receipt("mcp_tool_call", "success", {
                "tool": tool_name,
                "arguments": arguments,
            })
            ...
            self._emit_receipt("mcp_tool_call", "failure", {
                "tool": tool_name,
                "error": str(e),
            })
    ...
    async def query_reasoning_bank(self, query: str, limit: int = 10) -> dict:
        ...
        """
        return await self.call_mcp_tool("reasoning_bank_query", {
            "query": query,
            "limit": limit,
        })
    ...
    async def dispatch_worker(self, worker_type: WorkerType, directive: str) -> dict:
        ...
        """
        return await self.call_mcp_tool("worker_dispatch", {
