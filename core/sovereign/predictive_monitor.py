"""
Predictive Monitor — Trend Analysis and Forecasting
====================================================
...
class PredictiveMonitor:
        ...
        for name, analysis in self._analyses.items():
            ...
            if analysis.anomaly_score > 0.8:
                alert = PredictiveAlert(
                    ...
                    metric_name=name,
                    severity=AlertSeverity.WARNING if analysis.anomaly_score < 0.95 else AlertSeverity.CRITICAL,
                    message=f"Anomaly detected in {name}",
                    prediction=f"Current value deviates significantly from historical pattern",
            ...
            # Trend-based alerts
            if analysis.direction == TrendDirection.RISING and analysis.confidence > 0.7:
                if analysis.forecast_1h and analysis.forecast_1h > self._get_threshold(name, "high"):
                    alert = PredictiveAlert(
                        id=f"alert-{self._alert_count}",
                        metric_name=name,
            ...
            elif analysis.direction == TrendDirection.FALLING and analysis.confidence > 0.7:
                if analysis.forecast_1h and analysis.forecast_1h < self._get_threshold(name, "low"):
                    alert = PredictiveAlert(
                        id=f"alert-{self._alert_count}",
                        metric_name=name,
    ...
    def _get_threshold(self, metric_name: str, threshold_type: str) -> float:
        ...
        metric_thresholds = defaults.get(metric_name, {"high": float("inf"), "low": float("-inf")})
