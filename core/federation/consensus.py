...
class ConsensusState:
    ...
    def prepare_count(self) -> int:
        ...
        return sum(1 for v in self.prepare_votes.values() if v.decision == VoteDecision.ACCEPT)
    
    def commit_count(self) -> int:
        """Count ACCEPT votes in commit phase."""
        return sum(1 for v in self.commit_votes.values() if v.decision == VoteDecision.ACCEPT)
    ...
    def validate(
        ...
        if origin in self.node_reputation:
            if self.node_reputation[origin] < 0.5:
                return VoteDecision.REJECT, f"Origin node reputation too low", ihsan
        
        # 5. Check impact score
        if envelope.metadata.impact_score < 0.5:
            return VoteDecision.REJECT, f"Impact score too low: {envelope.metadata.impact_score}", ihsan
        ...
        if local_ihsan_score is not None:
            ...
            if combined < MIN_IHSAN_SCORE:
                return VoteDecision.REJECT, f"Combined Ihsān {combined:.2f} too low", combined
            ihsan = combined
...
class PatternConsensus:
    ...
    async def propose_pattern(self, envelope: PatternEnvelope) -> ConsensusState:
        ...
        if pattern_id in self.completed_rounds:
            # Already decided
            state = ConsensusState(pattern_id=pattern_id, phase=ConsensusPhase.FINALIZED)
    ...
    def _finalize_accept(self, state: ConsensusState) -> ConsensusResult:
        ...
        all_votes = list(state.prepare_votes.values()) + list(state.commit_votes.values())
        
        result = ConsensusResult(
            pattern_id=state.pattern_id,
            accepted=True,
            accept_votes=state.commit_count(),
            reject_votes=sum(1 for v in state.commit_votes.values() if v.decision == VoteDecision.REJECT),
            abstain_votes=sum(1 for v in state.commit_votes.values() if v.decision == VoteDecision.ABSTAIN),
            quorum_reached=True,
        ...
        logger.info(f"✅ Pattern {state.pattern_id[:16]} ACCEPTED by consensus ({result.accept_votes}/{CONSENSUS_QUORUM})")
    ...
    def _finalize_timeout(self, state: ConsensusState) -> ConsensusResult:
        ...
        result = ConsensusResult(
            ...
            reject_votes=sum(1 for v in state.prepare_votes.values() if v.decision == VoteDecision.REJECT),
            abstain_votes=sum(1 for v in state.prepare_votes.values() if v.decision == VoteDecision.ABSTAIN),
        ...
        logger.warning(f"⏱️ Pattern {state.pattern_id[:16]} TIMEOUT - no consensus reached")
    ...
    def cleanup_old_rounds(self, max_age_sec: float = 3600.0):
        ...
        to_remove = [
            pid for pid, result in self.completed_rounds.items()
...
class ConsensusStats:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "acceptance_rate": (
                self.rounds_accepted / max(1, self.rounds_started)
            ),
        }
