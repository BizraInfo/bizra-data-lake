...
class AgentCard:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentCard":
        ...
        return cls(
            ...
            security_schemes=[
                SecurityScheme.from_dict(s) for s in data.get("securitySchemes", [])
            ],
            signature=signature,
...
class A2AChannel:
    ...
    async def close(self) -> None:
        ...
        try:
            builder = MessageBuilder(self.local_agent.agent_id)
            close_msg = (
                builder.inform("Channel closing")
                .to(self.remote_agent.agent_id)
...
class ConstellationHandshake:
    ...
    def verify_agent_card(self, card: AgentCard) -> bool:
        ...
        try:
            ...
            if card.signature.algorithm == "SHA256-Fallback":
                logger.info(
                    f"AgentCard {card.agent_id} has fallback signature (not cryptographically verified)"
                )
                return True
            return False
                        ...
                        if response.status == 200:
                            ...
                            for card in cards:
                                if self.verify_agent_card(card):
                                    ...
                                    logger.warning(
                                        f"Discovered card failed verification: {card.name}"
                                    )
    ...
    async def establish_channel(
        ...
        logger.info(
            f"Established channel {result.channel_id}: {agent_a.name} <-> {agent_b.name}"
        )
