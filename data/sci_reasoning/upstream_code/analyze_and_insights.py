...
def call_gpt(messages: List[Dict]) -> str:
    ...
    print(
        f"  API call: {response.usage.prompt_tokens} in, {response.usage.completion_tokens} out, ${cost:.4f}"
    )
...
def analyze_results(papers: List[Dict], taxonomy: List[Dict]) -> Dict:
    ...
    return {
        ...
        "pattern_pairs": {
            str(k): v
            for k, v in sorted(pattern_pairs.items(), key=lambda x: x[1], reverse=True)[
                :30
            ]
        },
...
def generate_deep_insights(
    ...
    for year in sorted(analysis["by_year"].keys()):
        ...
        year_data[year] = {
            "total": year_total,
            "top_patterns": sorted(
                analysis["by_year"][year].items(), key=lambda x: x[1], reverse=True
            )[:5],
        }
    ...
    for year, data in year_data.items():
        top = [
            (analysis["pattern_names"].get(p, p), c, c / data["total"] * 100)
            for p, c in data["top_patterns"]
        ]
    ...
    for conf in sorted(analysis["by_conference"].keys()):
        ...
        conf_data[conf] = {
            "total": conf_total,
            "top_patterns": sorted(
                analysis["by_conference"][conf].items(),
                key=lambda x: x[1],
                reverse=True,
            )[:5],
        }
    ...
    for conf, data in conf_data.items():
        top = [
            (analysis["pattern_names"].get(p, p), c / data["total"] * 100)
            for p, c in data["top_patterns"]
        ]
    ...
    for ptype in analysis["by_presentation_type"]:
        ...
        type_data[ptype] = {
            ...
            "patterns": {
                p: c / type_total * 100
                for p, c in analysis["by_presentation_type"][ptype].items()
            },
        }
...
def main():
    ...
    for year in sorted(analysis["by_year"].keys()):
        ...
        print(
            f"  {year}: {total} papers | Top: {analysis['pattern_names'].get(top[0], top[0])}"
        )
    ...
    for conf in sorted(analysis["by_conference"].keys()):
        ...
        print(
            f"  {conf}: {total} papers | Top: {analysis['pattern_names'].get(top[0], top[0])}"
        )
    ...
    with open(OUTPUT_PATH / "deep_insights.md", "w") as f:
        ...
        f.write(
            f"*Analysis of {analysis['total_papers']} papers from NeurIPS, ICML, ICLR (oral & spotlight)*\n\n"
        )
        f.write("---\n\n")
        f.write(insights)
