...
from .receipt import CommitReceipt, get_receipt_generator, get_receipt_store
...
try:
    from bizra_kernel.snr_enforcer import get_snr_enforcer, OperationType, EnforcementContext
...
class GateChain:
    ...
    def verify(
        ...
        try:
            # CHEAP TIER (<10ms)
            cheap_gates = [
                (Gate.SCHEMA, lambda: self._gate_schema(envelope_data, digest, timestamp)),
                (Gate.SIGNATURE, lambda: self._gate_signature(envelope, digest, timestamp)),
                (Gate.TIMESTAMP, lambda: self._gate_timestamp(envelope, digest, timestamp)),
            ...
            if require_expensive:
                expensive_gates = [
                    ...
                    (Gate.FORMAL, lambda: self._gate_formal(envelope, digest, timestamp)),
                ]
    ...
    def _gate_timestamp(
        ...
        try:
            envelope_dt = datetime.fromisoformat(envelope.timestamp.replace('Z', '+00:00'))
            ...
            if skew_seconds > TIMESTAMP_SKEW_SECONDS:
                return GateResult(
                    ...
                    passed=False,
                    latency_ms=0.0,
                    rejection=reject_timestamp_stale(digest, timestamp, envelope.timestamp, skew_seconds),
                )
            ...
            if skew_seconds < -TIMESTAMP_SKEW_SECONDS:
                return GateResult(
                    ...
                    passed=False,
                    latency_ms=0.0,
                    rejection=reject_timestamp_future(digest, timestamp, envelope.timestamp, abs(skew_seconds)),
                )
            ...
            return GateResult(
                ...
                passed=False,
                latency_ms=0.0,
                rejection=reject_internal_error(digest, timestamp, f"Timestamp parse error: {e}"),
            )
            ...
            try:
                ...
                if not result.passed:
                    ...
                    return GateResult(
                        ...
                        details={"receipt_id": result.receipt_id, "enforcer_used": True},
                    )
        ...
        try:
            ...
            if not passed:
                return GateResult(
                    ...
                    passed=False,
                    latency_ms=0.0,
                    rejection=reject_fate_violation(digest, timestamp, invariant, details),
                )
    ...
    def _gate_formal(
        ...
        try:
            ...
            return GateResult(
                ...
                passed=False,
                latency_ms=0.0,
                rejection=reject_internal_error(digest, timestamp, f"Formal verifier error: {e}"),
            )
