...
class E2EPipeline:
    ...
    async def run(self, query: str) -> E2EResult:
        ...
        try:
            ...
            response = await self._run_response_phase(
                inference_result, snr_score, ihsan_score
            )
    ...
    async def _run_skills_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log(
                f"Agents: {list(stats['by_agent'].keys())[:3]}...", TestPhase.SKILLS
            )
            ...
            print(
                f"└─ ✓ SKILLS passed ({stats['total_skills']} skills, {duration:.1f}ms)"
            )
            print()
    ...
    async def _run_got_phase(self, query: str) -> Dict[str, Any]:
        ...
        try:
            from core.nexus import ThoughtGraph, ThoughtType
            ...
            print(
                f"└─ ✓ GOT passed (confidence: {graph_confidence:.4f}, {duration:.1f}ms)"
            )
            print()
        ...
        payload = json.dumps(
            {
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a BIZRA sovereign AI assistant. Respond with excellence.",
                    },
...
"""
    ...
    async def _run_snr_phase(self, response: str) -> float:
        ...
        try:
            ...
            print(
                f"└─ {status} SNR GATE {'passed' if passed else 'FAILED'} (SNR: {snr_score:.4f}, {duration:.1f}ms)"
            )
            print()
    ...
    async def _run_ihsan_phase(self, response: str, snr_score: float) -> float:
        ...
        try:
            ...
            status = "✓" if passed else "⚠"
            print(
                f"└─ {status} IHSĀN GATE {'passed' if passed else 'below threshold'} (Ihsān: {ihsan_score:.4f}, {duration:.1f}ms)"
            )
            print()
    ...
    async def _run_response_phase(
        self, response: str, snr_score: float, ihsan_score: float
    ) -> str:
...
"""
            ...
            print(
                f"└─ ✓ RESPONSE generated ({len(final_response)} chars, {duration:.1f}ms)"
            )
            print()
...
async def main():
    ...
    if args.benchmark:
        ...
        print(
            f"  Success Rate: {successes}/{args.iterations} ({100*successes/args.iterations:.1f}%)"
        )
