...
def main():
    ...
    parser.add_argument(
        "--query", type=str, default=None, help="Override the default synthesis query"
    )
    parser.add_argument(
        "--skip-preflight", action="store_true", help="Skip validation preflight checks"
    )
    parser.add_argument(
        "--skip-validation", action="store_true", help="Skip knowledge base validation"
    )
    parser.add_argument(
        "--full-spectrum",
        action="store_true",
        help="Run full-spectrum singularity test",
    )
    parser.add_argument(
        "--report-path", type=str, default=None, help="Write JSON report to path"
    )
    ...
    try:
        ...
        result = engine.process_query(
            query=query,
            context={
                "intent": "grand_synthesis",
                "mode": "professional_elite",
                "scan_depth": "full",
            },
        )
        ...
        # Display Results
        print("\n" + "=" * 80)
        print("  🌌 PEAK MASTERPIECE SYNTHESIS REPORT")
        print("=" * 80)
        print(f"  SNR Score:          {result.snr_score:.4f} (Target: >0.95)")
        print(
            f"  Ihsān Check:        {'✅ PASSED' if result.ihsan_check else '⚠️ WARNING'}"
        )
        ...
        if result.synergies:
            ...
            for i, syn in enumerate(result.synergies[:3], 1):
                print(
                    f"  {i}. [{syn.synergy_type.name}] {syn.source_domain} <-> {syn.target_domain} (Strength: {syn.strength:.2f})"
                )
