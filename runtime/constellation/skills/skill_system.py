...
class Skill:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "inputs": [
                {"name": i.name, "type": i.type_hint, "required": i.required}
...
class SubAgentManager:
    ...
    def get_by_parent(self, parent_agent: str) -> list[SubAgent]:
        ...
        return [
            sa for sa in self._sub_agents.values() if sa.parent_agent == parent_agent
        ]
...
class SkillChainStep:
    ...
    )  # step_input -> chain_input or prev_output
        ...
        for i, step in enumerate(self.steps):
            ...
            for step_key, source in step.input_mapping.items():
                if source.startswith("input."):
                    ...
                    step_inputs[step_key] = (
                        prev_output.get(output_key) if output_key else prev_output
                    )
                else:
