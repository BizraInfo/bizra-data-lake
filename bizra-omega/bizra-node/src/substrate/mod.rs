...
impl ResourceManifest {
    ...
    pub fn empty() -> Self {
        Self {
            hardware: HardwareManifest {
                cpu_name: String::new(), cpu_cores: 0, cpu_threads: 0,
                ram_total_gb: 0.0, ram_available_gb: 0.0,
                gpus: Vec::new(), disks: Vec::new(),
            },
            models: Vec::new(),
    ...
    pub fn total_models(&self) -> usize { self.models.len() }

    pub fn summary(&self) -> String {
        let gpu_summary = if self.hardware.gpus.is_empty() {
            "no GPU".to_string()
        } else {
            self.hardware.gpus.iter()
...
fn discover_hardware() -> HardwareManifest {
    ...
    #[cfg(not(any(target_os = "windows", target_os = "linux", target_os = "android")))]
    { HardwareManifest {
        cpu_name: "unknown".into(), cpu_cores: 1, cpu_threads: 1,
        ram_total_gb: 0.0, ram_available_gb: 0.0,
        gpus: Vec::new(), disks: Vec::new(),
    }}
}
...
fn discover_huggingface_models() -> Vec<LocalModel> {
    ...
    if !cache.exists() { return Vec::new(); }
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        let model_name = dir_name.strip_prefix("models--").unwrap_or(&dir_name).replace("--", "/");
        ...
        if found {
            models.push(LocalModel {
                name: model_name, size_bytes: total_size, quantization: String::new(),
                runtime: ModelRuntime::HuggingFace, path: dir, parameter_count: String::new(),
            });
        }
    }
    models
...
fn walk_inner(dir: &std::path::Path, out: &mut Vec<PathBuf>, depth: u32) -> std::io::Result<()> {
    ...
    for entry in std::fs::read_dir(dir)?.filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.is_dir() { let _ = walk_inner(&path, out, depth + 1); }
        else { out.push(path); }
    }
    Ok(())
}

pub fn scan_gguf_recursive(dir: &std::path::Path, runtime: ModelRuntime, out: &mut Vec<LocalModel>) {
    let Ok(files) = walk_files_recursive(dir) else { return };
    for path in files {
        let ext = path.extension().and_then(|e| e.to_str()).unwrap_or("");
        if ext != "gguf" { continue; }
        let stem = path.file_stem().and_then(|s| s.to_str()).unwrap_or("");
        if stem.starts_with("mmproj") { continue; }
        let size = path.metadata().map(|m| m.len()).unwrap_or(0);
        if size < 1_000_000 { continue; }
        let name = path.parent().and_then(|p| p.file_name()).and_then(|n| n.to_str()).unwrap_or(stem).to_string();
        out.push(LocalModel {
            name, size_bytes: size, quantization: extract_quantization(stem),
            runtime, path: path.clone(), parameter_count: String::new(),
        });
    }
}

pub fn extract_quantization(stem: &str) -> String {
    let quants = ["Q2_K","Q3_K_S","Q3_K_M","Q3_K_L","Q4_0","Q4_1","Q4_K_S","Q4_K_M",
        "Q5_0","Q5_1","Q5_K_S","Q5_K_M","Q6_K","Q8_0","F16","F32",
        "IQ1_S","IQ2_S","IQ2_XS","IQ3_S","IQ3_XS","IQ4_NL"];
    let upper = stem.to_uppercase();
    for q in &quants { if upper.contains(q) { return q.to_string(); } }
    String::new()
}
...
mod tests {
    ...
    fn extract_quant_from_filenames() {
        assert_eq!(extract_quantization("Qwen2.5-14B_Uncensored_Instruct-Q4_K_S"), "Q4_K_S");
        assert_eq!(extract_quantization("DeepSeek-R1-0528-Qwen3-8B-Q8_0"), "Q8_0");
