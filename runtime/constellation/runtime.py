...
class ConstellationRuntime:
    ...
    def hook_registry(self) -> HookRegistry:
        ...
        if self._hook_registry is None:
            raise RuntimeError(
                "Hook registry not initialized. Call initialize() first."
            )
    ...
    def trigger_engine(self) -> TriggerEngine:
        ...
        if self._trigger_engine is None:
            raise RuntimeError(
                "Trigger engine not initialized. Call initialize() first."
            )
    ...
    def command_registry(self) -> CommandRegistry:
        ...
        if self._command_registry is None:
            raise RuntimeError(
                "Command registry not initialized. Call initialize() first."
            )
    ...
    def skill_registry(self) -> SkillRegistry:
        ...
        if self._skill_registry is None:
            raise RuntimeError(
                "Skill registry not initialized. Call initialize() first."
            )
    ...
    def _register_default_triggers(self) -> None:
        ...
        trigger("medical-query-trigger").of_type(TriggerType.PATTERN).when_pattern(
            r"(health|medical|symptom|diagnos|treat)", is_regex=True
        ).invoke_agent("ibn-sina").with_cooldown(60).described_as(
            "Invoke Ibn Sina for medical queries"
        ).build()

        # Trigger mathematician for calculation queries
        trigger("math-query-trigger").of_type(TriggerType.PATTERN).when_pattern(
            r"(calculat|equation|algorithm|formula)", is_regex=True
        ).invoke_agent("al-khwarizmi").with_cooldown(60).described_as(
            "Invoke Al-Khwarizmi for mathematical queries"
        ).build()
    ...
    def get_memory_interface(self, agent_slug: str) -> AgentMemoryInterface:
        ...
        if agent_slug not in self._agent_memory:
            self._agent_memory[agent_slug] = self._memory_manager.get_interface(
                agent_slug
            )
...
async def initialize_constellation(
