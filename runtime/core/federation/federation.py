...
from core.federation.consensus import PatternConsensus
...
class FederationConfig:
    ...
    def from_env(cls) -> "FederationConfig":
        ...
        return cls(
            ...
            patterns_dir=Path(
                os.getenv("BIZRA_FED_PATTERNS", "data/federation/patterns")
            ),
...
class PatternFederation:
    ...
    def _setup_callbacks(self):
        ...
        def get_pattern(pattern_id: str) -> Optional[PatternEnvelope]:
            return self.local_patterns.get(pattern_id) or self.federated_patterns.get(
                pattern_id
            )
    ...
    async def elevate_pattern(
        ...
        logger.info(
            f"📤 Elevated pattern {pattern_id[:16]} (impact={impact_score:.2f})"
        )
    ...
    async def _handle_received_pattern(self, envelope: PatternEnvelope):
        ...
        logger.info(
            f"📥 Received pattern {pattern_id[:16]} from {envelope.metadata.origin_node_id[:16]}"
        )
    ...
    def get_all_patterns(self) -> List[PatternEnvelope]:
        ...
        return list(self.local_patterns.values()) + list(
            self.federated_patterns.values()
        )
