...
ck(
    "Genesis object",
    lambda: f"SEED={gen.genesis_wallet_state.seed_balance} Zakat={gen.genesis_wallet_state.zakat_due_ratio}",
)
...
from core.bus.subscribers import EventBus as SyncBus
from core.bus.subscribers import EventType, wire_all_subscribers
...
evt1 = bus.publish(
    EventType.ACTION_RECEIPT,
    {
        ...
        "result_summary": "sovereignty proven",
        "chain_hash": "test",
    },
)
ck(
    "ACTION_RECEIPT dispatched",
    lambda: f"hash={evt1.event_hash[:16]} height={bus.chain_height}",
)
...
for i, act in enumerate(
    ["genesis", "heartbeat", "mission", "reflex_compile", "token_mint", "audit", "seal"]
):
...
tampered = canonical_bytes(
    {"action": "heartbeat", "prev": "TAMPERED", "ihsan": 0.98, "tick": 1, "ts": 0}
)
...
c = router.route(
    "Redesign the ReflexCache to be thread-safe for concurrent UAB, while maintaining lock-free reads"
)
...
ck(
    "Bus chain grew",
    lambda: f"before={chain_before_boot} after={chain_after} delta={chain_after - chain_before_boot}",
)
...
from core.token.bloom import SEED_MINT_FLOOR, TOKEN_ZAKAT_RATE, compute_gini
...
ck(
    "GoT snapshot",
    lambda: GoTNodeSnapshot(node_id="root", content="test", score=0.94).score,
)
