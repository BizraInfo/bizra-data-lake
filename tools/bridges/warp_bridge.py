...
class WARPBridge:
    ...
    def _initialize(self) -> bool:
        ...
        try:
            ...
            if not index_path.exists():
                logger.info(
                    f"Index {self.index_name} not found. Build with build_index()."
                )
                self.status = WARPStatus.OFFLINE
                return False
    ...
    def build_index(
        self, collection_path: Path = None, overwrite: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"✓ Index built in {elapsed:.2f}s: {num_docs} docs, {index_size / 1e6:.2f} MB"
            )
            self.status = WARPStatus.READY
    ...
    def search(self, query: str, k: int = 10, nprobe: int = None) -> WARPQueryResponse:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stats": (
                {
                    "num_documents": (
                        self.index_stats.num_documents if self.index_stats else 0
                    ),
                    "index_size_mb": (
                        (self.index_stats.index_size_bytes / 1e6)
                        if self.index_stats
                        else 0
                    ),
                    "last_query": (
                        self.index_stats.last_query if self.index_stats else None
                    ),
                }
                if self.index_stats
                else None
...
def main():
    ...
    build_parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing"
    )
