...
class PatternMetadata(BaseModel):
    ...
    # Impact metrics
    repetition_count: int = Field(..., ge=MIN_REPETITIONS, description="Times pattern observed")
    ...
    ihsan_score: float = Field(..., ge=MIN_IHSAN_SCORE, le=1.0, description="Ihsān compliance")
    
    # Network metrics (updated by federation)
    adoption_count: int = Field(0, ge=0, description="Nodes that adopted pattern")
    global_success_rate: float = Field(0.0, ge=0.0, le=1.0, description="Network-wide success")
    
    # Provenance
    parent_patterns: List[str] = Field(default_factory=list, description="Derived from patterns")
...
class PatternPayload(BaseModel):
    ...
    # The actual pattern
    trigger_sequence: List[str] = Field(..., description="Sequence that triggers pattern")
    ...
    token_savings_percent: float = Field(0.0, ge=0.0, le=100.0, description="Token reduction")
    snr_improvement: float = Field(0.0, ge=0.0, le=1.0, description="SNR boost")
    
    # Implementation hints
    probe_scores: Dict[str, float] = Field(default_factory=dict, description="Expected probe scores")
    context_requirements: List[str] = Field(default_factory=list, description="Required context")
    
    # Evidence
    sample_inputs: List[str] = Field(default_factory=list, max_length=5, description="Example inputs")
    sample_outputs: List[str] = Field(default_factory=list, max_length=5, description="Example outputs")
...
class PatternEnvelope(BaseModel):
    ...
    def verify(self) -> Tuple[bool, str]:
        ...
        if self.metadata.ihsan_score < MIN_IHSAN_SCORE:
            return False, f"Ihsān score {self.metadata.ihsan_score} below minimum {MIN_IHSAN_SCORE}"
