    ...
    *,
    ...
    for term, baseline_spec in baseline_terms.items():
        if term not in current_terms:
            failures.append(
                f"append-only violation: removed term '{term}' ({lexicon_path})"
            )
            continue
        if current_terms.get(term) != baseline_spec:
            failures.append(
                f"append-only violation: modified term '{term}' ({lexicon_path})"
            )
    ...
    if isinstance(current_truth, dict) and isinstance(baseline_truth, dict):
        if current_truth != baseline_truth:
            failures.append(
                f"append-only violation: modified truth_labels ({lexicon_path})"
            )
    ...
    added_terms = sorted(
        set(current_terms.keys()) - set(baseline_terms.keys()), key=str.casefold
    )
...
def main() -> int:
    ...
    if lexicon_schema_ref != "schemas/lexicon_v1.schema.json":
        failures.append(
            f"lexicon schema must be schemas/lexicon_v1.schema.json ({lexicon_path})"
        )
    ...
    if not isinstance(truth_labels, dict):
        ...
        if missing:
            failures.append(
                f"lexicon truth_labels missing: {sorted(missing)} ({lexicon_path})"
            )
        for key, value in truth_labels.items():
            if str(key).upper() not in ALLOWED_TRUTH:
                failures.append(
                    f"lexicon truth_labels contains unknown key '{key}' ({lexicon_path})"
                )
            if not isinstance(value, str) or not value.strip():
                failures.append(
                    f"lexicon truth_labels['{key}'] must be a non-empty string ({lexicon_path})"
                )
    ...
    if not isinstance(terms, dict) or not terms:
        ...
        if keys != expected:
            failures.append(
                f"lexicon terms keys must be sorted (case-insensitive) ({lexicon_path})"
            )

        for term_name, term_spec in terms.items():
            if not isinstance(term_spec, dict):
                failures.append(
                    f"term '{term_name}' must be a mapping ({lexicon_path})"
                )
                continue
        ...
        if not isinstance(lexicon_receipt_term, dict):
            failures.append(
                f"term 'LexiconReceipt' must exist and be a mapping ({lexicon_path})"
            )
        else:
