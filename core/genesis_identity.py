    ...
    try:
        ...
        if result.get("verified"):
            if result.get("permissive_mode"):
                logger.warning("⚠ Running in PERMISSIVE MODE - No hardware covenant in genesis")
            else:
            ...
            logger.error("✗ Hardware verification FAILED - This is NOT the genesis node")
            logger.error(f"  Expected: {result.get('expected_fingerprint', 'N/A')[:16]}...")
