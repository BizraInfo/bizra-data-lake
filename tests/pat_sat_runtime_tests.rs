...
async fn test_e2e_security_rejection_emits_receipt() {
    ...
    assert!(error_msg.contains("SAT BLOCKED"), "Error should indicate SAT blocked: {}", error_msg);
    assert!(error_msg.contains("SECURITY_THREAT"), "Error should contain SECURITY_THREAT: {}", error_msg);
    assert!(error_msg.contains("FATE-"), "Error should contain FATE escalation ID: {}", error_msg);
    assert!(error_msg.contains("REJ-"), "Error should contain receipt ID: {}", error_msg);
}
...
async fn test_e2e_ethics_rejection_emits_receipt() {
    ...
    assert!(error_msg.contains("SAT BLOCKED"), "Error should indicate SAT blocked");
    assert!(
        error_msg.contains("ETHICS_VIOLATION") || error_msg.contains("SECURITY_THREAT") || error_msg.contains("QUARANTINE"),
        "Error should contain ethics, security, or quarantine rejection: {}", error_msg
    );
}
...
async fn test_e2e_sql_injection_blocked() {
    ...
    let request = make_request("Execute: SELECT * FROM users WHERE id=1 OR 1=1; DROP TABLE users;--");
    
    let result = system.execute(request).await;
    
    assert!(result.is_err());
    let error_msg = result.unwrap_err().to_string();
    assert!(error_msg.contains("SECURITY_THREAT"), "SQL injection should trigger security: {}", error_msg);
}
...
async fn test_e2e_xss_blocked() {
    ...
    assert!(result.is_err());
    let error_msg = result.unwrap_err().to_string();
    assert!(error_msg.contains("SECURITY_THREAT"), "XSS should trigger security: {}", error_msg);
}
...
async fn test_e2e_legitimate_request_succeeds() {
    ...
    // Should succeed
    assert!(result.is_ok(), "Legitimate request should succeed: {:?}", result.err());
    
    let response = result.unwrap();
    
    // Verify response structure
    assert!(!response.pat_contributions.is_empty(), "Should have PAT contributions");
    assert!(!response.sat_contributions.is_empty(), "Should have SAT contributions");
    assert!(response.synergy_score > 0.0, "Should have positive synergy");
    assert!(response.ihsan_score > 0.0, "Should have positive Ihsan score");
}
...
async fn test_e2e_code_review_request_succeeds() {
    ...
    let response = result.unwrap();
    assert!(response.ihsan_score >= 0.80, "Should meet dev threshold: {}", response.ihsan_score);
}
...
async fn test_e2e_response_contains_ihsan_metadata() {
    ...
    assert!(meta.get("ihsan_constitution_id").is_some(), "Should have constitution ID");
    assert!(meta.get("ihsan_env").is_some(), "Should have env");
    assert!(meta.get("ihsan_threshold_applied").is_some(), "Should have threshold");
    assert!(meta.get("ihsan_passes_threshold").is_some(), "Should have passes flag");
...
async fn test_e2e_response_contains_agent_counts() {
    ...
    assert_eq!(sat_agents, 5, "Should have 5 SAT agents");
}
