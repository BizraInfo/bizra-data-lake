...
class VectorLayer:
    ...
    def _initialize(self):
        ...
        try:
            ...
            log.info(
                f"   ✓ Model loaded: all-MiniLM-L6-v2 ({ApexConfig.EMBEDDING_DIM}-dim)"
            )
        except ImportError:
            log.warning(
                "   ⚠️ sentence-transformers not available, using mock embeddings"
            )
            self._model = None
    ...
    def embed(self, texts: List[str]) -> np.ndarray:
        ...
        if self._model is None:
            ...
            return np.random.randn(len(texts), ApexConfig.EMBEDDING_DIM).astype(
                np.float32
            )

        return self._model.encode(
            texts,
            batch_size=ApexConfig.BATCH_SIZE,
...
class GraphLayer:
    ...
    def get_neighbors(
        self, node_id: str, direction: str = "both", edge_types: Set[EdgeType] = None
    ) -> List[KnowledgeNode]:
...
class ReasoningLayer:
    ...
    def _gen_id(self) -> str:
        return hashlib.blake2b(
            f"{time.time()}{len(self.thoughts)}".encode(), digest_size=16
        ).hexdigest()[:12]
    ...
    def merge(self, node_ids: List[str], merged_thought: str) -> ThoughtNode:
        ...
        confidences = [
            self.thoughts[n].confidence for n in node_ids if n in self.thoughts
        ]
        merged_conf = (
            math.prod(confidences) ** (1 / len(confidences)) if confidences else 0.5
        )
        ...
        while current:
            ...
            if node.children:
                ...
                best_child = max(
                    node.children,
                    key=lambda c: (
                        self.thoughts[c].confidence if c in self.thoughts else 0
                    ),
                )
                current = best_child
            else:
    ...
    def discover_hub_patterns(self, min_degree: int = 5) -> List[DiscoveredPattern]:
        ...
        for nid, node in self.graph.nodes.items():
            ...
            if total_degree >= min_degree:
                ...
                connected.extend(
                    [e.source_id for e in self.graph.reverse_edges.get(nid, [])]
                )
            ...
            for n2 in node_ids[i + 1 :]:
                ...
                if len(overlap) >= min_support:
                    ...
                    pattern = DiscoveredPattern(
                        ...
                        edges=[(n1, o) for o in list(overlap)[:5]]
                        + [(n2, o) for o in list(overlap)[:5]],
                        support=len(overlap),
                        confidence=len(overlap)
                        / max(len(neighbor_sets[n1] | neighbor_sets[n2]), 1),
...
class ApexUnifiedEngine:
    ...
    def index_node0_inventory(self) -> int:
        ...
        root = KnowledgeNode(
            ...
            metadata={
                ...
                "gpu": (
                    inv.get("hardware", {}).get("gpu", [{}])[0].get("name")
                    if inv.get("hardware", {}).get("gpu")
                    else None
                ),
            },
        )
        ...
        if hw.get("cpu"):
            ...
            self.graph.add_edge(
                KnowledgeEdge("node0_genesis", "hw_cpu", EdgeType.CONTAINS)
            )
            count += 1
        ...
        for prog in inv.get("software", {}).get("programs", [])[:100]:  # Top 100
            ...
            self.graph.add_node(
                KnowledgeNode(id=pid, name=prog, type=NodeType.SOFTWARE)
            )
        ...
        if dl:
            ...
            self.graph.add_edge(
                KnowledgeEdge("node0_genesis", "folder_downloads", EdgeType.CONTAINS)
            )
            count += 1
            ...
            for fname in dl.get("folder_names", []):
                ...
                self.graph.add_edge(
                    KnowledgeEdge("folder_downloads", fid, EdgeType.CONTAINS)
                )
                if is_biz:
                    self.graph.add_edge(
                        KnowledgeEdge(fid, "node0_genesis", EdgeType.PART_OF)
                    )
                count += 1
    ...
    def _calculate_snr_scores(self):
        ...
        for nid, node in self.graph.nodes.items():
            ...
            # Combined SNR
            node.snr_score = round(
                connectivity * 0.4 + meta_signal * 0.3 + desc_signal * 0.3, 4
            )
    ...
    def query(
        ...
        # Calculate final SNR
        avg_centrality = sum(n.centrality for n in result_nodes) / max(
            len(result_nodes), 1
        )
        snr = self.reasoning.calculate_snr(
            result_nodes, terms, len(paths), avg_centrality
        )
    ...
    def save(self):
        ...
        graph_data = {
            ...
            "edges": [
                e.to_dict() for edges in self.graph.edges.values() for e in edges
            ],
        }
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            log.info(
                f"📂 Loaded {len(self.graph.nodes)} nodes, {len(self.pattern.patterns)} patterns"
            )
            return True
...
╚═══════════════════════════════════════════════════════════════════════════════════════╝
    ...
    if cmd == "build":
        ...
        if apex.load():
            ...
            bizra = [
                n
                for n in apex.graph.nodes.values()
        ...
        if apex.load():
            ...
            for p in apex.pattern.patterns[:25]:
                ...
                print(
                    f"  [{p.pattern_type:15}] {p.description[:45]:45} {conf_bar} ({p.support})"
                )
