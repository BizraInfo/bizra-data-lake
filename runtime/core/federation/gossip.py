...
class PeerInfo:
    ...
    def is_alive(self) -> bool:
        ...
        return (
            ...
            and time.time() - self.last_seen < PEER_TIMEOUT_SEC
        )
...
class GossipProtocol:
    ...
    async def _handle_pattern_announce(
        self, msg: GossipMessage
    ) -> Optional[GossipMessage]:
    ...
    async def _cleanup_loop(self):
        ...
        while self._running:
            try:
                ...
                stale_msgs = [
                    msg_id
                    for msg_id, ts in self.seen_message_times.items()
                    if now - ts > 300
                ]
                ...
                stale_requests = [
                    pid for pid, ts in self.pending_requests.items() if now - ts > 60
                ]
