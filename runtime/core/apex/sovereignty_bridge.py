...
from core.sovereignty.merkle_dag import VerificationResult
...
class SovereigntyBridge:
    ...
    async def verify_sovereignty(
        ...
        if add_evidence and self._merkle_dag_available and self._merkle_dag:
            try:
                ...
                node = self._merkle_dag.add_node(
                    data=evidence_data, metadata=verification_metadata
                )
        ...
        overall_passed = (
            constitution_passed and daughter_test_passed and evidence_chain_valid
        )
    ...
    async def add_evidence(
        ...
        # Add to DAG
        node = self._merkle_dag.add_node(
            data=full_evidence, parent_ids=parent_ids, metadata=metadata or {}
        )

        logger.debug(
            f"Evidence added: node_id={node.node_id[:8]}..., type={evidence_type}"
        )
        ...
        for _ in range(iterations):
            try:
                ...
                error_hash = hashlib.sha256(
                    (DOMAIN_PREFIX + f"error:{type(e).__name__}:{str(e)}").encode(
                        "utf-8"
                    )
                ).hexdigest()
        ...
        status = {
            ...
            "components": {
                ...
                "constitution": {
                    ...
                    "verification_count": (
                        len(self._constitution.verification_history)
                ...
                "merkle_dag": {
                    ...
                    "node_count": (
                        len(self._merkle_dag.nodes) if self._merkle_dag else 0
                    ),
                },
            },
    ...
    def _create_fallback_receipt(
        ...
        receipt_data = {
            ...
            "principles_passed": sum(
                1 for s in scores.values() if s >= self.ihsan_threshold
            ),
            "violations": [],
            "metadata": metadata or {},
        }
