...
License Gate Chain: SCHEMA -> SNR -> IHSAN -> LICENSE

This ordering is intentional and differs from the PCI Gate ordering (IHSAN before SNR).

Why SNR before IHSAN in License Gates?
--------------------------------------
License gates validate inference OUTPUT from local models that have already
passed the Constitution Challenge. The content source is authenticated and
trusted - we are checking the QUALITY of output, not defending against
adversarial input.

1. FILTER NOISE FIRST: Low SNR indicates garbled, incoherent, or degenerate
   model output (repetition loops, encoding errors, context window overflow).
   There is no point evaluating Ihsan (ethical quality) on noise - it would
   produce meaningless scores and waste computation.

2. MEANINGFUL QUALITY ASSESSMENT: Ihsan scoring requires coherent text to
   evaluate ethical alignment. By filtering low-SNR garbage first, we ensure
   Ihsan evaluation runs on content that CAN be meaningfully assessed.

3. COMPUTATIONAL EFFICIENCY: SNR is typically faster to compute (statistical
   analysis) than Ihsan scoring (may involve semantic analysis). Rejecting
   obvious failures early reduces unnecessary processing.

4. TRUSTED SOURCE CONTEXT: Unlike PCI envelopes from the federation network,
   license gate input comes from models we control. There is no adversarial
   crafting of high-SNR malicious content - the model either produces good
   output or it does not.

Contrast with PCI Gates (core/pci/gates.py):
--------------------------------------------
PCI gates check IHSAN before SNR because they process inter-node messages
from potentially untrusted peers. In adversarial contexts, ethical violations
must be caught first - a high-SNR malicious message is MORE dangerous than
a noisy benign one. PCI uses fail-fast on ethical violations for security.

See also: core/pci/gates.py for the PCI Gate rationale.
"""
