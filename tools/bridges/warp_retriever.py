...
class XTREncoder:
    ...
    def __init__(self, model_name: str = XTR_MODEL_NAME, device: Optional[str] = None):
        ...
        dense_path = hf_hub_download(
            repo_id=model_name, filename="2_Dense/pytorch_model.bin"
        )

        self.linear = torch.nn.Linear(768, 128, bias=False).to(self.device)
        self.linear.load_state_dict(
            torch.load(dense_path, map_location=self.device, weights_only=True)
        )
    ...
    def encode_documents(
        self, documents: List[str], max_length: int = 512, batch_size: int = 32
...
class WARPScorer:
    ...
    def score_documents(
        ...
        if total_doc_tokens <= self.max_doc_tokens_per_chunk:
            return self._score_documents_fast(
                query_embeddings, doc_embeddings, doc_lengths
            )

        # For large candidate sets, use chunked processing
        return self._score_documents_chunked(
            query_embeddings, doc_embeddings, doc_lengths
        )
...
class WARPRetriever:
    ...
    def __init__(
        ...
        log.info(
            f"WARPRetriever initialized | Backend: {backend.name} | Docs: {len(self.metadata)}"
        )
    ...
    def search(
        self, query: str, top_k: int = 5, return_content: bool = True
    ) -> RetrievalResult:
        """
    ...
    def _search_xtr_warp(self, query: str) -> np.ndarray:
        ...
        log.info(
            "XTR-WARP requested but corpus is MiniLM-indexed. Using hybrid scoring."
        )
        return self._search_minilm(query)
...
def create_retriever(
    backend: str = "auto", index_path: Optional[str] = None
) -> WARPRetriever:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", "--backend", default="auto", choices=["auto", "xtr", "minilm", "hybrid"]
    )
