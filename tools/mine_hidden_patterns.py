...
def analyze_thought_flow(df):
    ...
    knowledge_df = df[
        ...
        | (
            df["path"].str.contains(
                "conversation|chat|logs|journal|diary|idea|plan", case=False
            )
        )
    ].copy()
    ...
    knowledge_df["parent_dir"] = knowledge_df["path"].apply(
...
def extract_golden_gems(knowledge_df):
    ...
    for _, row in top_gems.iterrows():
        print(
            f"   {row['gem_score']:<6} | {row['modified'].strftime('%Y-%m-%d')}   | {row['name']}"
        )
