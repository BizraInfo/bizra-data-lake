...
npx claude-flow github release-plan \
  --commits "$COMMITS" \
...
npx claude-flow github release-version \
  --strategy "semantic" \
  --analyze-changes \
  --check-breaking \
  --update-files
...
CHANGELOG=$(gh api repos/:owner/:repo/compare/${LAST_TAG}...HEAD \
  ...
  npx claude-flow github generate-changelog)
...
CHANGELOG=$(npx claude-flow github changelog \
  --prs "$PRS" \
  --commits "$COMMITS" \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx claude-flow github release-init \
            --tag $RELEASE_TAG \
      ...
      - name: Generate Release Assets
        run: |
          ...
          CHANGELOG=$(npx claude-flow github release-changelog \
            --format markdown)
          ...
          npx claude-flow github release-assets \
            --changelog \
            --binaries \
            --documentation
      ...
      - name: Publish Release
        run: |
          ...
          npx claude-flow github release-publish \
            --platforms all
...
npx claude-flow github cd-pipeline \
...
npx claude-flow github release-docs \
  --api-changes \
  --migration-guide \
  --example-updates \
...
```bash
# NPM package release
npx claude-flow github npm-release \
  --version patch \
  --test-all \
  --publish-beta \
...
npx claude-flow github docker-release \
...
npx claude-flow github mobile-release \
