...
class SapePlanRequest(BaseModel):
    domain: str = Field(
        ...,
        min_length=1,
        description="What area the work is about (e.g., 'BIZRA kernel runtime').",
    )
    ...
    evidence_limit: int = Field(
        8, ge=1, le=25, description="Max evidence artifacts pulled from graph."
    )
...
def compile_sape_plan(
    req: SapePlanRequest, *, evidence: Optional[List[Dict[str, Any]]] = None
) -> CompiledSapePlan:
...
def format_gold_mine_evidence(evidence: List[Dict[str, Any]]) -> str:
    ...
    for item in evidence:
        hop_str = (
            f" (hop={item['hop_distance']})" if item.get("hop_distance", 0) > 0 else ""
        )
