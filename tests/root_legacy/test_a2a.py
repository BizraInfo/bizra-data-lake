...
def test_capability_creation():
    ...
    passed += assert_true(
        cap.type == CapabilityType.CODE_EXECUTION, "Type set correctly"
    )
...
def test_agent_card():
    ...
    passed += assert_true(
        card.has_capability("code.python.execute"), "Has execute capability"
    )
    passed += assert_true(
        not card.has_capability("unknown"), "Does not have unknown capability"
    )
...
def test_task_card():
    ...
    passed = 0
    passed += assert_true(
        task.status == TaskStatus.PENDING, "Initial status is PENDING"
    )
    ...
    passed += assert_true(
        task.status == TaskStatus.IN_PROGRESS, "Status is IN_PROGRESS"
    )
...
def test_message_signing():
    ...
    msg = engine.create_message(MessageType.PING, {"test": "data"})
...
async def test_task_delegation():
    ...
    worker = create_a2a_engine(
        ...
        capabilities=[
            {"name": "code.execute", "type": "code_execution", "description": "Execute"}
        ],
    ...
    task = orchestrator.create_task(
        capability="code.execute", prompt="Run my code", priority=8
    )
    ...
    passed += assert_true(
        msg.message_type == MessageType.TASK_REQUEST, "Message type is TASK_REQUEST"
    )
    ...
    passed += assert_true(
        response.message_type == MessageType.TASK_ACCEPT, "Worker accepted task"
    )
...
async def test_task_manager():
    ...
    passed += assert_true(
        manager.queue[0].task.priority == 10, "Highest priority first"
    )
...
async def test_multi_agent_communication():
    ...
    await asyncio.sleep(0.1)
    passed += assert_true(
        len(messages["orchestrator"]) == 1, "Orchestrator received announcement"
    )
