...
class ModelConfig:
    ...
    MODEL_MAP: Dict[ModelTier, str] = field(default_factory=lambda: {
...
class AgentDefinition:
    ...
    def __post_init__(self):
        self.id = f"agent_{self.name}_{hashlib.sha256(self.prompt.encode()).hexdigest()[:8]}"
...
class FilterOp(PipelineOp[List[T]]):
    ...
    async def execute(self, items: List[T], agent: Agent) -> List[T]:
        ...
        for item in items:
            ...
            if any(word in response.lower() for word in ["yes", "true", "feasible", "valid"]):
                result.append(item)
        return result
...
class Pipeline(Generic[T]):
    ...
    async def execute(self) -> Any:
        ...
        for op in self.operations:
            ...
            logger.info(f"Pipeline step {type(op).__name__}: {len(result) if isinstance(result, list) else 1} items")
        return result
...
class WorkflowEngine:
    ...
    def define_agent(self, definition: AgentDefinition) -> Agent:
        ...
        logger.info(f"🎯 Defined agent: {definition.name} (model={definition.model.value})")
        return agent
    ...
    def pipeline(self, data: Any, agent_name: str) -> Pipeline:
        ...
        if not agent:
            raise ValueError(f"Agent '{agent_name}' not found. Define it first with define_agent()")
        return Pipeline(data, agent)
    
    async def run_session(self, agent_name: str, prompt: str, context: Optional[List[str]] = None) -> str:
        """
...
class SnrOptimizedWorkflow:
    ...
    async def research_report(
        ...
        *,
        ...
        """
        # Define agents
        researcher = self.engine.define_agent(AgentDefinition(
            name="researcher",
        ...
        formatter = self.engine.define_agent(AgentDefinition(
            name="formatter",
        ...
        analysis = await researcher.session(
            f"Analyze the findings about {topic}",
            context=[research]
        )
        report = await formatter.session(
            f"Write a {depth}-level report on {topic}",
    ...
    async def idea_tournament(
        ...
        generator = self.engine.define_agent(AgentDefinition(
            name="generator",
            model=ModelTier.OPUS,
            persist=PersistenceScope.EXECUTION,
        ...
        evaluator = self.engine.define_agent(AgentDefinition(
            name="evaluator",
        ...
        result = await (
            pipeline
            .filter("Is this idea technically feasible and valuable? Answer yes/no.")
            .map("Expand this idea into a detailed one-page pitch.")
            .reduce("Compare these two pitches. Which is stronger? Return only the stronger one.")
            .execute()
        )
