...
class A2AEngine:
    ...
    def register_agent(self, card: AgentCard) -> bool:
        ...
        if card.ihsan_score < IHSAN_MINIMUM:
            print(f"❌ Rejected agent {card.agent_id}: Ihsān {card.ihsan_score} < {IHSAN_MINIMUM}")
            return False
        ...
        print(f"✅ Registered agent: {card.agent_id} ({len(card.capabilities)} capabilities)")
        return True
