...
class LocalLLMGateway:
    ...
    def _discover_models(self):
        ...
        if self.available_models:
            ...
            print(
                "   ⚠️ No local LLM available. Operating in OFFLINE mode (retrieval only)."
            )
    ...
    def _select_best_model(self):
        ...
        print(
            f"   🎯 Selected Model: {best.id} ({best.size_label}) via {best.provider.value}"
        )
    ...
    def generate(
        ...
        if target_provider == LLMProvider.LM_STUDIO:
            return self._generate_lm_studio(
                prompt, system, target_model, max_tokens, temperature
            )
    ...
    def generate_with_context(
        self, query: str, context: str, system: str = None, model: str = None
    ) -> LLMResponse:
        """
