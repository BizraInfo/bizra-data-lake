...
class UserModel:
    ...
    def __init__(self):
        ...
        self.active_projects = []           # [{"name": "...", "last_active": ..., "missions": 5}]
    ...
    def to_dict(self):
        return {
            "name": self.name, "node_id": self.node_id, "created_at": self.created_at,
            "preferred_agent": self.preferred_agent, "preferred_domains": self.preferred_domains,
            "active_hours": self.active_hours, "avg_mission_length": self.avg_mission_length,
            ...
            "streak_record": self.streak_record, "current_streak": self.current_streak,
            "avg_ihsan": self.avg_ihsan, "ihsan_trend": self.ihsan_trend,
            "active_projects": self.active_projects,
            "verbosity": self.verbosity, "prefers_explanations": self.prefers_explanations,
            "greeting_name": self.greeting_name,
            "tier": self.tier, "days_to_next_tier": self.days_to_next_tier,
            "strongest_skill": self.strongest_skill, "weakest_skill": self.weakest_skill,
    ...
    def from_dict(cls, d):
        m = cls()
        for k, v in d.items():
            if k == "vocabulary": m.vocabulary = Counter(v)
            elif k == "missions_by_agent": m.missions_by_agent = Counter(v)
            elif k == "missions_by_domain": m.missions_by_domain = Counter(v)
...
class EpisodicMemory:
    ...
    def active_projects_from_tasks(self):
        ...
        tasks = [e.get("task", "") for e in self.events if e.get("t") == "mission"][-100:]
        if not tasks: return []
        ...
        # Remove common words
        stop = {"that","this","with","from","your","have","will","been","what","when",
                "about","which","their","would","could","should","into","also","each",
                "make","like","just","over","such","than","them","very","some","only"}
        for w in stop: word_freq.pop(w, None)
...
class ProceduralMemory:
    ...
    def near_compilation(self):
        ...
        return [r for r in self.cache if 3 <= r.get("cnt", 0) < 5 and not r.get("compiled")]

    def top_reflexes(self, n=5):
        return sorted([r for r in self.cache if r.get("compiled")],
                      key=lambda r: r.get("cnt", 0), reverse=True)[:n]
...
class LivingMemory:
    ...
    def load(self):
        ...
        if node_path.exists():
            ...
            self.user.tier = ["Novice","Apprentice","Adept","Expert","Master","Grandmaster"][node.get("tier", 0)]
        ...
        if ledger_path.exists():
            events = [json.loads(l) for l in ledger_path.read_text().strip().split("\n") if l.strip()]
    ...
    def update_after_mission(self, task, agent_id, agent_domain, ihsan, seed):
        ...
        self.user.missions_by_agent[agent_id] = self.user.missions_by_agent.get(agent_id, 0) + 1
        self.user.preferred_agent = self.user.missions_by_agent.most_common(1)[0][0]

        # Update domain preference
        self.user.missions_by_domain[agent_domain] = self.user.missions_by_domain.get(agent_domain, 0) + 1
        ...
        self.user.avg_mission_length = (self.user.avg_mission_length * (n - 1) + word_count) / n
        ...
        if self.user.avg_mission_length < 8: self.user.verbosity = "terse"
        elif self.user.avg_mission_length > 20: self.user.verbosity = "detailed"
        else: self.user.verbosity = "balanced"
    ...
    def generate_morning_briefing(self):
        ...
        if hour < 6: tod = "Late night session"
        elif hour < 12: tod = "Good morning"
        elif hour < 17: tod = "Good afternoon"
        elif hour < 21: tod = "Good evening"
        else: tod = "Late evening"
        ...
        if hours_away is not None:
            if hours_away < 1:
                ...
                lines.append(f"It's been {int(hours_away)} hours. Here's what I remember.")
            elif hours_away < 168:
                days = int(hours_away / 24)
                lines.append(f"Welcome back — {days} day{'s' if days > 1 else ''} since our last session.")
            else:
                lines.append(f"It's been a while. {int(hours_away / 24)} days. Your node is intact.")

        # Active projects
        projects = self.user.active_projects
        if projects:
            top = projects[0]
            lines.append(f"Your main focus has been {top['name']} ({top['total_missions']} missions). Last: \"{top['last_task'][:50]}\".")

        # Last mission context
        recent = self.episodic.last_n_missions(1)
        if recent:
            last = recent[0]
            lines.append(f"Last mission: {last.get('an', 'agent')} completed \"{last.get('task', '')[:50]}\" — Ihsan {last.get('ih', 0):.3f}.")
        ...
        if near:
            ...
            lines.append(f"Pattern \"{r['p'][:40]}\" is {r.get('cnt',0)}/5 — {remaining} more to compile a reflex.")
        ...
        if self.user.ihsan_trend == "improving":
            ...
            lines.append("I've noticed your Ihsan trending down. Shall I suggest ways to improve?")

        # Streak
        if self.user.current_streak >= 7:
            lines.append(f"You're on a {self.user.current_streak}-mission streak. Personal best: {self.user.streak_record}.")
    ...
    def get_context_for_agent(self, agent_id):
        ...
        return {
            ...
            "recent_missions": [
                {"task": m.get("task", ""), "agent": m.get("a", ""), "ihsan": m.get("ih", 0)}
    ...
    def format_status(self):
        ...
        lines.append(f"  Streak:         {u.current_streak} (record: {u.streak_record})")
        lines.append(f"  Avg Ihsan:      {u.avg_ihsan:.4f} ({u.ihsan_trend})")
        lines.append(f"  Top agent:      {u.preferred_agent}")
        lines.append(f"  Top domains:    {', '.join(u.preferred_domains) if u.preferred_domains else 'learning...'}")
...
# ═══════════════════════════════════════
def demo():
    G = "\033[38;2;201;169;98m"; R = "\033[0m"; D = "\033[2m"
    BL = "\033[38;2;96;165;250m"; GR = "\033[38;2;52;211;153m"
    PU = "\033[38;2;167;139;250m"; B = "\033[1m"
