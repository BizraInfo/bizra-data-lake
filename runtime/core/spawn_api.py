...
class SpawnHandler(BaseHTTPRequestHandler):
    ...
    def _handle_spawn(self):
        ...
        try:
            ...
            self._send_json({"success": True, "agent": agent.to_dict()}, 201)

        except Exception as e:
            logger.error(f"Spawn error: {e}")
            self._send_json({"error": str(e), "error_type": type(e).__name__}, 500)

    def _handle_list_agents(self):
        """GET /v1/system/agents"""
        factory = get_factory()
        agents = factory.list_agents()
        self._send_json({"count": len(agents), "agents": [a.to_dict() for a in agents]})
    ...
    def _handle_terminate(self, agent_id: str):
        ...
        if factory.terminate(agent_id):
            self._send_json(
                {"success": True, "message": f"Agent {agent_id} terminated"}
            )
        else:
            self._send_json({"error": "Agent not found"}, 404)
