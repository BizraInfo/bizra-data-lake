...
class EmbeddingIndex:
    ...
    def _load_from_dataframe(self, df: pd.DataFrame):
        ...
        print(
            f"  ✅ Loaded {count} embeddings from DataFrame coverage: {count/len(df)*100:.1f}%"
        )
...
class DeduplicationEngine:
    ...
    def find_duplicates(
        self, doc_ids: List[str], threshold: float = DUPLICATE_THRESHOLD
    ) -> Dict[str, str]:
        """
        ...
        for doc_id in doc_ids:
            doc_chunks = (
                self.chunks_df[self.chunks_df["doc_id"] == doc_id]["chunk_text"]
                .head(5)
                .tolist()
            )
            if doc_chunks:
...
class SemanticGraphAnalyzer:
    ...
    def compute_semantic_degree(self) -> pd.DataFrame:
        ...
        out_degree = (
            self.semantic_edges.groupby("source").size().reset_index(name="out_degree")
        )
        in_degree = (
            self.semantic_edges.groupby("target").size().reset_index(name="in_degree")
        )
    ...
    def compute_pagerank(
        ...
        # Get unique nodes
        nodes = set(
            self.semantic_edges["source"].tolist()
...
class SemanticValidator:
    ...
    def compute_semantic_score(
        self, artifact_id: str, structural_degree: float
    ) -> Dict:
        """
...
def mine_golden_gems_v2():
        ...
        gems.append(
            {
                ...
                "validation": {"self_referential": False, "semantic_validated": True},
    ...
    if "kind" in nodes_df.columns:
        ...
        sem_with_kinds = semantic_analyzer.semantic_edges.merge(
            nodes_df[["id", "kind"]], left_on="source", right_on="id", how="left"
        ).rename(columns={"kind": "source_kind"})

        sem_with_kinds = sem_with_kinds.merge(
            nodes_df[["id", "kind"]], left_on="target", right_on="id", how="left"
        ).rename(columns={"kind": "target_kind"})

        # Cross-domain = different kinds
        cross_domain = sem_with_kinds[
            sem_with_kinds["source_kind"] != sem_with_kinds["target_kind"]
        ]
        bridge_counts = (
            cross_domain.groupby("source").size().sort_values(ascending=False)
        )
        ...
        for node_id, bridge_count in bridge_counts.head(10).items():
            ...
            node_info = nodes_df[nodes_df["id"] == node_id]
            label = (
                str(node_info.iloc[0]["label"])[:100]
    ...
    for doc_id in dedup_engine.canonical_docs:
        ...
        if doc_embedding is not None:
            ...
            if (
                not sem_degree.empty
                and sem_degree.iloc[0]["semantic_degree"] >= MIN_SEMANTIC_DEGREE
            ):
    ...
    coherent_docs = sorted(
        coherent_docs, key=lambda x: x["semantic_degree"], reverse=True
    )
    ...
    for doc in coherent_docs[:10]:
        ...
        label = (
            str(node_info.iloc[0]["label"])[:100] if not node_info.empty else "Unknown"
        )
    ...
    # Top gems
    sorted_gems = sorted(
        [g for g in gems[1:] if "score_semantic" in g],
        key=lambda x: x["score_semantic"],
        reverse=True,
    )
    print("\nTop 5 gems by semantic score:")
    for i, g in enumerate(sorted_gems[:5], 1):
        print(
            f"  {i}. [{g['type']}] {g['title'][:40]}... (score={g['score_semantic']:.4f})"
        )
