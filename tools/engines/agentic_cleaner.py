...
# Configure Logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s | %(levelname)s | CLEANER | %(message)s"
)
...
ALLOWED_BUILTINS: Set[str] = {
    ...
    "False",
    "None",
    "print",  # print for debugging only
}
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "clean_record":
            ...
            if len(node.args.args) != 1:
                raise CodeValidationError("clean_record must take exactly one argument")
            break
...
class AgenticCleaner:
    ...
    def process_file(self, file_path: str, output_dir: Optional[str] = None):
        ...
        try:
            ...
            exec(
                cleaning_code, execution_scope, execution_scope
            )  # nosec B102 — AST-validated + restricted builtins
            ...
            if not test_res or not isinstance(test_res, str):
                logger.warning(
                    "⚠️ Synthesized function failed sanity check. Using fallback."
                )
                clean_func = None
        ...
        if clean_func is None:
            ...
            exec(
                fallback_code, local_scope, local_scope
            )  # nosec B102 — hardcoded safe fallback
        ...
        with (
            open(input_path, "r", encoding="utf-8", errors="ignore") as infile,
        ...
        logger.info(
            f"✨ Cleaning Complete. {success_count} lines written, {error_count} errors. Output: {out_path}"
        )
