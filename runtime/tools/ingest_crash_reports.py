...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Safely ingest Windows crash artifacts (index + copy small files)."
    )
    ap.add_argument(
        "--roots",
        nargs="*",
        default=[],
        help="Roots to scan (defaults from env or C:\\temp C:\\tmp)",
    )
    ap.add_argument(
        "--out-dir",
        default="",
        help="Output directory (defaults to Data Lake indexed/windows_crash/<run_id>)",
    )
    ap.add_argument(
        "--copy-max-mb", type=int, default=50, help="Copy files up to this size (MB)"
    )
    ap.add_argument(
        "--extract-max-mb",
        type=int,
        default=5,
        help="Extract text members up to this size (MB)",
    )
    args = ap.parse_args()
    ...
    if not roots:
        ...
        for k in ("BIZRA_VAULT_CRASH_TEMP", "BIZRA_VAULT_CRASH_TMP"):
            ...
            if v:
                env_roots.append(v)
        roots = (
            [Path(r) for r in env_roots]
            if env_roots
            else [Path("C:\\temp"), Path("C:\\tmp")]
        )
    ...
    if args.out_dir:
        ...
        indexed = os.environ.get("BIZRA_DATALAKE_INDEXED") or os.environ.get(
