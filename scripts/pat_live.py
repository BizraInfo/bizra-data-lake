...
async def run_pat_team(task: str, token: str, agents: list = None):
    ...
    print("Model: DeepSeek R1 8B (reasoning)")
    ...
    for agent_id in agents:
        ...
        if result["success"]:
            ...
            print(
                f"   ✓ Complete ({result['latency_ms']:.0f}ms, {result.get('tokens', 0)} tokens)"
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "task",
        nargs="?",
        default="Analyze the best approach for building a sovereign AI system",
