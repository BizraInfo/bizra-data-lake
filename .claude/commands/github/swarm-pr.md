...
gh pr view 123 --json body,title,labels,files | npx claude-flow swarm create-from-pr

# Auto-spawn agents based on PR labels
gh pr view 123 --json labels | npx claude-flow swarm auto-spawn

# Create swarm with PR context
gh pr view 123 --json body,labels,author,assignees | \
  npx claude-flow swarm init --from-pr-data
```
      ...
      - name: Handle Swarm Command
        run: |
          if [[ "${{ github.event.comment.body }}" == /swarm* ]]; then
            npx claude-flow github handle-comment \
...
npx claude-flow github pr-topology --pr 123
```
...
npx claude-flow github pr-init 123 \
  --auto-agents \
  --pr-data "$PR_INFO" \
  --diff "$PR_DIFF" \
  --analyze-impact
...
PROGRESS=$(npx claude-flow github pr-progress 123 --format markdown)
...
# Run swarm review
REVIEW_RESULTS=$(npx claude-flow github pr-review 123 \
...
npx claude-flow github multi-pr \
  --prs "123,124,125" \
  --strategy "parallel" \
  --share-memory
```
...
npx claude-flow github pr-deps 123 \
  --spawn-agents \
  --resolve-conflicts
```
...
```bash
# Auto-fix PR issues
npx claude-flow github pr-fix 123 \
...
SWARM_STATUS=$(npx claude-flow github pr-status 123)
...
createServer((req, res) => {
  if (req.url === '/github-webhook') {
    ...
    if (event.action === 'opened' && event.pull_request) {
      execSync(`npx claude-flow github pr-init ${event.pull_request.number}`);
    }
...
npx claude-flow github pr-init 456 \
...
npx claude-flow github pr-init 789 \
  --topology mesh \
...
npx claude-flow github pr-init 321 \
  --topology ring \
...
npx claude-flow github pr-report 123 \
...
npx claude-flow github export-metrics \
  --pr 123 \
  --to-insights
```
