...
class ConstitutionalGate:
    ...
    def __init__(self, receipt_path: Optional[Path] = None):
        ...
        logger.info(f"Constitutional Gate initialized (cutoff: {GENESIS_CUTOFF_HOURS}h)")
    ...
    def verify_runtime_eligibility(
        ...
        if proof.age_hours > GENESIS_CUTOFF_HOURS:
            logger.error(f"Agent {agent_id} exceeds {GENESIS_CUTOFF_HOURS}h genesis cutoff")
            return False
    ...
    def promote_to_runtime(
        ...
        if agent_id in self.museum_agents:
            ...
            logger.info(f"Promoting {agent_id} from Museum (SNR: {museum_proof.snr_score})")
    ...
    def register_museum_agent(
        ...
        self._emit_receipt("registration", {
            "agent_id": agent_id,
            "tier": "museum",
            "snr_score": snr_score,
            "cutoff_at": (proof.created_at.timestamp() + GENESIS_CUTOFF_HOURS * 3600),
        })
    ...
    def list_runtime_agents(self) -> List[str]:
        ...
        return [
            agent_id for agent_id, proof in self.runtime_agents.items()
            if proof.age_hours <= GENESIS_CUTOFF_HOURS
        ]
    ...
    def list_museum_agents(self) -> List[str]:
        ...
        return [
            agent_id for agent_id, proof in self.museum_agents.items()
            if proof.age_hours <= GENESIS_CUTOFF_HOURS
        ]
    ...
    def enforce_genesis_cutoff(self) -> Dict[str, List[str]]:
        ...
        for agent_id, proof in list(self.runtime_agents.items()):
            if proof.age_hours > GENESIS_CUTOFF_HOURS:
                ...
                logger.warning(f"Agent {agent_id} archived from Runtime (age: {proof.age_hours:.1f}h)")

        # Check museum agents
        for agent_id, proof in list(self.museum_agents.items()):
            if proof.age_hours > GENESIS_CUTOFF_HOURS:
                archived["archived_from_museum"].append(agent_id)
                logger.warning(f"Agent {agent_id} archived from Museum (age: {proof.age_hours:.1f}h)")
    ...
    def _validate_z3_certificate(
        ...
        if expected_hash:
            ...
            if actual_hash != expected_hash:
                logger.error(f"Z3 certificate hash mismatch: {actual_hash[:16]}... != {expected_hash[:16]}...")
                return False
    ...
    def _emit_receipt(self, operation: str, data: Dict[str, Any]) -> None:
        ...
        # Write receipt
        receipt_file = self.receipt_path / f"gate-{operation}-{now.strftime('%Y%m%d-%H%M%S')}.json"
        try:
...
def create_constitutional_gate(receipt_path: Optional[Path] = None) -> ConstitutionalGate:
    """
