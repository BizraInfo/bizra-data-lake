...
class SAPEPCIBridge:
    ...
    def compute_scores(
        ...
        ihsan_equivalent = (
            sum(ihsan_scores) / len(ihsan_scores) if ihsan_scores else 0.0
        )
        ...
        with self._lock:
            if pattern_hash in self._pattern_cache:
                ...
                pattern.success_rate = (
                    pattern.success_rate * n + (1.0 if success else 0.0)
                ) / (n + 1)
    ...
    @classmethod
    def from_json(
        cls, json_str: str, elevation_callback: Optional[Callable] = None
    ) -> "SAPEPCIBridge":
...
def compute_metadata(
    results: List[ProbeResult], content: Optional[str] = None
) -> SAPEMetadata:
    """
