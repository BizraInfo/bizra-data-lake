...
def main() -> int:
    ...
    if not backend_dir.exists():
        print(
            f"node0 warning budget failed: backend directory not found: {backend_dir}"
        )
        return 2
    ...
    t = threading.Thread(
        target=read_all_stderr, args=(proc.stderr, stderr_lines), daemon=True
    )
    t.start()
    ...
    if warnings > int(args.max_warnings):
        ...
        print(
            "- ratchet policy: reduce warnings then lower max_warnings; never raise without a documented exception"
        )
        return 1
