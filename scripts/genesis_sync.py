...
WSL_GENESIS = Path(
    r"/mnt/c/BIZRA-DATA-LAKE/04_GOLD/genesis.json"
)  # Same file, WSL path
...
def propagate_genesis_to_data_lake(force_overwrite: bool = False) -> bool:
    ...
    if not SOURCE_GENESIS.exists():
        ...
        if DATA_LAKE_GENESIS.exists():
            print(
                "ℹ️ Source not found, but Data Lake has Genesis. Using Data Lake as source."
            )
            return True
    ...
    if not DATA_LAKE_GENESIS.exists():
        ...
        print("✅ GENESIS PROPAGATED (First Write)")
    ...
    if verify_genesis_integrity(SOURCE_GENESIS, DATA_LAKE_GENESIS):
        print("✅ GENESIS ALREADY SYNCED (No action needed)")
        print(f"   Hash: {compute_genesis_hash(DATA_LAKE_GENESIS)[:16]}...")
        return True
    ...
    if force_overwrite:
        ...
        backup_path = DATA_LAKE_GENESIS.with_suffix(
            f".backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        shutil.copy2(DATA_LAKE_GENESIS, backup_path)
        shutil.copy2(SOURCE_GENESIS, DATA_LAKE_GENESIS)
        print("⚠️ GENESIS OVERWRITTEN (Recovery Mode)")
...
def verify_all_genesis_copies() -> dict:
    ...
    results = {
        "source": {
            ...
            "hash": (
                compute_genesis_hash(SOURCE_GENESIS)
        ...
        "data_lake": {
            ...
            "hash": (
                compute_genesis_hash(DATA_LAKE_GENESIS)
                if DATA_LAKE_GENESIS.exists()
                else None
            ),
        },
    ...
    hashes = [
        ...
        if r["hash"] and r["hash"] != "FILE_NOT_FOUND"
    ]
