...
class ReasoningCandidate:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"candidate_id": self.candidate_id, "content": self.content,
...
class VerificationResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"candidate_id": self.candidate_id, "verified": self.verified,
                "critique": self.critique, "adjusted_confidence": self.adjusted_confidence}
...
class BicameralResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"final_answer": self.final_answer, "consensus_score": self.consensus_score,
                "candidates_generated": self.candidates_generated,
                "candidates_verified": self.candidates_verified, "reasoning_path": self.reasoning_path}
...
class BicameralReasoningEngine:
    ...
    def __init__(self, local_endpoint: Optional[LocalInferenceProtocol] = None,
                 api_client: Optional[AnalyticalClientProtocol] = None,
                 consensus_threshold: float = DEFAULT_CONSENSUS_THRESHOLD) -> None:
        self.local_endpoint = local_endpoint
        self.api_client = api_client
        self.consensus_threshold = consensus_threshold
        self._metrics = {"generations": 0, "verifications": 0, "consensus_hits": 0, "fallbacks": 0}
        logger.info("BicameralEngine init | local=%s api=%s", local_endpoint is not None, api_client is not None)
    ...
    async def generate_candidates(self, problem: str, num_candidates: int = DEFAULT_NUM_CANDIDATES) -> List[ReasoningCandidate]:
        """Generate multiple solution candidates using local model (R1-style)."""
        candidates: List[ReasoningCandidate] = []
        cot_prompt = f"<|reasoning|>\nProblem: {problem}\nThink step by step.\n<|answer|>\n"
        ...
        if self.local_endpoint:
            ...
            try:
                ...
                candidates.append(ReasoningCandidate(
                    str(uuid.uuid4())[:8], result.get("answer", str(result)),
                    "claude", result.get("confidence", 0.7), "[FALLBACK]"))
    ...
    async def verify_candidate(self, candidate: ReasoningCandidate, criteria: Dict[str, Any]) -> VerificationResult:
        """Verify a candidate using analytical model (Claude-style)."""
        if not self.api_client:
            return VerificationResult(candidate.candidate_id, True, "[FALLBACK]", candidate.confidence * 0.7)

        try:
            result = await asyncio.wait_for(
                self.api_client.analyze(candidate.content, criteria), timeout=30.0)
            self._metrics["verifications"] += 1
            adj = max(0.0, min(1.0, candidate.confidence + result.get("confidence_delta", 0.0)))
            return VerificationResult(candidate.candidate_id, result.get("passes", False),
                                      result.get("critique", ""), adj)
        except Exception as e:
            logger.error("Verification error: %s", e)
            return VerificationResult(candidate.candidate_id, False, f"[ERROR] {e}", candidate.confidence * 0.5)
    ...
    async def reason(self, problem: str, context: Dict[str, Any]) -> BicameralResult:
        ...
        candidates = await self.generate_candidates(problem, context.get("num_candidates", DEFAULT_NUM_CANDIDATES))
        path.append({"phase": "generation", "ts": datetime.now(timezone.utc).isoformat(),
        ...
        verifications = await asyncio.gather(*[self.verify_candidate(c, criteria) for c in candidates])
        vmap = {v.candidate_id: v for v in verifications}
        path.append({"phase": "verification", "ts": datetime.now(timezone.utc).isoformat(),
                     "results": [v.to_dict() for v in verifications]})

        # Phase 3: Select best
        verified = [(c, vmap[c.candidate_id]) for c in candidates if vmap[c.candidate_id].verified]
        if verified:
        ...
        path.append({"phase": "selection", "ts": datetime.now(timezone.utc).isoformat(),
                     "selected": best_c.candidate_id, "score": score, "elapsed_ms": elapsed})
        logger.info("Bicameral complete | gen=%d ver=%d score=%.2f", len(candidates), len(verified), score)
        return BicameralResult(best_c.content, score, len(candidates), len(verified), path)

    def _select_best(self, verified: List[Tuple[ReasoningCandidate, VerificationResult]]) -> Tuple[ReasoningCandidate, VerificationResult]:
        """Select best candidate by adjusted confidence."""
        return max(verified, key=lambda cv: (cv[1].adjusted_confidence, cv[0].confidence))
    ...
    async def _gen_single(self, prompt: str, idx: int) -> ReasoningCandidate:
        ...
        try:
            ...
            conf = 0.6 + (0.1 if len(trace.split()) > 50 else 0) + (0.1 if len(trace.split()) > 100 else 0)
            return ReasoningCandidate(str(uuid.uuid4())[:8], content.strip(), "r1", min(conf, 1.0), trace)
        except Exception as e:
