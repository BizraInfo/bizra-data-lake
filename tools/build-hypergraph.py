...
class HypergraphConstructor:
    ...
    def build_from_corpus(self):
        ...
        df_chunks = (
            pd.read_parquet(CHUNKS_TABLE_PATH) if CHUNKS_TABLE_PATH.exists() else None
        )

        print("🏗️ Building Graph Spine (Documents & Chunks)...")
        for _, doc in tqdm(
            df_docs.iterrows(), total=len(df_docs), desc="Mapping Documents"
        ):
            self.graph.add_node(
                doc["doc_id"], label=doc["title"], kind="Document", source=doc["uri"]
            )

            # Extract basic entities/concepts to create Assertion nodes later
            # (In a real system, this would call an LLM/NLP engine)
            concepts = re.findall(
                r"\b(BIZRA|SAPE|RTX|GPU|Organism|Node0)\b", doc["text"], re.I
            )
        ...
        if df_chunks is not None:
            ...
            for _, chunk in tqdm(
                df_chunks.iterrows(), total=len(df_chunks), desc="Mapping Chunks"
            ):
                self.graph.add_node(
                    chunk["chunk_id"], kind="Chunk", doc_id=chunk["doc_id"]
                )
                self.graph.add_edge(
                    chunk["chunk_id"], chunk["doc_id"], relation="PART_OF"
                )
                ...
                if "SAPE" in chunk["chunk_text"]:
                    ...
                    self.graph.add_node(
                        assertion_id,
                        kind="Assertion",
                        text="Discusses SAPE architecture",
                    )
                    self.graph.add_edge(
                        assertion_id,
                        chunk["chunk_id"],
                        relation="ASSERTION_SUPPORTED_BY",
                    )
                    self.graph.add_edge(
                        assertion_id, "ent::sape", relation="ASSERTION_INVOLVES"
                    )
