...
SPEARPOINT_SNR_FLOOR: float = UNIFIED_SNR_THRESHOLD  # Minimum acceptable signal quality
SPEARPOINT_IHSAN_THRESHOLD: float = UNIFIED_IHSAN_THRESHOLD  # Excellence constraint
...
class SpearheadResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "output": (
                self.output[:200] + "..." if len(self.output) > 200 else self.output
            ),
            "snr": round(self.snr_score, 4),
            "ihsan": round(self.ihsan_score, 4),
...
class SovereignSpearpoint:
    ...
    def _calculate_ihsan(self, content: str, snr: float) -> float:
        ...
        uncertainty = any(
            w in content.lower()
            for w in ["however", "although", "consider", "might", "could"]
        )
