...
from core.fate import FateEngine, FateSeal, IhsanVector
...
class CognitiveHook:
    ...
    def reason(self, query: str, **kwargs) -> HookResult:
        ...
        return self.invoke(query, slot="cold_core", hook_type=HookType.REASONING, **kwargs)
    
    def plan(self, objective: str, **kwargs) -> HookResult:
        """Invoke planning hook using primary_reasoning slot."""
        return self.invoke(objective, slot="primary_reasoning", hook_type=HookType.PLANNING, **kwargs)
    ...
    def think(self, problem: str, **kwargs) -> HookResult:
        ...
        return self.invoke(problem, slot="thinking", hook_type=HookType.REASONING, **kwargs)
...
register_hook("planner", CognitiveHook(HookConfig(
...
register_hook("thinker", CognitiveHook(HookConfig(
    slot="thinking",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hook", default="default", choices=["default", "fast", "planner", "thinker"])
    ...
    print(f"   Ihsān: {result.ihsan_score:.2f} ({'✓ PASS' if result.ihsan_passed else '✗ FAIL'})")
