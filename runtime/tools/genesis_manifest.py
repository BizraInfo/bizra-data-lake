...
def git_commit(repo_root: Path, commit: str | None) -> str:
    ...
    if res.returncode != 0:
        raise ValueError(
            f"could not determine git commit in {repo_root}: {res.stderr.strip()}"
        )
...
def load_core_policy_hashes(repo_root: Path, commit: str) -> dict[str, str]:
    ...
    if not all(
        ...
        for v in [contract_rel, schema_rel, receipt_schema_rel]
    ):
        raise ValueError(
            "lexicon must define non-empty contract/schema/receipt_schema paths"
        )
...
def generate_manifest(
    *,
    ...
    for repo in repos:
        ...
        if not is_git_repo(abs_root):
            if allow_missing_repos:
                continue
            print(
                f"genesis manifest generation failed: repo root is not a git repo: {repo.root}"
            )
            return 2
    ...
    entries = sorted(
        entries, key=lambda e: (str(e["repo"]).casefold(), str(e["path"]).casefold())
    )
    ...
    manifest = {
        ...
        "source": {
            ...
            "core_dirty": repo_is_dirty(repo_root),
        },
...
def verify_manifest(
    *,
    ...
    entries_sorted = sorted(
        ...
        key=lambda e: (
            str(e.get("repo", "")).casefold(),
            str(e.get("path", "")).casefold(),
        ),
    )
    ...
    for r in repos:
        ...
        for e in entries:
            ...
            if actual != expected:
                failures.append(
                    f"hash mismatch {name}:{relpath} expected={expected} actual={actual}"
                )
        ...
        if isinstance(profile_path, str) and isinstance(profile_sha, str):
            ...
            if core:
                try:
                    ...
                    failures.append(
                        f"could not load profile from git for verification ({exc})"
                    )
        ...
        if core:
            try:
                ...
                expected_sha = sha256_bytes(
                    json.dumps(expected_hashes, sort_keys=True).encode("utf-8")
                )
                if policy_sha != expected_sha:
                    failures.append(
                        f"policy_sha256 mismatch expected={expected_sha} actual={policy_sha}"
                    )
            except Exception as exc:
...
def profile_lint(*, profile_path: Path) -> int:
    ...
    for repo in repos:
        if any(Path(p).is_absolute() for p in [repo.root]):
            print(
                f"genesis profile lint failed: repo root must be relative: {repo.root}"
            )
            return 1
        for pattern in repo.include + repo.exclude:
            if pattern.startswith("../") or pattern.startswith("..\\"):
                print(
                    f"genesis profile lint failed: pattern must not escape repo: {pattern}"
                )
                return 1
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate/verify a GenesisManifest (multi-repo)"
    )
    ...
    p_gen = sub.add_parser(
        "generate", help="Generate a GenesisManifest JSON from the profile"
    )
    p_gen.add_argument(
        "--profile",
        default=str(repo_root / "constitution" / "genesis_manifest_profile_v1.yaml"),
    ...
    p_verify = sub.add_parser(
        "verify", help="Verify a GenesisManifest JSON against git content"
    )
