...
npx claude-flow github review-init \
  --pr 123 \
  --pr-data "$PR_DATA" \
  --diff "$PR_DIFF" \
...
# Run security review
SECURITY_RESULTS=$(npx claude-flow github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
...
npx claude-flow github review-performance \
  --pr 123 \
...
```bash
# Architecture review
npx claude-flow github review-architecture \
  --pr 123 \
...
npx claude-flow github review-context \
  --pr 123 \
  --load-related-prs \
  --analyze-impact \
...
npx claude-flow github review-learn \
...
npx claude-flow github review-batch \
  --prs "123,124,125" \
  --check-consistency \
  --verify-integration \
  --combined-impact
    ...
    steps:
      ...
      - name: Run Review Swarm
        run: |
          ...
          # Run swarm review
          REVIEW_OUTPUT=$(npx claude-flow github review-all \
            --pr $PR_NUM \
            --pr-data "$PR_DATA" \
...
COMMENTS=$(npx claude-flow github review-comment \
  --pr 123 \
  --diff "$PR_DIFF" \
  --files "$PR_FILES" \
  --style "constructive" \
...
npx claude-flow github review-comments \
  --pr 123 \
...
npx claude-flow github quality-gates \
  --define '{
...
npx claude-flow github review-metrics \
  --period 30d \
...
npx claude-flow github review-train \
  --learn-patterns \
  --adapt-to-style \
  --improve-accuracy
```
...
npx claude-flow github review-orchestrate \
  --strategy "risk-based" \
...
npx claude-flow github review-dashboard \
  --real-time \
...
npx claude-flow github review-report \
  --format "markdown" \
