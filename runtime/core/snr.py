...
def estimate_snr(
    output_text: str, input_text: str = "", evidence_count: int = 0
) -> float:
    """
    ...
    noise_count = sum(
        1 for phrase in noise_phrases if phrase.lower() in output_text.lower()
    )
    ...
    spec_count = sum(
        1 for phrase in speculation_phrases if phrase.lower() in output_text.lower()
    )
    ...
    return max(
        ...
        min(
            1,
            base_score + evidence_score + concise_score - noise_penalty - spec_penalty,
        ),
    )
