...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Validate sealed model family manifest"
    )
    parser.add_argument(
        "--manifest",
    ...
    if not (isinstance(sealed_by, list) and all(is_nonempty_str(s) for s in sealed_by)):
        failures.append(
            f"sealed_by must be a non-empty list of strings ({manifest_path})"
        )

    capability_slots = mf.get("capability_slots")
    if not isinstance(capability_slots, dict) or not capability_slots:
        failures.append(
            f"capability_slots must be a non-empty mapping ({manifest_path})"
        )

    pinned = mf.get("pinned_artifacts")
    if not isinstance(pinned, dict) or not pinned:
        failures.append(
            f"pinned_artifacts must be a non-empty mapping ({manifest_path})"
        )
    ...
    if isinstance(pinned, dict):
        for artifact_id, spec in pinned.items():
            if not is_nonempty_str(artifact_id):
                failures.append(
                    f"pinned_artifacts contains non-string key ({manifest_path})"
                )
                continue
            if not isinstance(spec, dict):
                failures.append(
                    f"pinned_artifacts['{artifact_id}'] must be a mapping ({manifest_path})"
                )
                continue

            provider = spec.get("provider")
            if (
                not is_nonempty_str(provider)
                or provider.strip().lower() not in PROVIDERS
            ):
                failures.append(
            ...
            if provider_norm == "ollama":
                ...
                if not is_nonempty_str(digest) or not SHA256_PREFIXED_RE.match(
                    digest.strip().lower()
                ):
                    failures.append(
            ...
            if provider_norm == "lmstudio":
                ...
                if not (isinstance(files, list) and files):
                    ...
                    for idx, item in enumerate(files, start=1):
                        ...
                        sha = item.get("sha256")
                        if not is_nonempty_str(sha) or not SHA256_HEX_RE.match(
                            sha.strip().lower()
                        ):
                            failures.append(
    ...
    if isinstance(capability_slots, dict) and isinstance(pinned, dict):
        for slot_name, slot_spec in capability_slots.items():
            if not is_nonempty_str(slot_name):
                failures.append(
                    f"capability_slots contains non-string key ({manifest_path})"
                )
                continue
            if not isinstance(slot_spec, dict):
                failures.append(
                    f"capability_slots['{slot_name}'] must be a mapping ({manifest_path})"
                )
                continue
            ...
            if not (
                ...
                and allowed
                and all(is_nonempty_str(s) for s in allowed)
            ):
                failures.append(
            ...
            missing = sorted(
                [a for a in allowed_set if a not in pinned], key=str.casefold
            )
            if missing:
                failures.append(
