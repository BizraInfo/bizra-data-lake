...
class IhsanGate:
    ...
    def validate_request(
        self, message: str, context: dict[str, Any], agent: str = ""
    ) -> GateDecision:
        """
        ...
        if ihsan_score >= self.threshold:
            ...
            decision = GateDecision(
                allowed=True, ihsan_score=ihsan_score, latency_ms=latency_ms
            )
    ...
    def validate_response(
        ...
        if response_score < self.threshold:
            ...
            return GateDecision(
                allowed=False, ihsan_score=response_score, reason=reason
            )

        return GateDecision(allowed=True, ihsan_score=response_score)
            ...
            if isinstance(result, str):
                ...
                if not response_decision.allowed:
                    return {
                        "error": True,
                        "rejection": (
                            response_decision.reason.to_dict()
                            if response_decision.reason
                            else None
                        ),
...
def main():
    ...
    parser.add_argument(
        "--threshold", "-t", type=float, default=IHSAN_THRESHOLD, help="Ihsān threshold"
    )
    ...
    if args.message:
        context = {"backend": "ollama", "endpoint": "http://127.0.0.1:11434"}
