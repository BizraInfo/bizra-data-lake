...
class GraphOfThoughts:
    ...
    def _gen_id(self) -> str:
        return hashlib.blake2b(
            f"{datetime.now().isoformat()}{len(self.thoughts)}".encode(), digest_size=16
        ).hexdigest()[:12]
    ...
    def branch(
        ...
        with self._lock:
            ...
            if parent_id in self.thoughts:
                self.thoughts[parent_id].children.append(nid)
        return node
    ...
    def merge(self, node_ids: List[str], merged_thought: str) -> ThoughtNode:
        ...
        all_ev = []
        [
            all_ev.extend(self.thoughts[n].evidence)
            for n in node_ids
    ...
    def get_best_path(self) -> List[ThoughtNode]:
        ...
        while cur:
            ...
            if node.children:
                child_confs = [
                    (c, self.thoughts[c].confidence)
                    for c in node.children
                    if c in self.thoughts
                ]
...
class SNREngine:
    ...
    @staticmethod
    def calculate_node_snr(
        node: KnowledgeNode, edge_count: int, total_nodes: int, avg_edges: float
    ) -> float:
    ...
    @staticmethod
    def calculate_query_snr(
        results: List[KnowledgeNode], terms: List[str], paths: int
    ) -> float:
        if not results:
            return 0.0
        coverage = sum(
            1
            for t in terms
            if any(t in (n.name + (n.description or "")).lower() for n in results)
        )
...
class SovereignKnowledgeNexus:
    ...
    def index_node0_inventory(self) -> int:
        ...
        if not Config.NODE0_INVENTORY.exists():
            return 0
        with open(Config.NODE0_INVENTORY, "r", encoding="utf-8") as f:
            inv = json.load(f)
        n = 0
        ...
        if hw.get("cpu"):
            cpu = KnowledgeNode(
                id="hw_cpu",
                name=hw["cpu"].get("name", "CPU"),
                type=NodeType.HARDWARE,
                metadata=hw["cpu"],
            )
            self.add_node(cpu)
            self.add_edge(KnowledgeEdge("node0_root", "hw_cpu", EdgeType.CONTAINS))
            n += 1
        ...
        for i, gpu in enumerate(hw.get("gpu", [])):
            ...
            self.add_node(g)
            self.add_edge(KnowledgeEdge("node0_root", f"hw_gpu_{i}", EdgeType.CONTAINS))
            n += 1
        ...
        for prog in inv.get("software", {}).get("programs", []):
            ...
            self.add_edge(KnowledgeEdge("node0_root", pid, EdgeType.CONTAINS))
            n += 1
        ...
        for distro in inv.get("software", {}).get("wsl_distros", []):
            ...
            self.add_edge(KnowledgeEdge("node0_root", did, EdgeType.CONTAINS))
            n += 1
        ...
        for folder in inv.get("data", {}).get("all_folders", []):
            ...
            self.add_node(fn)
            self.add_edge(KnowledgeEdge("node0_root", fid, EdgeType.CONTAINS))
            if folder.get("is_bizra"):
                self.add_edge(KnowledgeEdge(fid, "node0_root", EdgeType.PART_OF, 2.0))
            n += 1
        ...
        if dl:
            dn = KnowledgeNode(
                ...
                path=dl.get("path"),
                metadata={
                    "folders": dl.get("top_level_folders", 0),
                    "bizra": len(dl.get("bizra_related_folders", [])),
                },
            )
            self.add_node(dn)
            self.add_edge(
                KnowledgeEdge("node0_root", "folder_downloads", EdgeType.CONTAINS)
            )
            n += 1
            ...
            for fname in dl.get("folder_names", []):
                ...
                if is_biz:
                    self.add_edge(KnowledgeEdge(fid, "node0_root", EdgeType.PART_OF))
                n += 1
                ...
                for line in f:
                    ...
                    try:
                        ...
                        nt = {
                            ...
                            "technology": NodeType.CONCEPT,
                            "pattern": NodeType.PATTERN,
                            "insight": NodeType.INSIGHT,
                            "reference": NodeType.RESEARCH,
                        }.get(et, NodeType.CONCEPT)
                        node = KnowledgeNode(
                            id=e.get(
                                "id",
                                hashlib.blake2b(
                                    line.encode(), digest_size=16
                                ).hexdigest()[:12],
                            ),
                            name=e.get("name", "?"),
                            type=nt,
                            ...
                            metadata={
                                ...
                                for k, v in e.items()
                                if k
                                not in ("id", "name", "type", "path", "description")
                            },
                        )
                        self.add_node(node)
                        n += 1
                        if e.get("parent"):
                            self.add_edge(
                                KnowledgeEdge(e["parent"], node.id, EdgeType.CONTAINS)
                            )
                    except:
                        continue
    ...
    def _calc_snr_all(self):
        ...
        for nid, node in self.nodes.items():
            ...
            node.snr_score = SNREngine.calculate_node_snr(
                node, ec, len(self.nodes), avg
            )
    ...
    def query(self, query_text: str, max_results: int = 20) -> QueryResult:
        ...
        for term in terms:
            if term in self.name_index:
                matched.update(self.name_index[term])
        got.branch(got.root_id, f"Direct: {len(matched)}", 0.8, list(matched)[:5])

        # Expand
        expanded = set(matched)
        for nid in matched:
            for n in self.get_neighbors(nid)[:10]:
                expanded.add(n.id)
        got.branch(got.root_id, f"Expanded: {len(expanded)}", 0.7)

        # Rank
        ranked = sorted(
            [self.nodes[n] for n in expanded if n in self.nodes],
            key=lambda x: x.snr_score,
            reverse=True,
        )[:max_results]
        ...
        got.merge(
            [t.id for t in got.get_leaves()], f"Synthesized {len(ranked)} results"
        )

        # Edges
        rids = {n.id for n in ranked}
        edges = [
            e for n in ranked for e in self.edges.get(n.id, []) if e.target_id in rids
        ]
    ...
    def _gen_insights(self, nodes: List[KnowledgeNode], terms: List[str]) -> List[str]:
        ins = []
        tc = defaultdict(int)
        [tc.__setitem__(n.type.name, tc[n.type.name] + 1) for n in nodes]
        if tc:
            dom = max(tc.items(), key=lambda x: x[1])
            ins.append(f"Dominated by {dom[0]} ({dom[1]}/{len(nodes)})")
        biz = sum(
            1 for n in nodes if n.metadata.get("is_bizra") or "bizra" in n.name.lower()
        )
        if biz:
            ins.append(f"{biz} BIZRA assets found")
        hi = [n for n in nodes if n.snr_score > 0.6]
        if hi:
            ins.append(f"{len(hi)} high-SNR nodes (>0.6)")
        return ins
    ...
    def save(self):
        ...
        with open(Config.NEXUS_GRAPH, "w", encoding="utf-8") as f:
            json.dump(gd, f, indent=2, ensure_ascii=False)
        with open(Config.NEXUS_STATS, "w", encoding="utf-8") as f:
            json.dump(self.stats, f, indent=2)
        log.info(f"💾 Saved to {Config.NEXUS_GRAPH}")

    def load(self) -> bool:
        if not Config.NEXUS_GRAPH.exists():
            return False
        try:
            with open(Config.NEXUS_GRAPH, "r", encoding="utf-8") as f:
                data = json.load(f)
            for nd in data.get("nodes", []):
                self.add_node(KnowledgeNode.from_dict(nd))
            for ed in data.get("edges", []):
                self.add_edge(KnowledgeEdge.from_dict(ed))
            if Config.NEXUS_STATS.exists():
                with open(Config.NEXUS_STATS, "r", encoding="utf-8") as f:
