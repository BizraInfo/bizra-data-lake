...
class GraphOfThoughts:
    ...
    def add_thought(
        ...
        if snr_score < self.prune_threshold:
            ...
            logger.debug(
                f"Pruned thought {thought_id}: SNR {snr_score:.3f} < {self.prune_threshold}"
            )

        return thought

    def merge_thoughts(
        self, thought_ids: list[str], merged_content: str, snr_score: float
    ) -> ThoughtNode:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        active = sum(
            1 for t in self.thoughts.values() if t.status == ThoughtStatus.ACTIVE
        )
        pruned = sum(
            1 for t in self.thoughts.values() if t.status == ThoughtStatus.PRUNED
        )
        merged = sum(
            1 for t in self.thoughts.values() if t.status == ThoughtStatus.MERGED
        )
...
class SNRApexEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"SNR Apex Engine initialized | Target: {target_snr} | Floor: {floor_snr}"
        )
    ...
    def _generate_recommendations(
        ...
        if signal.get("novelty", 0) < 0.5:
            recommendations.append(
                "Include novel insights not present in prior context"
            )
        ...
        if gap > 0:
            recommendations.append(
                f"Gap to APEX target: {gap:.3f} (need {self.target_snr})"
            )
    ...
    def maximize(
        ...
        for i in range(self.max_iterations):
            ...
            if best_analysis.apex_achieved:
                logger.info(
                    f"APEX achieved at iteration {i}: SNR {best_analysis.snr_linear:.4f}"
                )
                break

            # Apply optimization if provided
            if optimization_fn:
                current_signal, current_noise = optimization_fn(
                    current_signal, current_noise
                )
            else:
                # Default optimization: boost weak signals, reduce high noise
                current_signal = self._default_signal_boost(
                    current_signal, best_analysis
                )
            ...
            if improvement > 0:
                ...
                logger.debug(
                    f"Iteration {i}: SNR improved by {improvement:.4f} to {analysis.snr_linear:.4f}"
                )
            else:
        ...
        status = (
            "APEX"
            if best_analysis.apex_achieved
            else "IHSAN" if best_analysis.ihsan_achieved else "BELOW"
        )
        logger.info(
            f"Maximization complete: SNR {best_analysis.snr_linear:.4f} [{status}] in {iterations_used} iterations"
        )

        return best_analysis, iterations_used

    def _default_signal_boost(
        self, signal: dict[str, float], analysis: SNRAnalysis
    ) -> dict[str, float]:
    ...
    def gate(
        self, signal: dict[str, float], noise: dict[str, float]
    ) -> tuple[bool, SNRAnalysis]:
        """
...
class ApexReasoningEngine:
    ...
    async def reason(
        ...
        analysis, iterations = self.snr_engine.maximize(
            signal_components, noise_components
        )
        ...
        return {
            ...
            "status": (
                "APEX_ACHIEVED"
                if analysis.apex_achieved
                else "IHSAN_ACHIEVED" if analysis.ihsan_achieved else "BELOW_THRESHOLD"
            ),
        }
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        print(
            f"SNR: {result['snr_analysis']['snr_linear']:.4f} ({result['snr_analysis']['snr_db']:.2f} dB)"
        )
