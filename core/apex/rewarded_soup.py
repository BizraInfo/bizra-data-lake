...
class PersonaSoup:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isclose(total_weight, 1.0, abs_tol=1e-6):
            raise ValueError(
                f"Lambda weights must sum to 1.0, got {total_weight}"
            )
    ...
    def base_vote_weight(self) -> float:
        ...
        return sum(
            c.lambda_weight * c.persona.base_vote_weight
    ...
    def _compute_hash(self) -> str:
        ...
        hash_payload = {
            ...
            "voice_embedding": self.voice_embedding[:16],  # First 16 dims for efficiency
    ...
    def get_component_by_persona(self, persona_id: str) -> Optional[PersonaSoupComponent]:
...
def interpolate_embeddings(
    ...
    for i, emb in enumerate(embeddings):
        if len(emb) != dim:
            raise ValueError(
                f"Embedding {i} dimension mismatch: {len(emb)} vs {dim}"
            )
...
def compose_weighted_prompt(
    ...
    weighted_personas = sorted(
        zip(personas, lambdas),
        key=lambda x: x[1],
        reverse=True
    )
    ...
    guidance = f"Primary behavioral model ({dominant_persona.name}, {dominant_weight:.0%}):\n"
...
def validate_soup_integrity(soup: PersonaSoup) -> bool:
    ...
    if not math.isclose(total_weight, 1.0, abs_tol=1e-6):
        logger.error(f"Soup {soup.soup_id} lambda weights sum to {total_weight}, not 1.0")
        return False
