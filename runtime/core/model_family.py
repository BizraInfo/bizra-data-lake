...
def load_model_family(path: Optional[Path] = None) -> ModelFamily:
    ...
    allow_unsealed = os.getenv("BIZRA_ALLOW_UNSEALED_MODEL_FAMILY", "").strip() in {
        "1",
        "true",
        "TRUE",
        "yes",
    ...
    if not sealed and not allow_unsealed:
        raise ValueError(
            f"model family manifest must be sealed (set BIZRA_ALLOW_UNSEALED_MODEL_FAMILY=1 to override): {mf_path}"
        )

    sealed_at = data.get("sealed_at_utc")
    sealed_at_utc = (
        str(sealed_at) if isinstance(sealed_at, str) and sealed_at.strip() else None
    )
    ...
    if isinstance(sealed_by_raw, list):
        sealed_by = [
            str(x).strip() for x in sealed_by_raw if isinstance(x, str) and x.strip()
        ]
    ...
    for name, spec in pinned_raw.items():
        ...
        digest_norm = (
            str(digest).strip() if isinstance(digest, str) and digest.strip() else None
        )
        modelfile_sha = spec.get("modelfile_sha256")
        modelfile_sha_norm = (
            str(modelfile_sha).strip().lower()
            if isinstance(modelfile_sha, str) and modelfile_sha.strip()
            else None
        )
    ...
    for slot_name, spec in slots_raw.items():
        ...
        if not (
            ...
            and allowed_raw
            and all(isinstance(s, str) and s.strip() for s in allowed_raw)
        ):
            raise ValueError(
                f"capability_slots['{slot_name}'].allowed_models must be a non-empty list: {mf_path}"
            )
        allowed = [
            str(s).strip() for s in allowed_raw if isinstance(s, str) and s.strip()
        ]

        routing = spec.get("routing")
        if not isinstance(routing, dict):
            raise ValueError(
                f"capability_slots['{slot_name}'].routing must be a mapping: {mf_path}"
            )
        primary = routing.get("primary")
        if not isinstance(primary, str) or not primary.strip():
            raise ValueError(
                f"capability_slots['{slot_name}'].routing.primary must be a string: {mf_path}"
            )
        primary = primary.strip()

        fallback = routing.get("fallback")
        fallback_norm = (
            fallback.strip() if isinstance(fallback, str) and fallback.strip() else None
        )
        ...
        missing = [
            m
            for m in set(
                allowed + [primary] + ([fallback_norm] if fallback_norm else [])
            )
            if m and m not in pinned
        ]
        if missing:
            raise ValueError(
                f"capability_slots['{slot_name}'] references unpinned models {sorted(missing)}: {mf_path}"
            )
