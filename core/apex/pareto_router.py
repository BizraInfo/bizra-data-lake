...
class ObjectiveVector:
    ...
    def satisfies_thresholds(
        ...
        return (
            ...
            self.ethics >= ethics_threshold and
...
class DefaultObjectiveEvaluator:
    ...
    def evaluate(
        ...
        """
        # Compute alignment
        alignment = persona.compute_task_alignment(task_domains) if task_domains else 0.5

        # SNR: Higher alignment and vote_weight = better quality
        snr = min(1.0, self.base_snr + alignment * 0.05 + persona.base_vote_weight * 0.03)

        # Novelty: Creative personas score higher
        creative_domains = {"creativity", "synthesis", "ideation", "cross-domain-transfer"}
...
class ParetoOptimalRouter:
    ...
    def select_from_pareto(
        ...
        if require_threshold:
            viable_points = [
                ...
                if p.objectives.satisfies_thresholds()
            ]
            ...
            if not viable_points:
                ...
                if not viable_points:
                    ...
                    logger.warning(
                        "No points meet thresholds, using full Pareto front"
                    )
                    viable_points = self.pareto_front
    ...
    def get_front_statistics(self) -> Dict[str, Any]:
        ...
        if NUMPY_AVAILABLE:
            ...
            means = [
                sum(obj[i] for obj in obj_arrays) / len(obj_arrays)
                for i in range(5)
            ]
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="BIZRA Pareto-Optimal Router Testing"
    )
    parser.add_argument(
        "--domains",
        nargs="+",
