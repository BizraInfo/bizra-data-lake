...
class SATValidator:
    ...
    def __post_init__(self):
        if self.role not in SAT_ROLES:
            raise ValueError(f"Invalid SAT role: {self.role}. Must be one of {SAT_ROLES}")
...
class DistributedQuorum:
    ...
    def __init__(
        ...
        self._post_collect_hooks: List[Callable[[str, List[SignatureResponse]], None]] = []
    ...
    async def _collect_via_synapse(
        ...
        while True:
            ...
            if len([s for s in signatures if s.approved]) + remaining_validators < self.REQUIRED_VALIDATORS:
                return QuorumState.FAILED, signatures
    ...
    def check_quorum(
        ...
        if approved_count < self.REQUIRED_VALIDATORS:
            return False, f"Insufficient signatures: {approved_count}/{self.REQUIRED_VALIDATORS}"
