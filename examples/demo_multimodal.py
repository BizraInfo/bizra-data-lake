...
async def test_multimodal_pipeline():
    ...
    print(
        f'   Multi-Modal: {"✅ Enabled" if orchestrator.multimodal_enabled else "⚠️ Disabled"}'
    )
    print(
        f'   PAT Engine: {"✅ Active" if orchestrator.pat_enabled else "⚠️ Inactive"}'
    )
    ...
    if orchestrator.dual_agentic_enabled and orchestrator.dual_agentic:
        ...
        if da_available:
            print(
                f"   LM Studio: ✅ Connected ({len(orchestrator.dual_agentic._models)} models)"
            )
            # Check for vision model
            vision_models = [
                m
                for m in orchestrator.dual_agentic._models
