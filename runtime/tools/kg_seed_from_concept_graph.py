...
def seed_from_concept_graph(
    conn: psycopg.Connection, nodes_path: Path, edges_path: Path
) -> Dict[str, int]:
...
def seed_from_unified_hypergraph(
    conn: psycopg.Connection, graph_dir: Path
) -> Dict[str, int]:
    ...
    try:
        with psycopg.connect(args.dsn) as conn:
            ...
            if args.concept_nodes.exists():
                ...
                stats = seed_from_concept_graph(
                    conn, args.concept_nodes, args.concept_edges
                )
                print(
                    f"   ✅ Entities: {stats['entities']}, Edges: {stats['edges']}, Skipped: {stats['skipped']}"
                )
            ...
            if args.unified_graph.exists():
                ...
                print(
                    f"   ✅ Entities: {stats['entities']}, Documents: {stats['documents']}, Chunks: {stats['chunks']}"
                )
