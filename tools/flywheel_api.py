...
async def health():
    ...
    if status.state == FlywheelState.READY:
        return HealthResponse(
            status="healthy", state=status.state.value, version=status.version
        )
    else:
        return JSONResponse(
            status_code=503,
...
async def audio_websocket(websocket: WebSocket):
    ...
    try:
        ...
        if result != AuthResult.ALLOWED:
            await websocket.send_json(
                {"type": "auth_result", "success": False, "reason": result.value}
            )
...
async def transcribe_audio(
    audio: bytes = None, language: str = "en", token: str = Depends(require_auth)
):
    """
...
async def synthesize_speech(
    text: str, voice: str = "en-US-AriaNeural", token: str = Depends(require_auth)
):
    """
...
async def voice_chat(
    ...
    try:
        ...
        response_audio = await flywheel_instance.audio.synthesize(
