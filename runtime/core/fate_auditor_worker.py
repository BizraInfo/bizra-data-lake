...
def main() -> int:
    ...
    redis_timeout_s = max(
        0.05, min(_env_float("FATE_AUDITOR_REDIS_TIMEOUT_S", 0.35), 2.0)
    )
    neo4j_timeout_s = max(
        0.05, min(_env_float("FATE_AUDITOR_NEO4J_TIMEOUT_S", 0.55), 3.0)
    )

    strict_raw = (
        os.getenv("BIZRA_FATE_STRICT") or os.getenv("FATE_LEVEL") or "1"
    ).strip()
    ...
    while True:
        ...
        auth = (
            _parse_neo4j_auth(neo4j_auth_raw)
            if (wisdom_url and neo4j_auth_raw)
            else None
        )
