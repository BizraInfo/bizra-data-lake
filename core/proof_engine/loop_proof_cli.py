...
def gather_evidence(question: str) -> tuple[str, list[str], str]:
    ...
    for f in [
        "core/zpk/kernel.py",
        "core/proof_engine/fate_gate.py",
        "core/proof_engine/sat_validator.py",
    ]:
        if (repo / f).exists():
        ...
        payload = json.dumps(
            {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "Answer using only the evidence provided. Be precise.",
                    },
                    {
                        "role": "user",
...
def main():
    mission = (
        " ".join(sys.argv[1:])
        if len(sys.argv) > 1
        else ("What is the Spearpoint seal (commit b08f2208) and why does it matter?")
    )
    ...
    for step in proof.steps:
        ...
        status_icon = (
            "✓"
            if step.status in ("ok", "completed", "pass")
            else "✗" if step.status in ("fail", "blocked") else "○"
        )
        print(
            f"  {status_icon} [{step.seq}] {step.actor:40s} {step.action:30s} {step.status}"
        )
        if verdict:
            print(
                f"         verdict={verdict}  ihsan={step.evidence.get('ihsan_score', '')}"
            )
    ...
    print(
        f"FATE verdict:  {proof.fate_result.get('verdict', {}).get('verdict', 'N/A')}"
    )
