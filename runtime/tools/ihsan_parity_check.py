...
def main() -> int:
    ...
    thresholds_by_artifact = {
        normalize_key(str(k)): float(v)
        for k, v in (
            thresholds_by_artifact_raw.items()
    ...
    env_aliases = {
        ...
        for k, v in (
            env_aliases_raw.items() if isinstance(env_aliases_raw, dict) else []
        )
    }
    artifact_aliases = {
        normalize_key(str(k)): normalize_key(str(v))
        for k, v in (
            artifact_aliases_raw.items()
    ...
    if abs(sum(snap["weights"].values()) - 1.0) > eps:
        print(
            f"[FAIL] python weights do not sum to 1.0: {sum(snap['weights'].values())}"
        )
        return 2
    ...
    for k, w in weights.items():
        if abs(float(snap["weights"].get(k, -1.0)) - w) > eps:
            print(
                f"[FAIL] python weight mismatch for {k}: python={snap['weights'].get(k)} constitution={w}"
            )
            return 2
