            ...
            for sat_id in sat_validators[: self.quorum_total]:
                ...
                if validator_keys and sat_id in validator_keys:
                    ...
                    if validator is None or not validator.can_sign():
                        logger.warning(
                            f"SAT validator {sat_id} not found or cannot sign"
                        )
                        continue
                ...
                sig_digest = domain_separated_digest(
                    sig_bytes, "bizra-pci-verifier-v1:"
                )
                signature = sign_message(bytes.fromhex(sig_digest), priv_key)

                verifier_signatures.append(
                    VerifierSignature(
                        sat_id=sat_id,
                        public_key=pub_key,
                        signature=signature,
...
if __name__ == "__main__":
    ...
    async def test_integration():
        ...
        try:
            ...
            print(
                f"  PASS: Envelope created and signed: {envelope.envelope_id[:16]}..."
            )
        except Exception as e:
            print(f"  FAIL: {e}")
            return False
        ...
        try:
            ...
            limited_keys = {
                k: v for k, v in validator_keys.items() if k in limited_validators
            }
            ...
            print("  PASS: Correctly rejected for quorum 2/3")
        except Exception as e:
            print(f"  FAIL: {e}")
            return False
        ...
        try:
            ...
            print("  PASS: Correctly rejected for policy mismatch")
        except Exception as e:
            print(f"  FAIL: {e}")
            return False
        ...
        try:
            ...
            print("  PASS: FlowResult serialized correctly")
        except Exception as e:
            print(f"  FAIL: {e}")
            return False
        ...
        try:
            ...
            print("  PASS: Bridge with registry works correctly")
        except Exception as e:
            print(f"  FAIL: {e}")
            return False
