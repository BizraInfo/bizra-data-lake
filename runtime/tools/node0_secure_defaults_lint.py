...
def main() -> int:
    ...
    if "AllowOrigin::any()" in text:
        failures.append(
            "Node0 backend uses AllowOrigin::any(); CORS must be allowlist-only"
        )

    if 'unwrap_or_else(|_| "0.0.0.0"' in text:
        failures.append(
            'Node0 backend defaults API_HOST to "0.0.0.0"; must default to loopback'
        )

    if "BIZRA_EXPOSE_EXTERNAL" not in text:
        failures.append(
            "Node0 backend missing BIZRA_EXPOSE_EXTERNAL gate for external bind"
        )
    ...
    if "Refusing to bind Node0 API" not in text:
        failures.append(
            "Node0 backend missing explicit refusal when binding non-loopback without override"
        )
