...
impl Mastery {
    ...
    pub fn can_execute(&self) -> bool {
        matches!(self, Self::Novice | Self::Competent | Self::Expert | Self::Master)
    }
...
impl SkillNode {
    ...
    pub fn success_rate(&self) -> f32 {
        ...
        if total == 0 { 0.0 } else { self.successes as f32 / total as f32 }
    }
}
...
impl SkillTree {
    ...
    pub fn summary(&self) -> String {
        ...
        let unlocked = self.nodes.values().filter(|n| n.mastery.can_execute()).count();
        let reflexive = self.nodes.values().filter(|n| n.mastery.is_reflexive()).count();
        let mastered = self.nodes.values().filter(|n| n.mastery == Mastery::Master).count();
        format!(
...
fn skill(
    id: SkillId, name: &'static str, parent: Option<SkillId>,
    prereqs: Vec<SkillId>, sat: bool, hitl: bool,
    roles: Vec<&'static str>, children: Vec<SkillId>,
    ...
    SkillNode {
        ...
        sat_required: sat, hitl_required: hitl,
        affinity_roles: roles, children,
    }
}
...
pub fn filesystem_skill_tree() -> SkillTree {
    ...
    // Root
    tree.insert(skill("fs_root", "Filesystem management", None,
        vec![], false, false,
        vec!["Navigator", "Artisan", "Scholar", "Sentinel"],
        vec!["fs_classify", "fs_rename", "fs_merge", "fs_delete", "fs_archive"],
        Mastery::Novice,
    ));

    // Branch 1: Classification → Organization → Dedup → SNR
    tree.insert(skill("fs_classify", "Smart classification", Some("fs_root"),
        vec![], false, false,
        vec!["Navigator", "Scholar"],
        vec!["fs_organize", "fs_dedup", "fs_snr_score"],
        Mastery::Novice, // available from boot
    ));
    tree.insert(skill("fs_organize", "Auto organization", Some("fs_classify"),
        vec!["fs_classify"], true, false, // SAT validates manifest
        vec!["Artisan", "Navigator"],
        vec![],
        Mastery::Locked, // unlocks when classify reaches Competent
    ));
    tree.insert(skill("fs_dedup", "Duplicate detection", Some("fs_classify"),
        vec!["fs_classify"], false, false,
        vec!["Scholar", "Sentinel"],
        vec![],
        Mastery::Locked,
    ));
    tree.insert(skill("fs_snr_score", "SNR scoring", Some("fs_classify"),
        vec!["fs_classify"], false, false,
        vec!["Scholar"],
        vec![],
        Mastery::Locked,
    ...
    tree.insert(skill("fs_rename", "Batch renaming", Some("fs_root"),
        vec![], false, false,
        vec!["Artisan"],
        vec!["fs_sanitize"],
        Mastery::Novice,
    ));
    tree.insert(skill("fs_sanitize", "Name sanitization", Some("fs_rename"),
        vec!["fs_rename"], false, false,
        vec!["Artisan"],
        vec![],
        Mastery::Locked,
    ));
    ...
    tree.insert(skill("fs_merge", "File merging", Some("fs_root"),
        vec!["fs_classify", "fs_organize"], false, false,
        vec!["Artisan", "Scholar"],
        vec![],
        Mastery::Locked,
    ));
    ...
    tree.insert(skill("fs_delete", "Safe deletion", Some("fs_root"),
        vec!["fs_classify"], true, true, // SAT validates + HITL confirms
        vec!["Sentinel"],
        vec![],
        Mastery::Locked,
    ));
    ...
    tree.insert(skill("fs_archive", "Archive management", Some("fs_root"),
        vec!["fs_classify"], false, false,
        vec!["Scholar", "Artisan"],
        vec![],
        Mastery::Locked,
...
pub fn browser_skill_tree() -> SkillTree {
    ...
    // Root
    tree.insert(skill("br_root", "Browser management", None,
        vec![], false, false,
    ...
    tree.insert(skill("br_navigate", "URL navigation", Some("br_root"),
        vec![], true, false, // SAT validates URLs
        vec!["Navigator"],
        vec!["br_history", "br_tabs"],
        Mastery::Novice,
    ));

    tree.insert(skill("br_history", "History traversal", Some("br_navigate"),
        vec!["br_navigate"], false, false,
        vec!["Navigator"],
        vec![],
        Mastery::Locked,
    ));
    tree.insert(skill("br_tabs", "Tab management", Some("br_navigate"),
        vec!["br_navigate"], false, false,
        vec!["Navigator"],
        vec![],
        Mastery::Locked,
    ...
    tree.insert(skill("br_read", "Page reading", Some("br_root"),
        vec![], false, false,
        vec!["Scholar"],
        vec!["br_extract", "br_find", "br_screenshot"],
        Mastery::Novice,
    ));
    tree.insert(skill("br_extract", "Text extraction", Some("br_read"),
        vec!["br_read"], false, false,
        vec!["Scholar"],
        vec![],
        Mastery::Locked,
    ));
    tree.insert(skill("br_find", "Element finding", Some("br_read"),
        vec!["br_read"], false, false,
        vec!["Scholar", "Artisan"],
        vec![],
        Mastery::Locked,
    ));
    tree.insert(skill("br_screenshot", "Visual capture", Some("br_read"),
        vec!["br_read"], false, false,
        vec!["Sentinel"],
        vec![],
        Mastery::Locked,
    ...
    tree.insert(skill("br_fill", "Form interaction", Some("br_root"),
        vec!["br_read"], true, false, // SAT validates field targets
        vec!["Artisan"],
        vec!["br_click"],
        Mastery::Locked,
    ));
    tree.insert(skill("br_click", "Element clicking", Some("br_fill"),
        vec!["br_fill"], true, false, // SAT validates click targets
        vec!["Artisan"],
        vec![],
        Mastery::Locked,
    ));
    ...
    tree.insert(skill("br_execute_js", "JavaScript execution", Some("br_root"),
        vec!["br_read", "br_fill", "br_click"], true, true, // full gate
        vec!["Scholar", "Sentinel"],
        vec![],
        Mastery::Locked,
    ));
...
mod tests {
    ...
    fn skill_node_records_success_and_promotes() {
        let mut node = skill("test", "Test skill", None,
            vec![], false, false, vec![], vec![], Mastery::Novice);
    ...
    fn skill_node_success_rate() {
        let mut node = skill("test", "Test", None,
            vec![], false, false, vec![], vec![], Mastery::Novice);
        node.successes = 8;
        node.failures = 2;
    ...
    fn merge_requires_both_classify_and_organize() {
        ...
        for _ in 0..3 { tree.record_success("fs_organize"); }
    ...
    fn browser_cascade_read_to_extract() {
        ...
        for _ in 0..3 { tree.record_success("br_read"); }
    ...
    fn browser_js_stays_locked_until_all_prereqs() {
        ...
        for _ in 0..3 { tree.record_success("br_fill"); }
        // JS still locked — needs click at Competent too
        assert_eq!(tree.nodes["br_execute_js"].mastery, Mastery::Locked);

        // Promote click → Competent
        for _ in 0..3 { tree.record_success("br_click"); }
        // NOW JS unlocks
