...
class ExpertTier(Enum):
    ...
    def __init__(
        self, level: int, key: str, size: str, cost_per_1k: float, tok_per_sec: float
    ):
        self.level = level
        self.key = key
        self.size = size
...
class SequentialAttention:
    ...
    def select_subset(
                ...
                if selected:
                    for _, sel_emb in selected:
                        diversity_penalty = max(
                            diversity_penalty, self.similarity_fn(emb, sel_emb)
                        )

                # Combined score
                score = (
                    1 - diversity_weight
                ) * relevance - diversity_weight * diversity_penalty
        ...
        return [
            (cid, self.similarity_fn(query_embedding, emb)) for cid, emb in selected
        ]
...
class MoERouter:
    ...
    def route(
        ...
        query_id = hex_digest(query.encode())[:12]
    ...
    def _generate_reasoning(
        ...
        if selected == default:
            return (
                f"Query classified as {complexity.name}, routed to {selected.key} tier"
            )
        else:
            return (
    ...
    async def federated_dispatch(
        ...
        total_cost = sum(
            (estimated_tokens / 1000) * tier.cost_per_1k for tier, _ in dispatches
        )
        ...
        if dispatches:
            ...
            dispatches_sorted = sorted(
                dispatches, key=lambda x: x[0].level, reverse=True
            )
...
if __name__ == "__main__":
    ...
    test_queries = [
        ...
        (
            "Explain the trade-offs between CAP theorem constraints in distributed systems.",
            None,
        ),
        (
            "Design a fault-tolerant consensus algorithm for a heterogeneous network.",
            None,
        ),
    ...
    for query, context in test_queries:
        ...
        print(
            f"  Estimated: {decision.estimated_tokens} tokens, "
