...
npx claude-flow github issue-to-swarm 456 \
...
npx claude-flow github issues-batch \
  --issues "$ISSUES" \
  --parallel
...
npx claude-flow github issue-analyze 456 \
  --suggest-agents \
  --estimate-complexity \
  --create-subtasks
```
...
# Initialize swarm
npx claude-flow github issue-init 456 \
  --issue-data "$ISSUE" \
...
SUBTASKS=$(npx claude-flow github issue-decompose 456 \
  --body "$ISSUE_BODY" \
  --max-subtasks 10 \
  --assign-priorities)
...
# Get swarm progress
PROGRESS=$(npx claude-flow github issue-progress 456)

# Update checklist in issue body
UPDATED_BODY=$(echo "$CURRENT" | jq -r '.body' | \
  npx claude-flow github update-checklist --progress "$PROGRESS")
...
npx claude-flow github issue-deps 456 \
  --resolve-order \
  --parallel-safe \
  --update-blocking
```
...
npx claude-flow github epic-swarm \
  --epic 123 \
...
npx claude-flow github create-issues \
  --from-analysis \
  --template "bug-report" \
  --auto-assign
```
        ...
        with:
          command: |
            if [[ "${{ github.event.label.name }}" == "swarm-ready" ]]; then
              npx claude-flow github issue-init ${{ github.event.issue.number }}
            fi
```
...
npx claude-flow github issue-board-sync \
  --project "Development" \
  --column-mapping '{
    "To Do": "pending",
    "In Progress": "active",
...
npx claude-flow github bug-swarm 456 \
  --reproduce \
  --isolate \
  --fix \
  --test
...
npx claude-flow github feature-swarm 456 \
  --design \
  --implement \
  --document \
  --demo
...
```bash
# Refactoring swarm
npx claude-flow github debt-swarm 456 \
  --analyze-impact \
  --plan-migration \
  --execute \
  --validate
...
echo "$STALE_ISSUES" | jq -r '.number' | while read -r num; do
  ...
  # Analyze with swarm
  ACTION=$(npx claude-flow github analyze-stale \
    --issue "$ISSUE" \
    --suggest-action)
...
npx claude-flow github find-duplicates \
  --threshold 0.8 \
  --link-related \
  --close-duplicates
```
...
npx claude-flow github link-pr \
  --issue 456 \
  --pr 789 \
  --update-both
```
...
npx claude-flow github milestone-swarm \
  --milestone "v2.0" \
  --parallel-issues \
  --track-progress
```
...
npx claude-flow github cross-repo \
  --issue "org/repo#456" \
...
npx claude-flow github issue-metrics \
  --issue 456 \
...
npx claude-flow github effectiveness \
...
npx claude-flow github issue-init 789 \
...
npx claude-flow github issue-init 234 \
  --topology mesh \
...
npx claude-flow github issue-init 567 \
  --topology ring \
