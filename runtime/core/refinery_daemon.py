...
class DaemonConfig:
    ...
    watch_paths: List[Path] = field(
        default_factory=lambda: [Path("bizra_data_vault/roots")]
    )
    ...
    priority_extensions: Set[str] = field(
        default_factory=lambda: {".py", ".rs", ".md", ".json"}
    )
...
class RefineryWorker:
    ...
    def __init__(
        self, config: DaemonConfig, file_queue: FileQueue, metrics: DaemonMetrics
    ):
        self.config = config
        self.queue = file_queue
...
class RefineryDaemon:
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"  Target throughput: {self.config.target_bytes_per_sec / (1024*1024):.1f} MB/sec"
        )
        ...
        try:
            ...
            self._http_server = HTTPServer(
                ("0.0.0.0", self.config.health_port), HealthHandler
            )
            threading.Thread(
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["bizra_data_vault/roots"],
        help="Directories to watch",
    )
    parser.add_argument(
        "--ledger",
        type=str,
        default="BIZRA_KNOWLEDGE_LEDGER.jsonl",
        help="Ledger output file",
    )
    parser.add_argument(
        "--throughput", type=float, default=10.0, help="Target throughput in MB/sec"
    )
    parser.add_argument("--port", type=int, default=8081, help="Health server port")
