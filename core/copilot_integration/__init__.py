...
class SnrTier(Enum):
    ...
    def from_ihsan(cls, score: float) -> 'SnrTier':
        ...
        if snr >= 9.0: return cls.T6
        if snr >= 8.6: return cls.T5
        if snr >= 8.2: return cls.T4
        if snr >= 7.8: return cls.T3
        if snr >= 7.4: return cls.T2
        if snr >= 7.0: return cls.T1
        return cls.T0
...
class ThinkingConfig:
    ...
    MODEL_BY_LEVEL: Dict[ThinkLevel, str] = field(default_factory=lambda: {
...
def select_thinking_level(
    ...
    return ThinkingConfig(
        level=level,
        reasoning=ReasoningLevel.ON if level.value >= ThinkLevel.MEDIUM.value else ReasoningLevel.OFF,
