...
impl InferenceGateway {
    ...
    pub async fn infer(&self, request: InferenceRequest) -> Result<InferenceResponse, GatewayError> {
        let start = Instant::now();
        let tier = request.preferred_tier.unwrap_or_else(|| self.selector.select_tier(&request.complexity));
        ...
        match result {
            Ok(Ok(mut response)) => {
                ...
                if response.duration_ms > 0 {
                    response.tokens_per_second = (response.completion_tokens as f32 * 1000.0) / response.duration_ms as f32;
                }
                Ok(response)
            }
