...
def validate_pat_agent(agent_name: str, config: Dict[str, str]) -> ValidationResult:
    ...
    if backend == "ollama":
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Respond with exactly: SOVEREIGNTY_CHECK_OK",
                }
            ],
            "max_tokens": 50,
            "stream": False,
    ...
    try:
        ...
        return ValidationResult(
            name=f"PAT:{agent_name}", status=ValidationStatus.FAIL, message=str(e)
        )


def benchmark_pat_agent(
    agent_name: str, config: Dict[str, str], iterations: int = 5
) -> BenchmarkStats:
...
def validate_sat_agent(agent_name: str, config: Dict[str, str]) -> ValidationResult:
    ...
    try:
        ...
        return ValidationResult(
            name=f"SAT:{agent_name}", status=ValidationStatus.FAIL, message=str(e)
        )
...
def validate_ihsan_gate() -> ValidationResult:
    ...
    try:
        ...
        return ValidationResult(
            name="Ihsān Gate", status=ValidationStatus.FAIL, message=str(e)
        )
...
def run_full_validation(
    ...
    if not offline_mode:
        ...
        if sovereign_ok:
            print("  ✅ Sovereignty: No external AI endpoints detected")
        else:
            print(f"  ⚠️ Sovereignty: External endpoints reachable: {violations}")
    else:
        print("  ℹ️ Sovereignty: Offline mode - network blocked")
    ...
    if not sat_only:
        ...
        for agent_name, config in PAT_AGENTS.items():
            ...
            if benchmark_iterations > 0 and result.status in (
                ValidationStatus.PASS,
    ...
    if benchmarks:
        ...
        print(
            f"  {'Agent':<20} {'Mean':>8} {'P50':>8} {'P95':>8} {'P99':>8} {'Success':>8}"
        )
        print("  " + "─" * 60)
        ...
        for stats in benchmarks:
            ...
            print(
                f"  {stats.agent:<20} {stats.mean_ms:>7.0f}ms {stats.p50_ms:>7.0f}ms "
...
def main():
    ...
    parser.add_argument(
        "--pat-only", action="store_true", help="Validate PAT agents only"
    )
    parser.add_argument(
        "--sat-only", action="store_true", help="Validate SAT agents only"
    )
    ...
    parser.add_argument(
        "--output", type=str, default=None, help="Save report to JSON file"
    )
    ...
    if args.output:
        ...
        output_path = (
            Path("docs/evidence/validation")
            / f"sovereignty_validation_{report.timestamp.replace(':', '-').replace('+', '_')}.json"
        )
