...
npx claude-flow github multi-repo-init \
...
npx claude-flow github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx claude-flow github task-execute \
...
npx claude-flow github link-prs --urls "$PR_URLS"
```
...
npx claude-flow github multi-repo-refactor \
...
npx claude-flow github multi-repo-security \
  --scan-all \
...
npx claude-flow github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx claude-flow github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx claude-flow github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx claude-flow github multi-repo-dashboard \
  --port 3000 \
...
npx claude-flow github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx claude-flow github health-check \
  --repos "org/*" \
...
npx claude-flow github microservices \
...
npx claude-flow github lib-update \
...
npx claude-flow github org-policy \
...
npx claude-flow github cache-strategy \
  --analyze-patterns \
...
npx claude-flow github parallel-optimize \
  --analyze-dependencies \
...
npx claude-flow github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx claude-flow github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx claude-flow github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx claude-flow github perf-analysis \
  --profile-operations \
...
npx claude-flow github fullstack-update \
...
npx claude-flow github cross-team \
